---
title: Set up hosts for live migration
ms.custom: na
...
- The Hyper\-V role in [!INCLUDE[winthreshold_server_2_mdincludes/winthreshold_server_2_md.md)] installed on the source and destination servers. For instructions, see [Install the Hyper\-V role and configure a virtual machine](assetId:///243b5705-96c9-4ec7-9ec5-c68a22b0d42d).  
...
- The Hyper\-V management tools installed on a computer running [!INCLUDE[winthreshold_server_2_mdincludes/winthreshold_server_2_md.md)] or [!INCLUDE[winthreshold_client_2_mdincludes/winthreshold_client_2_md.md)], unless the tools are installed on the source or destination server.  
...
-  **Authentication**: Which protocol will be used to authenticate live migration traffic between the source and destination servers? The choice determines whether you'll need to sign on to the source server before starting a live migration:   
   - Kerberos lets you avoid having to sign on to the server, but requires constrained delegation to be set up. Instructions for are included below.  
