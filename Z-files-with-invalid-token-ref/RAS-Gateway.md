...
Virtual networks are created by using Hyper-V Network Virtualization, which is a technology that was introduced in [!INCLUDE[win8_server_1](includes/win8_server_1_md.md)], and is improved in [!INCLUDE[winthreshold_server_2](includes/winthreshold_server_2_md.md)]. RAS Gateway is integrated with Hyper-V Network Virtualization, and is able to route network traffic effectively in circumstances where there are many different customers – or tenants – who have isolated virtual networks in the same datacenter.
...
In [!INCLUDE[winthreshold_server_2](includes/winthreshold_server_2_md.md)], RAS Gateway routes network traffic between the physical network and VM network resources, regardless of where the resources are located. You can use RAS Gateway to route network traffic between physical and virtual networks at the same physical location or at many different physical locations.
...
When you deploy RAS Gateway, the host servers running Hyper-V and the VMs that you configure as gateways must be running [!INCLUDE[winblue_server_2](includes/winblue_server_2_md.md)] or [!INCLUDE[winthreshold_server_2](includes/winthreshold_server_2_md.md)].
