...
---
# What&#39;s New in IPAM
This topic describes the IP Address Management (IPAM) functionality that is new or changed in [!INCLUDE[winthreshold_server_2](includes/winthreshold_server_2_md.md)].
...
Following are the new and improved features for IPAM in [!INCLUDE[winthreshold_server_2](includes/winthreshold_server_2_md.md)].
...
IPAM in [!INCLUDE[winthreshold_server_2](includes/winthreshold_server_2_md.md)] now supports /31, /32, and /128 subnets. For example, a two address subnet (/31 IPv4) may be required for a point-to-point link between switches. Also, some switches may require single loopback addresses (/32 for IPv4, /128 for IPv6).
...
IPAM in [!INCLUDE[winthreshold_server_2](includes/winthreshold_server_2_md.md)] now supports discovery of file-based, domain-joined DNS servers in an Active Directory forest in which IPAM is running.
...
In [!INCLUDE[winblue_server_2](includes/winblue_server_2_md.md)], IPAM was able to discover and manage DNS and DHCP servers belonging to the same Active Directory forest as the IPAM server. Now you can manage DNS and DHCP servers belonging to a different AD forest when it has a  two-way trust relationship with the forest where the IPAM server is installed. You can go to the **Configure Server Discovery** dialog box and add domains from the other trusted forests that you want to manage. After the servers are discovered, the management experience is the same as for the servers that belong to the same forest where IPAM is installed.
