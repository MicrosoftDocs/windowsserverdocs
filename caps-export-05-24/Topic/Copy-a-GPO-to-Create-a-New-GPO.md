...
13. If required, change the WMI filter to one appropriate for the new GPO. For example, if the original GPO is for client computers running [!INCLUDE[win8_client_2](../Token/win8_client_2_md.md)], and the new boundary zone GPO is for computers running [!INCLUDE[win8_server_2](../Token/win8_server_2_md.md)], then select a WMI filter that allows only those computers to read and apply the GPO.  
