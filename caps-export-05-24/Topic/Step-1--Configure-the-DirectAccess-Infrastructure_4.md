---
title: Step 1: Configure the DirectAccess Infrastructure_4
ms.custom: na
...
ms.tgt_pltfrm: na
ms.topic: article
ms.assetid: aad8e18a-4b20-4140-b209-057a041a92d3
---
# Step 1: Configure the DirectAccess Infrastructure_4
This topic describes how to configure the infrastructure required for enabling DirectAccess in an existing VPN deployment. Before beginning the deployment steps, ensure that you have completed the planning steps described in [Step 1: Plan the DirectAccess Infrastructure_3](../Topic/Step-1--Plan-the-DirectAccess-Infrastructure_3.md).  
...
> [!IMPORTANT]  
> The administrator can manually link the DirectAccess [!INCLUDE[gpos](../Token/gpos_md.md)] to an Organizational Unit using these steps:  
>   
...
> 4.  If the linking to the OU has not been done before running the DirectAccess wizard, then after the configuration is complete, the domain administrator can link the DirectAccess [!INCLUDE[gpos](../Token/gpos_md.md)] to the required Organizational Units. The link to the domain can be removed. Steps for linking a [!INCLUDE[gpo](../Token/gpo_md.md)] to an Organization Unit can be found [here](http://technet.microsoft.com/library/cc732979.aspx).  
  
> [!NOTE]  
> If a [!INCLUDE[gpo](../Token/gpo_md.md)] was created manually, it is possible during the DirectAccess configuration that the [!INCLUDE[gpo](../Token/gpo_md.md)] will not be available. The [!INCLUDE[gpo](../Token/gpo_md.md)] may not have been replicated to the closest Domain Controller to the management computer. In this event, the administrator can wait for replication to complete, or force the replication.  
  
## <a name="ConfigSGs"></a>Configure security groups  
The DirectAccess settings contained in the client computer [!INCLUDE[gpo](../Token/gpo_md.md)] are applied only to computers that are members of the security groups that you specify when configuring Remote Access. In addition, if you are using security groups to manage your application servers, create a security group for these servers.  
