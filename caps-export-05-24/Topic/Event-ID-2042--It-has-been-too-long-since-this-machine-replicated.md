...
exceeded the tombstone lifetime.</code>
  ...
  <section>
    ...
    <content>
      ...
      <para>Membership in <system>Domain Admins</system>, or equivalent, is the minimum required to complete this procedure. <token>gen_win7_review_details</token></para>
      <procedure>
        <title>Identify lingering objects</title>
        <steps class="ordered">
          <step>
            <content>
              <para>On a domain controller that you expect to have the latest changes, open an elevated Command Prompt window. <token>cmd_elev</token></para>
            </content>
          </step>
          <step>
            <content>
    ...
    <sections>
      <section>
        ...
        <content>
          ...
          <alert class="caution">
            <para>Restart replication only after you have removed all lingering objects. <token>Registry</token></para>
          </alert>
          <para>Membership in <embeddedLabel>Domain Admins</embeddedLabel>, or equivalent, is the minimum required to complete this procedure. <token>gen_win7_review_details</token></para>
          <procedure>
            <title>Use Repadmin to restart replication following Event ID 2042</title>
            <steps class="ordered">
              <step>
                <content>
                  <para>Open an elevated Command Prompt. <token>cmd_elev</token></para>
                </content>
              </step>
              <step>
                <content>
