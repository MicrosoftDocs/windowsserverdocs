...
ms.tgt_pltfrm: na
ms.topic: article
ms.assetid: 6cd9e1c8-0093-4d32-8ff2-6d235077ab96
author: billmath
---
# Configure Client Computers to Trust the Account Federation Server
So that client computers can successfully access federated applications using [!INCLUDE[firstref_adfs2](../Token/firstref_adfs2_md.md)], you must first configure the Internet Explorer settings on each client computer so that the browser trusts the account [!INCLUDE[adfs2_fs](../Token/adfs2_fs_md.md)]. You can do this manually or through Group Policy, depending on your administrative preference, by completing one of the following procedures.  
  
## Configuring Internet Explorer settings manually  
You can use the following procedure to manually configure each user's Internet Explorer settings to support federation through [!INCLUDE[nextref_adfs2](../Token/nextref_adfs2_md.md)]. If multiple users use a single computer, complete this procedure multiple times—once for each user profile.  
...
4.  Click **Advanced**, and in **Add this Web site to the zone**, type the full Domain Name System \(DNS\) name of the account [!INCLUDE[adfs2_fs](../Token/adfs2_fs_md.md)] \(for example, https:\/\/fs1.fabrikam.com\), and then click **Add**.  
...
7.  In **Add this Web site to the zone**, type the full DNS name of the account [!INCLUDE[adfs2_fs](../Token/adfs2_fs_md.md)] \(for example, https:\/\/fs1.fabrikam.com\), click **Add**, and then click **Close**.  
