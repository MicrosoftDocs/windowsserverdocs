---
title: Managing Client Computers and Computer Groups
ms.custom: na
...
ms.tgt_pltfrm: na
ms.topic: article
ms.assetid: a806d9b1-b806-435a-8155-376db7365b37
author: coreyp
---
# Managing Client Computers and Computer Groups
...
Selecting one of the computer groups in the Computers node causes the computers in that group to be displayed in the Details pane. \(If a computer is assigned to multiple groups, it will appear in the listings of both groups.\) If you select a computer in the list, you can see its properties, which include general details about the computer and the status of updates for it, such as the installation or detection status of an update for a particular computer. You can filter the list of computers under a given computer group by status. The default shows only computers for which updates are needed or which have had installation failures; however, you can filter the display by any status. Click **Refresh** after changing the status filter.  
  
You can also manage computer groups on the **Computers** page, which includes creating the groups and assigning computers to them. For more information about managing computer groups, see Managing Computer Groups in the next section of this guide, and section [1.5 Plan WSUS Computer Groups](http://technet.microsoft.com/library/hh852344.aspx#BKMK_1_5) in Step 1: Prepare for Your WSUS Deployment in the WSUS deployment guide.  
  
> [!IMPORTANT]  
> You must first configure client computers to contact the WSUS server before you can manage them from that server. Until you perform this task, your WSUS server will not recognize your client computers and they will not be displayed in the list on the Computers page. For more information about setting up client computers, see [1.5. Plan WSUS computer groups](http://technet.microsoft.com/library/hh852344.aspx#BKMK_1_5) of Step 1: Prepare for Your WSUS Deployment, and [Step 3: Configure WSUS](http://technet.microsoft.com/library/hh852346.aspx), in the WSUS deployment guide.  
...
1.  Approval with deadlines: Deadlines strictly enforce when an update is installed.  
  
2.  WSUS Group Policies: Group Policies control when the Windows Update Agent scans and installs updates  
  
    Several new group policies were introduces in [!INCLUDE[winblue_client_1](../Token/winblue_client_1_md.md)] and [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)]. For more information, see: [Step 5: Configure Group Policy Settings for Automatic Updates](http://technet.microsoft.com/library/dn595129.aspx)  
  
## <a name="BKMK_PlanGroups"></a>Managing Computer Groups  
...
> [!WARNING]  
> If a WSUS server is running in replica mode, computer groups cannot be created on that server. All the computer groups needed for clients of the replica server must be created on the WSUS server that is the root of the WSUS server hierarchy. For more information about replica mode, see [Running WSUS Replica Mode_1](../Topic/Running-WSUS-Replica-Mode_1.md) and for more information about server\-side and client\-side targeting, see section [1.5. Plan WSUS computer groups](http://technet.microsoft.com/library/hh852344.aspx#BKMK_1_5) of Step 1: Prepare for Your WSUS Deployment in the WSUS deployment guide.  
