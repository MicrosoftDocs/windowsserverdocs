---
title: Step 4: Verify the Multisite Deployment_1
ms.custom: na
...
ms.tgt_pltfrm: na
ms.topic: article
ms.assetid: 9d278e9a-2e1f-49a9-a4b1-8b02de484c18
---
# Step 4: Verify the Multisite Deployment_1
...
4.  Connect a [!INCLUDE[firstref_client_7](../Token/firstref_client_7_md.md)] client computer to the corporate network and obtain the group policy.  
  
5.  Connect the [!INCLUDE[nextref_client_7](../Token/nextref_client_7_md.md)] client computer to the external network and attempt to access internal resources.  
  
    You should be able to access all corporate resources.  
  
6.  Test connectivity for [!INCLUDE[nextref_client_7](../Token/nextref_client_7_md.md)] clients through each server in the multisite deployment by accessing the Active Directory Users and Computers console, and moving the client computer to the security group that corresponds to each server. After the changes have replicated throughout the domain, restart the client computer while connected to the corporate network to obtain the new group policy. Attempt to access corporate resources. Repeat the test on a different multisite server.  
  
    You should be able to access all corporate resources through each multisite server.  
  
    In a production environment this method may not be feasible due to the amount of time required for changes to replicate throughout the domain. You may want to force replication where possible. Testing can also be done from multiple different [!INCLUDE[nextref_client_7](../Token/nextref_client_7_md.md)] client computers that are already members of the different [!INCLUDE[nextref_client_7](../Token/nextref_client_7_md.md)] security groups in the multisite deployment.  
