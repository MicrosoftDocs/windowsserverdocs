...
Local security information is stored in the registry under **HKEY\_LOCAL\_MACHINE\\SECURITY**. Stored information includes policy settings, default security values, and account information, such as cached logon credentials. A copy of the SAM database is also stored here, although it is write\-protected.  
...
In [!INCLUDE[nextref_longhorn](../Token/nextref_longhorn_md.md)] and [!INCLUDE[nextref_vista](../Token/nextref_vista_md.md)], the Graphical Identification and Authentication \(GINA\) architecture was replaced with a credential provider model, which made it possible to enumerate different logon types through the use of logon tiles. Both models are described below.  
...
![](../Image/AuthN_GINA_Architecture.gif)  
...
![](../Image/AuthN_CredMan_CredProv.gif)  
...
Managed service accounts and virtual accounts were introduced in [!INCLUDE[nextref_server_7](../Token/nextref_server_7_md.md)] and [!INCLUDE[nextref_client_7](../Token/nextref_client_7_md.md)] to provide crucial applications, such as Microsoft SQL Server and Internet Information Services \(IIS\), with the isolation of their own domain accounts, while eliminating the need for an administrator to manually administer the service principal name \(SPN\) and credentials for these accounts. For more information about these features and their role in authentication, see [Managed Service Accounts Documentation for Windows 7 and Windows Server 2008 R2](http://technet.microsoft.com/library/ff641731(v=ws.10).aspx) and [Group Managed Service Accounts Overview](../Topic/Group-Managed-Service-Accounts-Overview.md).  
...
> [!NOTE]  
> Services normally run in security contexts known as Local System \(SYSTEM\), Network Service, or Local Service. [!INCLUDE[nextref_server_7](../Token/nextref_server_7_md.md)] introduced services that run under a managed service account, which are domain principals.  
...
Introduced in [!INCLUDE[winblue_client_2](../Token/winblue_client_2_md.md)], the client operating system provides additional protection for the LSA to prevent reading memory and code injection by non\-protected processes. This protection increases security for the credentials that the LSA stores and manages.  
  
For more information about these additional protections, see [Configuring Additional LSA Protection](../Topic/Configuring-Additional-LSA-Protection.md).  
...
Credential Manager was introduced in [!INCLUDE[nextref_server_7](../Token/nextref_server_7_md.md)] and [!INCLUDE[nextref_client_7](../Token/nextref_client_7_md.md)] as a Control Panel feature to store and manage user names and passwords. Credential Manager lets users store credentials relevant to other systems and websites in the secure Windows Vault. Some versions of Internet Explorer use this feature for authentication to websites.  
...
> [!NOTE]  
> SHA1 is the default in [!INCLUDE[nextref_client_7](../Token/nextref_client_7_md.md)] and [!INCLUDE[nextref_vista](../Token/nextref_vista_md.md)], but was changed to SHA2 in [!INCLUDE[win8_client_2](../Token/win8_client_2_md.md)].  
...
Virtual smart card technology was introduced in [!INCLUDE[win8_client_2](../Token/win8_client_2_md.md)]. It stores the smart card’s certificate in the PC, and then protects it by using the device’s tamper\-proof Trusted Platform Module \(TPM\) security chip. In this way, the PC actually becomes the smart card which must receive the user’s PIN in order to be authenticated.  
