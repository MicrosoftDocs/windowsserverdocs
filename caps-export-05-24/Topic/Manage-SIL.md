...
---
# Manage SIL
This document describes how to manage Software Inventory Logging in [!INCLUDE[winblue_server_1](../Token/winblue_server_1_md.md)].  
  
The Software Inventory Logging feature that was introduced in [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)], is intended to help datacenter administrators easily log Microsoft software asset management data for their deployments over time. This document describes how to manage Software Inventory Logging. Before using Software Inventory Logging, make sure that Windows Update [KB 3000850](http://support.microsoft.com/kb/3000850) and [KB 3060681](http://support.microsoft.com/kb/3060681) are installed on each [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] system needing to be inventoried. This feature runs locally on each server to be inventoried. It does not collect data from remote servers.  
...
-   [Starting and Stopping Software Inventory Logging](../Topic/Manage-Software-Inventory-Logging-in-Windows-Server-2012-R2.md#BKMK_Step1)  
  
-   [Software Inventory Logging over time](../Topic/Manage-Software-Inventory-Logging-in-Windows-Server-2012-R2.md#BKMK_Step2)  
  
-   [Displaying Software Inventory Logging data](../Topic/Manage-Software-Inventory-Logging-in-Windows-Server-2012-R2.md#BKMK_Step3)  
  
-   [Deleting data logged by Software Inventory Logging](../Topic/Manage-Software-Inventory-Logging-in-Windows-Server-2012-R2.md#BKMK_Step4)  
  
-   [Backing up and restoring data logged by Software Inventory Logging](../Topic/Manage-Software-Inventory-Logging-in-Windows-Server-2012-R2.md#BKMK_Step5)  
  
-   [Reading data logged and published by Software Inventory Logging](../Topic/Manage-Software-Inventory-Logging-in-Windows-Server-2012-R2.md#BKMK_Step6)  
  
-   [Software Inventory Logging Security](../Topic/Manage-Software-Inventory-Logging-in-Windows-Server-2012-R2.md#BKMK_Step7)  
  
-   [Working with date and time settings in Windows Server Software Inventory Logging](../Topic/Manage-Software-Inventory-Logging-in-Windows-Server-2012-R2.md#BKMK_Step8)  
  
-   [Enabling and Configuring Software Inventory Logging in a Mounted Virtual Hard Disk](../Topic/Manage-Software-Inventory-Logging-in-Windows-Server-2012-R2.md#BKMK_Step10)  
  
-   [Overview of Using Software Inventory Logging in Windows Server 2012 R2 without KB 3000850](../Topic/Manage-Software-Inventory-Logging-in-Windows-Server-2012-R2.md#BKMK_Step11)  
  
-   [Using Software Inventory Logging in a Windows Server 2012 R2 Hyper\-V environment without KB 3000850](../Topic/Manage-Software-Inventory-Logging-in-Windows-Server-2012-R2.md#BKMK_Step12)  
  
> [!NOTE]  
> [!INCLUDE[wps_howtorun](../Token/wps_howtorun_md.md)]  
  
## <a name="BKMK_Step1"></a>Starting and Stopping Software Inventory Logging  
Software Inventory Logging daily collection and forward over the network must be enabled on a computer running [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] to log software inventory.  
...
For a comprehensive guide to setting up the SIL framework as a whole, see [Software Inventory Logging Aggregator](../Topic/Software-Inventory-Logging-Aggregator.md).  In particular, if **Publish\-SilData** produces an error, or SIL Logging fails otherwise, see the troubleshooting section .  
...
> [!NOTE]  
> If for any reason managing the retention duration of data logged locally by SIL becomes important, this can be configured by changing the registry value here: \\HKEY\_LOCAL\_MACHINE\\SOFTWARE\\Microsoft\\Windows\\SoftwareInventoryLogging. The default is ‘30’ for 30 days.  
...
To support these uses, Software Inventory Logging has registry entries associated with each configurable option.  These registry values can be found under \\HKEY\_LOCAL\_MACHINE\\SOFTWARE\\Microsoft\\Windows\\SoftwareInventoryLogging.  
...
When using Software Inventory Logging on a [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] Hyper\-V host, it is possible to retrieve SIL data from [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] guests that are running locally, if SIL logging has been started in the guest\(s\).  However, this is only possible when using the Get\-SilData and Publish\-SilData Powershell cmdlets, and only possible with [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] in both host and guest.  The purpose of this capability is to allow data center administrators that provide guest VMs to tenants, or other entities of a large corporation, to capture software inventory data at the hypervisor host and subsequently forward all of this data to an aggregator \(or target URI\).  
  
Below are two examples of what the output on the Powershell console would look like \(much abbreviated\) on a Windows Server 2012 R2 Hyper\-V host running one [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] guest VM with SIL logging started.  You will notice the first example, which uses Get\-SilData alone, will output all data from the host as expected.  Also included is all SIL data from the guest, but in a collapsed format.  To expand and view this data from the guest, simply cut and paste the snippet used in the second example below.  SIL data objects from the guest will always have the VM GUID associated within the object.  
...
![](../Image/SILHyper-VExample1.png)  
  
**Output Example 2** \(w\/Expand\-SilData function\)  
  
![](../Image/SILHyper-VExample2.png)  
  
## See Also  
[Software Inventory Logging Overview](../Topic/Software-Inventory-Logging-Overview.md)  
[Software Inventory Logging Aggregator](../Topic/Software-Inventory-Logging-Aggregator.md)  
