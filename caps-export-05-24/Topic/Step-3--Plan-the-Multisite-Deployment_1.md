---
title: Step 3: Plan the Multisite Deployment_1
ms.custom: na
...
ms.tgt_pltfrm: na
ms.topic: article
ms.assetid: 19d49dbf-1786-47bb-ab97-f0458c53d91d
---
# Step 3: Plan the Multisite Deployment_1
...
|Task|Description|  
|--------|---------------|  
|[3.1 Plan IP-HTTPS certificates](../Topic/Step-3--Plan-the-Multisite-Deployment_1.md#bkmk_3_1_IPHTTPS)|Plan the certificate to use for the IP\-HTTPS website for each entry point.|  
|[3.2 Plan the network location server](../Topic/Step-3--Plan-the-Multisite-Deployment_1.md#bkmk_3_2_NLS)|Decide where to host the network location server website and the certificate to use.|  
|[3.3 Plan the IPsec root certificate for all Remote Access servers](../Topic/Step-3--Plan-the-Multisite-Deployment_1.md#bkmk_3_3_IPsec)|Plan to configure all Remote Access servers with the same IPsec root certificate.|  
|[3.4 Plan global server load balancing](../Topic/Step-3--Plan-the-Multisite-Deployment_1.md#bkmk_3_4_GSLB)|Decide whether to use global server load balancing.|  
|[3.5 Plan DirectAccess client entry point selection](../Topic/Step-3--Plan-the-Multisite-Deployment_1.md#bkmk_3_5_EP_Selection)|Plan how DirectAccess client computers will select the entry point to connect to.|  
|[3.6 Plan prefixes and routing](../Topic/Step-3--Plan-the-Multisite-Deployment_1.md#bkmk_3_6_IPv6)|Plan the IPv6 prefixes for your deployment.|  
|[3.7 Plan the transition to IPv6 when multisite Remote Access is deployed](../Topic/Step-3--Plan-the-Multisite-Deployment_1.md#bkmk_3_7_TransitionIPv6)|Plan for a transition to an IPv6\-only multisite deployment.|  
...
When you configure a multisite deployment, by default, [!INCLUDE[win8_client_2](../Token/win8_client_2_md.md)] client computers are configured with the information required to connect to all entry points in the deployment and to automatically connect to a single entry point based on a selection algorithm. You can also configure your deployment to allow [!INCLUDE[win8_client_2](../Token/win8_client_2_md.md)] client computers to manually select the entry point to which they will connect. If a [!INCLUDE[win8_client_2](../Token/win8_client_2_md.md)] client computer is currently connected to the United States entry point and automatic entry point selection is enabled, if the United States entry point becomes unreachable, after a few minutes the client computer will attempt to connect through the Europe entry point. Using automatic entry point selection is recommended; however, allowing manual entry point selection enables end users to connect to a different entry point based on current network conditions. For example, if a computer is connected to the United States entry point and the connection to the internal network becomes much slower than expected. In this situation, the end user can manually select to connect to the Europe entry point to improve the connection to the internal network.  
...
[!INCLUDE[nextref_client_7](../Token/nextref_client_7_md.md)] client computers are configured with the information required to connect to a single entry point in the multisite deployment. They cannot store the information for multiple entry points simultaneously. For example, a [!INCLUDE[nextref_client_7](../Token/nextref_client_7_md.md)] client computer can be configured to connect to the United States entry point, but not the Europe entry point. If the United States entry point is unreachable, the [!INCLUDE[nextref_client_7](../Token/nextref_client_7_md.md)] client computer will lose connectivity to the internal network until the entry point is reachable. The end user cannot make any changes to attempt to connect to the Europe entry point.  
...
When a client computer running [!INCLUDE[win8_client_2](../Token/win8_client_2_md.md)] is connected to an entry point, the client computer is immediately associated with the Active Directory site of the entry point, and is configured with IPv6 prefixes associated with the entry point. The preference is for client computers to connect to resources using these IPv6 prefixes, since they are configured dynamically in the IPv6 prefix policy table with higher precedence when connecting to an entry point.  
...
1.  For each GPO used for [!INCLUDE[win8_client_2](../Token/win8_client_2_md.md)] client computers, run the Set\-DAEntryPointTableItem PowerShell cmdlet  
...
To transform the deployment to an IPv6\+IPv4 deployment and take advantage of the native IPv6 capabilities, you must reinstall DirectAccess. To maintain client connectivity throughout the reinstallation see [Transition from an IPv4-only to an IPv6-only deployment using dual DirectAccess deployments](../Topic/Step-3--Plan-the-Multisite-Deployment_1.md#DualDeployment).  
...
-   [Configure a Multisite Deployment](../Topic/Configure-a-Multisite-Deployment.md)  
