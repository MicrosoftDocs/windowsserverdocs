...
The overall function of the Federation Service in [!INCLUDE[firstref_adfs2](../Token/firstref_adfs2_md.md)] is to issue a token that contains a set of claims. The decision regarding what claims [!INCLUDE[nextref_adfs2](../Token/nextref_adfs2_md.md)] accepts and then issues is governed by claim rules.  
...
For more information about the claims pipeline, see [The Role of the Claims Engine](../Topic/The-Role-of-the-Claims-Engine.md).  
...
For more information about this process, see [The Role of the Claims Engine](../Topic/The-Role-of-the-Claims-Engine.md).  
  
## What are claim rule templates?  
[!INCLUDE[nextref_adfs2](../Token/nextref_adfs2_md.md)] includes a predefined set of claim rule templates that are designed to help you easily select and create the most appropriate claim rules for your particular business need. Claim rule templates are only used during the claim rule creation process.  
  
In the [!INCLUDE[nextref_adfs2](../Token/nextref_adfs2_md.md)] Management snap\-in, rules can only be created using claim rule templates. After you use the snap\-in to select a claim rule template, input the necessary data for the rule logic and save it to the configuration database, it will be \(from that point forward\) referred to in the UI as a claim rule.  
...
For more information about how to use the claim rule language syntax, see [The Role of the Claim Rule Language](../Topic/The-Role-of-the-Claim-Rule-Language.md) in the [!INCLUDE[nextref_adfs2](../Token/nextref_adfs2_md.md)] Deployment Guide.  
...
All of these options coexist to provide you with the flexibility of choosing the appropriate method for a given scenario. For more information about how to create a claim rule, see [Configuring Claim Rules](../Topic/Configuring-Claim-Rules.md) in the [!INCLUDE[nextref_adfs2](../Token/nextref_adfs2_md.md)]Deployment Guide.  
...
For more information describing each of these claim rule templates, see [Determine the Type of Claim Rule Template to Use](../Topic/Determine-the-Type-of-Claim-Rule-Template-to-Use.md).  
...
You can also use the ADFSClaimRuleSet cmdlet object with Windows PowerShell to create or administer rules in [!INCLUDE[nextref_adfs2](../Token/nextref_adfs2_md.md)]. For more information about how you can use Windows PowerShell with this cmdlet, see [AD FS Administration with Windows PowerShell](http://go.microsoft.com/fwlink/?LinkID=179634).  
...
For more information about select the appropriate claim rules to use in your organization, see [Determine the Type of Claim Rule Template to Use](../Topic/Determine-the-Type-of-Claim-Rule-Template-to-Use.md).  
