...
2.  [Create security groups for Windows 7 DirectAccess clients on DC1](assetId:///205ca795-93ce-4e53-aa6b-b44c87f0e14a#SecGroup)—When DirectAccess is configured, it automatically creates [!INCLUDE[gpos](../Token/gpos_md.md)] \(GPOs\) and GPO settings that are applied to DirectAccess clients and servers. The DirectAccess client GPO is applied to specific Active Directory security groups.  
...
Create the DirectAccess security groups for [!INCLUDE[firstref_client_7](../Token/firstref_client_7_md.md)] with the following procedure.  
  
[!INCLUDE[nextref_client_7](../Token/nextref_client_7_md.md)] client computers must be members of separate security groups because they are able to connect to internal resources through a single entry point only. When enabling Multisite support or adding entry points, if [!INCLUDE[nextref_client_7](../Token/nextref_client_7_md.md)] support is requested, then a separate GPO will be automatically created by DirectAccess for [!INCLUDE[nextref_client_7](../Token/nextref_client_7_md.md)] clients for each entry point.  
...
1.  [!INCLUDE[clickstart](../Token/clickstart_md.md)]**dssite.msc**, and then press ENTER.  
