...
-   Specifying an alternate source path on the **Confirm installation options** page while you are installing features by using the [!INCLUDE[arfw](../Token/arfw_md.md)]  
...
This section describes how to set up a remote feature file shared folder \(also called a side\-by\-side store\) that stores the files required to install [!INCLUDE[rrsandf_plural](../Token/rrsandf_plural_md.md)] on servers that run [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] or [!INCLUDE[win8_server_2](../Token/win8_server_2_md.md)]. After you have set up a feature store, you can install [!INCLUDE[rrsandf_plural](../Token/rrsandf_plural_md.md)] on servers that are running those operating systems, and specify the feature store as the location of installation source files.  
...
-   The `Remove` parameter of the `Uninstall-WindowsFeature` cmdlet lets you delete feature files from a server or offline virtual hard disk \(VHD\) that is running [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] or [!INCLUDE[win8_server_2](../Token/win8_server_2_md.md)]. Valid values for the `Remove` parameter are the names of [!INCLUDE[rrsandf_plural](../Token/rrsandf_plural_md.md)].  
...
You can use the Uninstall\-WindowsFeature cmdlet both to uninstall [!INCLUDE[rrsandf_plural](../Token/rrsandf_plural_md.md)] from servers and offline VHDs that are running [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] or [!INCLUDE[win8_server_2](../Token/win8_server_2_md.md)], and to delete feature files. You can both uninstall and delete the same [!INCLUDE[rrsandf_plural](../Token/rrsandf_plural_md.md)] in the same command if desired.  
  
> [!IMPORTANT]  
> When you delete feature files for a [!INCLUDE[rrsorf_sing](../Token/rrsorf_sing_md.md)], [!INCLUDE[rrsandf_plural](../Token/rrsandf_plural_md.md)] that depend upon the files you are removing are also deleted. If you are deleting feature files for a role service or subfeature, and no other role services or subfeatures for the parent role or feature remain installed, then files for the entire parent role or feature are deleted. To view all feature files that would be deleted by the `Uninstall-WindowsFeature -Remove` command, add the `whatIf` parameter to the command to run it and view results without actually deleting feature files.  
...
## See Also  
[Install or Uninstall Roles, Role Services, or Features_1](../Topic/Install-or-Uninstall-Roles,-Role-Services,-or-Features_1.md)  
