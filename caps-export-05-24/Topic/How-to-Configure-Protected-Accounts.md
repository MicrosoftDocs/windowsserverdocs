...
Through Pass\-the\-hash \(PtH\) attacks, an attacker can authenticate to a remote server or service by using the underlying NTLM hash of a user's password \(or other credential derivatives\). Microsoft has previously [published guidance](http://www.microsoft.com/download/details.aspx?id=36036) to mitigate pass\-the\-hash attacks. [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] includes new features to help mitigate such attacks further. For more information about other security features that help protect against credential theft, see [Credentials Protection and Management](http://technet.microsoft.com/library/dn408190.aspx). This topic explains how to configure the following new features:  
  
-   [Protected Users](../Topic/How-to-Configure-Protected-Accounts.md#BKMK_AddtoProtectedUsers)  
  
-   [Authentication policies](../Topic/How-to-Configure-Protected-Accounts.md#BKMK_CreateAuthNPolicies)  
  
-   [Authentication policy silos](../Topic/How-to-Configure-Protected-Accounts.md#BKMK_CreateAuthNPolicySilos)  
...
Protected Users is a new global security group to which you can add new or existing users. [!INCLUDE[winblue_client_2](../Token/winblue_client_2_md.md)] devices and [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] hosts have special behavior with members of this group to provide better protection against credential theft. For a member of the group, a [!INCLUDE[winblue_client_2](../Token/winblue_client_2_md.md)] device or a [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] host does not cache credentials that are not supported for Protected Users. Members of this group have no additional protection if they are logged on to a device that runs a version of Windows earlier than [!INCLUDE[winblue_client_2](../Token/winblue_client_2_md.md)].  
  
Members of the Protected Users group who are signed\-on to [!INCLUDE[winblue_client_2](../Token/winblue_client_2_md.md)] devices and [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] hosts can *no longer* use:  
...
If the domain functional level is [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)], members of the group can no longer:  
...
Members of the Protected Users group must be able to authenticate by using Kerberos with Advanced Encryption Standards \(AES\). This method requires AES keys for the account in Active Directory. The built\-in Administrator does not have an AES key unless the password was changed on a domain controller that runs [!INCLUDE[nextref_longhorn](../Token/nextref_longhorn_md.md)] or later. Additionally, any account, which has a password that was changed at a domain controller that runs an earlier version of Windows Server, is locked out. Therefore, follow these best practices:  
  
-   Do not test in domains unless **all domain controllers run [!INCLUDE[nextref_longhorn](../Token/nextref_longhorn_md.md)] or later**.  
...
-   **Change password** for each user before adding the account to the Protected Users group or ensure that the password was changed recently on a domain controller that runs [!INCLUDE[nextref_longhorn](../Token/nextref_longhorn_md.md)] or later.  
...
-   To provide client\-side restrictions for Protected Users, hosts must run [!INCLUDE[winblue_client_2](../Token/winblue_client_2_md.md)] or [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)]. A user only has to sign\-on with an account that is a member of a Protected Users group. In this case, the Protected Users group can be created by [transferring the primary domain controller \(PDC\) emulator role](http://technet.microsoft.com/library/cc816944(v=ws.10).aspx) to a domain controller that runs [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)]. After that group object is replicated to other domain controllers, the PDC emulator role can be hosted on a domain controller that runs an earlier version of Windows Server.  
  
-   To provide domain controller\-side restrictions for Protected Users, that is to restrict usage of NTLM authentication, and other restrictions, the domain functional level must be [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)]. For more information about functional levels, see [Understanding Active Directory Domain Services \(AD DS\) Functional Levels](http://technet.microsoft.com/library/understanding-active-directory-functional-levels(v=WS.10).aspx).  
...
![](../Image/ADDS_ProtectAcct_TGTExpiration.png)  
...
![](../Image/ADDS_ProtectAcct_TshootDelegation.gif)  
...
-   Reject Data Encryption Standard \(DES\) in Kerberos pre\-authentication: [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] domain controllers do not accept DES for computer accounts unless they are configured for DES only because every version of Windows released with Kerberos also supports RC4.  
...
In [!INCLUDE[win8_server_2](../Token/win8_server_2_md.md)], Dynamic Access Control introduced an Active Directory forest\-scope object class called Central Access Policy to provide an easy way to configure file servers across an organization. In [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)], a new object class called Authentication Policy \(objectClass msDS\-AuthNPolicies\) can be used to apply authentication configuration to account classes in [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] domains. Active Directory account classes are:  
...
![](../Image/ADDS_ProtectAcct_KerbRefresher.gif)  
...
![](../Image/ADDS_ProtectAcct_RestrictAS.gif)  
...
|Policy|Requirements|  
|----------|----------------|  
|Provide custom TGT lifetimes|[!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] domain functional level account domains|  
|Restrict user sign\-on|-   [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] domain functional level account domains with Dynamic Access Control support<br />-   [!INCLUDE[win8_client_2](../Token/win8_client_2_md.md)], [!INCLUDE[winblue_client_2](../Token/winblue_client_2_md.md)], [!INCLUDE[win8_server_2](../Token/win8_server_2_md.md)] or [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] devices with Dynamic Access Control support|  
|Restrict service ticket issuance that is based on user account and security groups|[!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] domain functional level resource domains|  
|Restrict service ticket issuance based on user claims or device account, security groups, or claims|[!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] domain functional level resource domains with Dynamic Access Control support|  
...
The user’s account domain must be at [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] domain functional level \(DFL\). Ensure all the domain controllers are [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)], and then use Active Directory Domains and Trusts to [raise the DFL](http://technet.microsoft.com/library/cc753104.aspx) to [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)].  
...
1.  In the Default Domain Controllers Policy, click **Enabled** to enable **Key Distribution Center \(KDC\) client support for claims, compound authentication and Kerberos armoring** in Computer Configuration | Administrative Templates | System | KDC.  
    ...
    ![](../Image/ADDS_ProtectAcct_EnableKDCClaims.gif)  
  
2.  Under **Options**, in the drop\-down list box, select **Always provide claims**.  
  
    > [!NOTE]  
    > **Supported** can also be configured, but because the domain is at [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] DFL, having the DCs always provide claims will allow user claims\-based access checks to occur when using non\-claims aware devices and hosts to connect to claims\-aware services.  
  
    ![](../Image/ADDS_ProtectAcct_AlwaysProvideClaims.png)  
  
    > [!WARNING]  
    > Configuring **Fail unarmored authentication requests** will result in authentication failures from any operating system which does not support Kerberos armoring, such as [!INCLUDE[nextref_client_7](../Token/nextref_client_7_md.md)] and previous operating systems, or operating systems beginning with [!INCLUDE[win8_client_2](../Token/win8_client_2_md.md)], which have not been explicitly configured to support it.  
  
#### Create a user account audit for authentication policy with ADAC  
  
1.  Open Active Directory Administrative Center \(ADAC\).  
  
    ![](../Image/ADDS_ProtectAcct_OpenADAC.gif)  
  
    > [!NOTE]  
    > The selected **Authentication** node is visible for domains which are at [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] DFL. If the node does not appear, then try again by using a domain administrator account from a domain that is at [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] DFL.  
  
2.  Click **Authentication Policies**, and then click **New** to create a new policy.  
  
    ![](../Image/ADDS_ProtectAcct_NewAuthNPolicy.gif)  
...
3.  To create an audit\-only policy, click **Only audit policy restrictions**.  
    ...
    ![](../Image/ADDS_ProtectAcct_NewAuthNPolicyAuditOnly.gif)  
...
4.  To configure a TGT lifetime for user accounts, select the **Specify a Ticket\-Granting Ticket lifetime for user accounts** check box and enter the time in minutes.  
    ...
    ![](../Image/ADDS_ProtectAcct_TGTLifetime.gif)  
...
5.  To restrict the user account to select devices, click **Edit** to define the conditions that are required for the device.  
    ...
    ![](../Image/ADDS_ProtectAcct_EditAuthNPolicy.gif)  
  
6.  In the **Edit Access Control Conditions** window, click **Add a condition**.  
  
    ![](../Image/ADDS_ProtectAcct_AddCondition.png)  
...
1.  To configure computer accounts or groups, in the drop\-down list, select the drop\-down list box **Member of each** and change to **Member of any**.  
    ...
    ![](../Image/ADDS_ProtectAcct_AddCompMember.png)  
...
2.  Type the name of OU, the user account should be restricted to sign on.  
    ...
    ![](../Image/ADDS_ProtectAcct_CompClaimOUName.gif)  
  
3.  When done, then click OK and the box will show the conditions defined.  
  
    ![](../Image/ADDS_ProtectAcct_CompClaimComplete.gif)  
...
![](../Image/ADDS_ProtectAcct_KerbClientDACSupport.gif)  
...
![](../Image/ADDS_ProtectAcct_AccountsAssigned.gif)  
...
You can create an authentication policy silo by using Active Directory Administrative Center or [!INCLUDE[wps_2](../Token/wps_2_md.md)]. By default, an authentication policy silo only audits silo policies, which is equivalent to specifying the **WhatIf** parameter in [!INCLUDE[wps_2](../Token/wps_2_md.md)] cmdlets. In this case, policy silo restrictions do not apply, but audits are generated to indicate whether failures occur if the restrictions are applied.  
...
1.  Open **Active Directory Administrative Center**, click **Authentication**, right\-click **Authentication Policy Silos**, click **New**, and then click **Authentication Policy Silo**.  
    ...
    ![](../Image/ADDS_ProtectAcct_CreateNewAuthNPolicySilo.gif)  
...
2.  In **Display name**, type a name for the silo. In **Permitted Accounts**, click **Add**, type the names of the accounts, and then click **OK**. You can specify users, computers, or service accounts. Then specify whether to use a single policy for all principals or a separate policy for each type of principal, and the name of the policy or policies.  
    ...
    ![](../Image/ADDS_ProtectAcct_NewAuthNPolicySiloDisplayName.gif)  
