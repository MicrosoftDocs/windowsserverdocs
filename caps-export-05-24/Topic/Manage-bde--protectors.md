...
|\-add|Adds key protection methods as specified by using additional [–add parameters](../Topic/Manage-bde--protectors.md#BKMK_addprotectors).|  
|\-delete|Deletes key protection methods used by BitLocker. All key protectors will be removed from a drive unless the optional [\-delete parameters](../Topic/Manage-bde--protectors.md#BKMK_deleteprotectors) are used to specify which protectors to delete. When the last protector on a drive is deleted, BitLocker protection of the drive is disabled to ensure that access to data is not lost inadvertently.|  
|\-disable|Disables protection, which will allow anyone to access encrypted data by making the encryption key available unsecured on drive. No key protectors are removed. Protection will be resumed the next time Windows is booted unless the optional [\-disable parameters](../Topic/Manage-bde--protectors.md#BKMK_disableprot) are used to specify the reboot count.|  
