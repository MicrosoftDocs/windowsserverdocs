...
|Allow monitoring of this computer’s desktops|Allows you to control whether desktops can be monitored on the MultiPoint Services system. If this setting is off \(not selected\), desktops of stations \(both local and remote\) that are connected to the computer that is running MultiPoint Services will not display in the Desktops tab of [!INCLUDE[wms_2011_manager](../Token/wms_2011_manager_md.md)] \(including on a different computer if the computer is being remotely managed\).|  
...
|Allow IM between MultiPoint Dashboard and a user session on this computer|Enables chat between MultiPoint Manager and user session on this computer.|  
