...
-   **Validation after the cluster is configured and in use**  
    ...
    -   **When making changes that affect firmware or drivers**  
        ...
        If you want to upgrade or make other changes to the cluster that would require changing the firmware or drivers, you must run the Validate a Configuration Wizard to confirm that the new combination of hardware, firmware, drivers, and software supports your failover cluster functionality. If the change affects the firmware or drivers for storage, we recommend that you keep a small LUN available \(unused by clustered roles\) so that you can run the storage validation tests without taking your clustered roles offline. For more information about running storage tests on a small unused LUN, see [Considerations for validating an existing cluster](#BKMK_CONS_EXIST).  
...
-   [Failover Clustering Overview \[Role\/Tech Overview\]](assetId:///6eeffe4f-3558-495b-bcea-c640fe4d6c49)  
