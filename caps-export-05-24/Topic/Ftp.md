...
|\-g|Disables file name globbing.  **Glob** permits the use of the asterisk \(\*\) and question mark \(?\) as wildcard characters in local file and path names. For more information, see [Additional references](../Topic/Ftp.md#BKMK_AdditionalRef).|  
|\-s:<FileName>|Specifies a text file that contains **ftp** commands. These commands run automatically after **ftp** starts. This parameter allows no spaces. Use this parameter instead of redirection \(**<**\). **Note:** In [!INCLUDE[win8_client_2](../Token/win8_client_2_md.md)] and [!INCLUDE[win8_server_2](../Token/win8_server_2_md.md)] or later operating systems, the text file must be written in UTF\-8.|  
...
-   **Ftp** supports the use of IPv6 when the IPv6 protocol is installed. For more information, see [Additional references](../Topic/Ftp.md#BKMK_AdditionalRef).  
