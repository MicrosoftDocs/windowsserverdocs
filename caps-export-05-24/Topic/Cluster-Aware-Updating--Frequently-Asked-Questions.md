...
ms.tgt_pltfrm: na
ms.topic: article
ms.assetid: 1502b7f7-dd24-47ae-b17a-67f5ec5cce96
author: kumudd
---
...
No. CAU coordinates the cluster updating operation only from computers running the [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)], [!INCLUDE[win8_server_2](../Token/win8_server_2_md.md)], [!INCLUDE[winblue_client_2](../Token/winblue_client_2_md.md)] or [!INCLUDE[win8_client_2](../Token/win8_client_2_md.md)] operating system.  
  
### Is CAU functionality limited to only specific clustered applications?  
No. CAU is agnostic to the type of the clustered application. CAU is an external cluster\-updating solution that is layered on top of clustering APIs and [!INCLUDE[wps_2](../Token/wps_2_md.md)] cmdlets. As such, CAU can coordinate updating for any clustered application that is configured in a [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] or [!INCLUDE[win8_server_2](../Token/win8_server_2_md.md)] failover cluster.  
  
> [!NOTE]  
> Currently, the following [!INCLUDE[win8_server_2](../Token/win8_server_2_md.md)] clustered workloads are tested and certified for CAU: SMB, Hyper\-V, DFS Replication, DFS Namespaces, iSCSI, and NFS.  
...
**Remote\-updating** Enables you to start an Updating Run at any time from a computer running [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)], [!INCLUDE[win8_server_2](../Token/win8_server_2_md.md)], [!INCLUDE[winblue_client_2](../Token/winblue_client_2_md.md)] or [!INCLUDE[win8_client_2](../Token/win8_client_2_md.md)]. You can start an Updating run through the UI or by using the **Invoke\-CauRun**[!INCLUDE[wps_2](../Token/wps_2_md.md)] cmdlet. Remote\-updating is the default updating mode for CAU. You can use Task Scheduler to run the **Invoke\-CauRun** cmdlet on a desired schedule from a remote computer that is not one of the cluster nodes.  
...
> [!NOTE]  
> A Configuration Manager plug\-in for CAU is not available in [!INCLUDE[win8_server_2](../Token/win8_server_2_md.md)].  
...
Yes. CAU comes with [!INCLUDE[wps_2](../Token/wps_2_md.md)] cmdlets that offer a rich set of scripting options. These are the same cmdlets that the CAU UI calls to perform CAU actions.  
...
**Scripts** An Updating Run can specify a pre\-update [!INCLUDE[wps_2](../Token/wps_2_md.md)] script and a post\-update [!INCLUDE[wps_2](../Token/wps_2_md.md)] script. The pre\-update script runs on each cluster node before the node is paused. The post\-update script runs on each cluster node after the node updates are installed.  
  
> [!NOTE]  
> .NET Framework 4.5 and [!INCLUDE[wps_2](../Token/wps_2_md.md)] must be installed on each cluster node on which you want to run the pre\-update and post\-update scripts. You must also enable [!INCLUDE[wps_2](../Token/wps_2_md.md)] remoting on the cluster nodes. For detailed system requirements, see [Requirements and Best Practices for Cluster-Aware Updating](../Topic/Requirements-and-Best-Practices-for-Cluster-Aware-Updating.md).  
  
**Advanced Updating Run options** The administrator can additionally specify from a large set of advanced Updating Run options such as the maximum number of times that the update process will be retried on each node. These options can be specified using either the CAU UI or the CAU [!INCLUDE[wps_2](../Token/wps_2_md.md)] cmdlets. These custom settings can be saved in an Updating Run Profile and reused for later Updating Runs.  
...
-   Preview results by using the [!INCLUDE[wps_2](../Token/wps_2_md.md)] cmdlet **Invoke\-CauScan | ConvertTo\-Xml**. Output: XML  
  
-   Report results by using the [!INCLUDE[wps_2](../Token/wps_2_md.md)] cmdlet **Invoke\-CauRun | ConvertTo\-Xml**. Output: XML  
  
-   Report results by using the [!INCLUDE[wps_2](../Token/wps_2_md.md)] cmdlet **Get\-CauReport | Export\-CauReport**. Output: HTML, CSV  
...
CAU is included with all [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] and [!INCLUDE[win8_server_2](../Token/win8_server_2_md.md)] editions. A CAU installation is seamlessly integrated into the Failover Clustering feature. CAU is installed as follows:  
...
-   When Failover Clustering tools are installed on a server or client computer, the CAU [!INCLUDE[wps_2](../Token/wps_2_md.md)] cmdlets are automatically installed.  
  
-   When the Failover Clustering UI is installed on a server or client computer, the CAU [!INCLUDE[wps_2](../Token/wps_2_md.md)] cmdlets and the CAU UI are automatically installed.  
...
-   VMM requires additional licensing, whereas CAU is licensed for all [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] and [!INCLUDE[win8_server_2](../Token/win8_server_2_md.md)] editions. The CAU features, tools, and UI are installed with Failover Clustering components.  
...
-   CAU is supported only on clusters that are running [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] and [!INCLUDE[win8_server_2](../Token/win8_server_2_md.md)]. VMM also supports Hyper\-V clusters on computers running [!INCLUDE[nextref_longhorn](../Token/nextref_longhorn_md.md)] and [!INCLUDE[nextref_server_7](../Token/nextref_server_7_md.md)].  
