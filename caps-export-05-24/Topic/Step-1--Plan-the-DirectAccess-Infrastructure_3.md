---
title: Step 1: Plan the DirectAccess Infrastructure_3
ms.custom: na
...
ms.tgt_pltfrm: na
ms.topic: article
ms.assetid: 27cdb4a4-189b-413c-9e52-c7c29d5cccb5
---
# Step 1: Plan the DirectAccess Infrastructure_3
...
Certificate requirements for IPsec include a computer certificate used by DirectAccess client computers when establishing the IPsec connection between the client and the Remote Access server, and a computer certificate used by Remote Access servers to establish IPsec connections with DirectAccess clients. For DirectAccess in [!INCLUDE[win8_server_1](../Token/win8_server_1_md.md)] the use of these IPsec certificates is not mandatory. The Enable DirectAccess Wizard configures the Remote Access server to act as a Kerberos proxy to perform IPsec authentication without requiring certificates.  
...
-   For DirectAccess clients, you must use either a DNS server running Windows Server 2003, [!INCLUDE[nextref_longhorn](../Token/nextref_longhorn_md.md)], [!INCLUDE[nextref_server_7](../Token/nextref_server_7_md.md)], [!INCLUDE[win8_server_2](../Token/win8_server_2_md.md)], or any DNS server that supports IPv6.  
  
### <a name="bkmk_1_6_AD"></a>Plan Active Directory  
Remote Access uses Active Directory and Active Directory [!INCLUDE[gpos](../Token/gpos_md.md)] as follows:  
...
-   **Extended IPsec policies**—Remote Access can use IPsec authentication and encryption between clients and the Remote Access server. You can extend IPsec authentication and encryption through to specified internal application servers. To do this, see [Deploy a Single DirectAccess Server with Advanced Settings](../Topic/Deploy-a-Single-DirectAccess-Server-with-Advanced-Settings.md).  
  
**Active Directory Requirements**  
  
When planning Active Directory for a Remote Access deployment, the following is required:  
  
-   At least one domain controller installed on the [!INCLUDE[win8_server_2](../Token/win8_server_2_md.md)], [!INCLUDE[nextref_server_7](../Token/nextref_server_7_md.md)][!INCLUDE[nextref_longhorn](../Token/nextref_longhorn_md.md)], or Windows Server 2003 operating systems.  
...
DirectAccess settings configured when you configure Remote Access are collected into [!INCLUDE[gpos](../Token/gpos_md.md)] \(GPO\). Three different GPOs are populated with DirectAccess settings, and distributed as follows:  
