...
>   
> -   [RAS Gateway High Availability](../Topic/RAS-Gateway-High-Availability.md)  
> -   [GRE Tunneling in Windows Server Technical Preview](../Topic/GRE-Tunneling-in-Windows-Server-Technical-Preview.md)  
...
Single tenant mode allows organizations of any size to deploy the gateway as an exterior, or Internet\-facing edge virtual private network \(VPN\) and DirectAccess server. In single tenant mode, you can deploy RAS Gateway on a physical server or virtual machine \(VM\) running [!INCLUDE[winthreshold_server_1](../Token/winthreshold_server_1_md.md)].  
  
Multitenant mode allows Cloud Service Providers \(CSPs\) and Enterprises to use RAS Gateway to enable datacenter and cloud network traffic routing between virtual and physical networks, including the Internet. For multitenant mode, it is recommended that you deploy RAS Gateway on VMs that are running [!INCLUDE[winthreshold_server_1](../Token/winthreshold_server_1_md.md)].  
...
Virtual networks are created by using Hyper\-V Network Virtualization, which is a technology that was introduced in [!INCLUDE[win8_server_1](../Token/win8_server_1_md.md)], and is improved in [!INCLUDE[winthreshold_server_2](../Token/winthreshold_server_2_md.md)]. RAS Gateway is integrated with Hyper\-V Network Virtualization, and is able to route network traffic effectively in circumstances where there are many different customers – or tenants – who have isolated virtual networks in the same datacenter.  
...
In [!INCLUDE[winthreshold_server_2](../Token/winthreshold_server_2_md.md)], RAS Gateway routes network traffic between the physical network and VM network resources, regardless of where the resources are located. You can use RAS Gateway to route network traffic between physical and virtual networks at the same physical location or at many different physical locations.  
...
For more information, see [RAS Gateway High Availability](../Topic/RAS-Gateway-High-Availability.md).  
...
When you deploy RAS Gateway, the host servers running Hyper\-V and the VMs that you configure as gateways must be running [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] or [!INCLUDE[winthreshold_server_2](../Token/winthreshold_server_2_md.md)].  
...
-   In [!INCLUDE[winthreshold_server_2](../Token/winthreshold_server_2_md.md)], to deploy an RAS Gateway router, you must use Windows PowerShell commands. For more information, see  [Remote Access Cmdlets](https://technet.microsoft.com/en-us/library/hh918399.aspx) for [!INCLUDE[winthreshold_server_2](../Token/winthreshold_server_2_md.md)] and [!INCLUDE[winthreshold_client_2](../Token/winthreshold_client_2_md.md)].  
