...
After you install [!INCLUDE[winthreshold_server_2](../Token/winthreshold_server_2_md.md)] on the computers that you are planning to use as your certification authority and Web server, you must rename the computer and assign and configure a static IP address for the local computer.  
  
For more information, see the [!INCLUDE[winthreshold_server_2](../Token/winthreshold_server_2_md.md)] [Core Network Guide](https://technet.microsoft.com/library/mt604042.aspx).  
...
-   **RenewalKeyLength**. The default renewal key length for AD CS in [!INCLUDE[win8_server_2](../Token/win8_server_2_md.md)] is 2048. The key length that you select should be as long as possible while still providing compatibility with the applications that you intend to use.  
