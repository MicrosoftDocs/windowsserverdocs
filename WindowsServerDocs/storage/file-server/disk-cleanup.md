...
To start the Disk Cleanup tool, either run the Cleanmgr.exe file, or select **Start**, select **Windows Administrative Tools**, and then select **Disk Cleanup**.
...
> [!NOTE]
> If you're just looking to free up disk space, consider using Azure File Sync with cloud tiering enabled. This allows you to cache your most frequently accessed files locally and tier your least frequently accessed files to the cloud, saving local storage space while maintaining performance. For more information, see [Planning for an Azure File Sync deployment](/azure/storage/files/storage-sync-files-planning).
...
1. If Server Manager is already open, go to the next step. If Server Manager isn't already open, open it by doing one of the following options.
...
1. Verify that the **Disk Cleanup** button appears in the Properties dialog box.
   ...
   :::image type="content" source="media/disk-prop-sw-cleanup.png" alt-text="Screen shot of the Disk Properties dialog with the Disk Cleanup button highlighted.":::
...
Now you can launch the Disk Cleanup tool by running Cleanmgr.exe from a Command Prompt window, or by clicking **Start** and typing **Cleanmgr** in the search field.

To have the Disk Cleanup button appear on a disk's Properties dialog, you also need to install the Desktop Experience feature.
