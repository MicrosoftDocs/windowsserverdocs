...
This is expected behavior if you attempted to migrate from or to a domain controller with SMS and used the "migrate users and groups" option to rename or reuse accounts. instead of selecting "Don't transfer users and groups". DC migration is [not supported with SMS](https://docs.microsoft.com/windows-server/storage/storage-migration-service/faq). Because a DC doesn't have true local users and groups, SMS treats these security principals as it would when migrating between two member servers and attempts to adjust ACLs as instructed, leading to the errors and mangled or copied accounts. 
...
`Get-ADObject -Filter 'Description -like "*storage migration service renamed*"' -SearchBase 'DC=<domain>,DC=<TLD>' | ft name,distinguishedname`
 ...
 2. For any users returned with their original name, edit their "User Logon Name (pre-Windows 2000)" to remove the random character suffix added by SMS, so that this loser can logon.
 3. For any groups returned with their original name, edit their "Group Name (pre-Windows 2000)" to remove the random character suffix added by SMS.
 4. For any disabled users or groups with names that now contain a suffix added by SMS, you can delete these accounts. You can confirm that user accounts were added later because they will only contain the Domain Users group and will have a created date/time matching the SMS transfer start time.
