...
In this step you transfer data after specifying where to put it on the destination servers.
 ...
 1. Select **Start transfer** to start transferring data.<br>If there are existing files in the destination location the first time you transfer, we'll move the existing files to a backup folder. On subsequent transfers, by default we'll refresh the destination without backing it up first. <br>Also, Storage Migration Service is smart enough to deal with overlapping shares so that we don't copy the same folders twice in the same job.
