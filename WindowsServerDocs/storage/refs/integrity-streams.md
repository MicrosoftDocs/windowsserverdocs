...
Integrity streams is an optional feature in ReFS that validates and maintains data integrity. ReFS uses checksums for metadata and optionally for file data--file data checksums are disabled by default on current versions of ReFS. Using checksums is an important feature for ReFS, for it enables ReFS to clearly determine if a block is valid or corrupt. Additionally, ReFS and Spaces can jointly correct corrupt metadata and data automatically. 
