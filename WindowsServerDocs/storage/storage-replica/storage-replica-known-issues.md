...
This issue occurs when you create a VSS snapshot of the log volume. The underlying cause is a legacy design aspect of VSS, not Storage Replica. The resulting behavior when you snapshot the SR log volume is a VSS IO queing mechanism deadlocks the server.

To prevent this behavior, do not snapshot SR log volumes. There is no need to snapshot SR log volumes, as these logs cannot be restored. Furthermore, the log volume should never contain any other workloads, so no snapshot is needed in general.
