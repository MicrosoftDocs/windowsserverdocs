---
description: Stretch Cluster Replication and Shared Storage
title: Stretch cluster replication using shared storage for Storage Replica in Windows Server
manager: eldenc
ms.author: alalve
ms.topic: how-to
author: nedpyle
...
- Four servers, named **SR-SRV01**, **SR-SRV02**, **SR-SRV03**, and **SR-SRV04** in a single cluster called **SR-SRVCLUS**.

- A pair of logical "sites" that represents two different datacenters, with one called **Redmond** and one called **Bellevue.**

> [!NOTE]
> You can use only as few as two nodes, where one node is in each site. However, you will not be able to perform intra-site failover with only two servers. You can use as many as 64 nodes.

The following diagram shows storage replication in a stretch cluster:

![Diagram that shows two nodes in Redmond replicating with two nodes of the same cluster in the Bellevue site.](./media/Stretch-Cluster-Replication-Using-Shared-Storage/Storage_SR_StretchClusterExample.png)

## Prerequisites

- Active Directory Domain Services forest (does not need to run Windows Server 2016).
- From 2 and 64 servers running Windows Server 2019 or Windows Server 2016 Datacenter Edition. If you're running Windows Server 2019, you can instead use Standard Edition if you're OK replicating only a single volume up to 2 TB in size.
...
> [!IMPORTANT]
> Although it is possible to attach a storage device to a single server and to use this for replication, Windows Failover Clustering still relies on SCSI Persistent Reservations. Therefore, the storage must still be a Shared Storage type such as a SAN technology. Local disks or disks presented by a hypervisor might not be compatible. In Azure, the disks must be a Premium SSD size that supports sharing, even if only one VM is attached to it.
