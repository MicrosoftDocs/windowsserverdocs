---
title: Cluster-to-Cluster Storage Replication in the Same Azure Region
description: Learn how to set up cluster-to-cluster storage replication within the same Azure region.
author: arduppal
ms.author: roharwoo
ms.date: 02/18/2025
ms.topic: article
...
1. Create two [availability sets](https://ms.portal.azure.com/#create/Microsoft.AvailabilitySet-ARM) in the resource group (**SR-AZ2AZ**) created above, one for each cluster.
    a. Availability set (**az2azAS1**)
    b. Availability set (**az2azAS2**)
1. Create a [virtual network](https://ms.portal.azure.com/#create/Microsoft.VirtualNetwork-ARM) (**az2az-Vnet**) in the previously created resource group (**SR-AZ2AZ**), having at-least one subnet.
1. Create a [network security group](https://ms.portal.azure.com/#create/Microsoft.NetworkSecurityGroup-ARM) (**az2az-NSG**), and add one Inbound security rule for RDP:3389. You can choose to remove this rule once you finish your setup.
1. Create Windows Server [virtual machines](https://ms.portal.azure.com/#create/Microsoft.WindowsServer2016Datacenter-ARM) in the previously created Resource group (**SR-AZ2AZ**). Use the previously created virtual network (**az2az-Vnet**) and network security group (**az2az-NSG**).
...
1. Connect all the nodes to the domain and provide Administrator privileges to the previously created user.

1. Change the DNS Server of the virtual network to domain controller private IP address.
1. In our example, the domain controller **az2azDC** has private IP address (10.3.0.8). In the Virtual Network (**az2az-Vnet**) change DNS Server 10.3.0.8. Connect all the nodes to "Contoso.com" and provide administrator privileges to "contosoadmin".
...
1. Run the following command from any one node **az2az3**/**az2az4**.
...
1. Create cloud witnesses for both clusters. Create two [storage accounts](https://ms.portal.azure.com/#create/Microsoft.StorageAccount-ARM) (**az2azcw**, **az2azcw2**) in Azure one for each cluster in the same resource group (**SR-AZ2AZ**).
...
1. Run [cluster validation tests](../../failover-clustering/create-failover-cluster.md#run-cluster-validation-tests) before moving on to the next step.

1. Start Windows PowerShell and use the [Test-SRTopology](/powershell/module/storagereplica/test-srtopology) cmdlet to determine if you meet all the Storage Replica requirements. You can use the cmdlet in a requirements-only mode for a quick test as well as a long-running performance evaluation mode.

1. Configure cluster-to-cluster Storage Replica.
    ...
    If you use Windows Server 2016, run this command also:

    ```powershell
      Grant-SRAccess -ComputerName az2az3 -Cluster SRAZC1
    ```

1. Create SRPartnership for the clusters:</ol>
