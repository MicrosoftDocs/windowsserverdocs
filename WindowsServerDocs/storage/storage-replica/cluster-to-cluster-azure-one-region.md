...
> [!VIDEO https://www.youtube.com/embed/O8X-KYk5uqs]
...
## Storage spaces direct
   11. Enable storage spaces direct
```PowerShell
    Enable-clusterS2D
```
...
## Load balancers
   12. Create an internal Standard SKU [Load Balancer](https://ms.portal.azure.com/#create/Microsoft.LoadBalancer-ARM) for each cluster (**azlbr1**,**azlbr2**).
...
## Open port 59999 (Health Probe)
   13. On each cluster node, open port 59999 (Health Probe)
...
## Instructing the cluster to listen for Health Probe messages
   14. Instruct the cluster to listen for Health Probe messages on Port 59999 and respond from the node that currently owns this resource.
...
## Creating cloud witness for both clusters
...
## Configuring cluster-to-cluster Storage Replica
   17. Grant SR-Access from one cluster to another cluster in both direction
In our example:
```PowerShell
...
## Create Partnership
   18. Create partnership for the clusters:
- For cluster **SRAZC1**.
