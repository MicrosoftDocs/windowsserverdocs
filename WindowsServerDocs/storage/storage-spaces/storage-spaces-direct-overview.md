...
Deploying Storage Spaces Direct on VM guest clusters delivers virtual shared storage across a set of VMs on top of a private or public cloud. In production environments, you can deploy only Datacenter or Azure Edition editions of Windows Server on VM guest clusters. You can run Windows Server as a guest on most cloud and hyperconverged platforms if it’s licensed correctly. However, currently Azure Edition of Windows Server can run as a guest only in Azure. For information about how to deploy Storage Spaces Direct on VM guest clusters running Windows Server, see [Using Storage Spaces Direct in guest virtual machine clusters](storage-spaces-direct-in-vm.md). To find out what's new in Windows Server 2022, see [What's new in Windows Server 2022](/windows-server/get-started/whats-new-in-windows-server-2022).
