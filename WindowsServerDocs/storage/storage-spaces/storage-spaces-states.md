...
|---------            |---------       |--------   |
|Read-only|Incomplete|This can occur if the storage pool pool loses its quorum, which means that the majority of disks in the pool have failed or are offline for some reason. When a pool loses its quorum, Storage Spaces automatically sets the pool configuration to read-only until enough disks become available again.<br><br>**Action:** Reconnect any missing disks and then set the pool back to read-write by opening a PowerShell session with administrative permissions and then typing:<br> `Get-StoragePool <PoolName>` &#124;  `Set-StoragePool -IsReadOnly $false`|
