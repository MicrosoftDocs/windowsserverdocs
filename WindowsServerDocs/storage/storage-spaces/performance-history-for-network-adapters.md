...
| Series                           | Unit             |
|----------------------------------|------------------|
| `netadapter.bytes.inbound`       | bytes per second |
| `netadapter.bytes.outbound`      | bytes per second |
| `netadapter.bytes.total`         | bytes per second |
| `netadapter.bytes.rdma.inbound`  | bytes per second |
| `netadapter.bytes.rdma.outbound` | bytes per second |
| `netadapter.bytes.rdma.total`    | bytes per second |
...
|--------------------------------------|-----------------------------------------------------------------------|
| `netadapter.bytes.inbound`       | Rate of data received by the network adapter.                         |
| `netadapter.bytes.outbound`      | Rate of data sent by the network adapter.                             |
| `netadapter.bytes.total`         | Total rate of data received or sent by the network adapter.           |
| `netadapter.bytes.rdma.inbound`  | Rate of data received over RDMA by the network adapter.               |
| `netadapter.bytes.rdma.outbound` | Rate of data sent over RDMA by the network adapter.                   |
| `netadapter.bytes.rdma.total`    | Total rate of data received or sent over RDMA by the network adapter. |
...
|----------------------------------|----------------------|
| `netadapter.bytes.inbound`       | `Bytes Received/sec` |
| `netadapter.bytes.outbound`      | `Bytes Sent/sec`     |
| `netadapter.bytes.total`         | `Bytes Total/sec`    |
...
| Series                               | Source counter       |
|--------------------------------------|----------------------|
| `netadapter.bytes.rdma.inbound`  | `Inbound bytes/sec`  |
| `netadapter.bytes.rdma.outbound` | `Outbound bytes/sec` |
| `netadapter.bytes.rdma.total`    | *sum of the above*   |

   > [!NOTE]
   > Counters are measured over the entire interval, not sampled. For example, if the network adapter is idle for 9 seconds but transfers 30 bytes in the 10th second, its `netadapter.bytes.total` will be recorded as 3 bytes per second on average during this 10-second interval. This ensures its performance history captures all activity and is robust to noise.
