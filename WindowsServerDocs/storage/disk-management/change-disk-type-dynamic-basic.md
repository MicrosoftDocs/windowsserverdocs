...
1. Prepare your disk for conversion by cleaning (deleting) any partitions or volumes.
   ...
   1. Type `select disk <disk number>`, replace `<disk number>` with the number you noted in the previous step, and then select **Enter**.

   1. To verify the details of the currently selected disk, type `detail disk` and select **Enter**. The `detail disk` command displays information about the currently selected disk and doesn't accept a disk number as an argument.

   1. Type `clean` and select **Enter** to delete all partitions and volumes on the disk.
