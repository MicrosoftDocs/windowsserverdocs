...
1. If the disk appears in Disk Management as *Online*, and has a primary partition (shown as blue) that's listed as *Healthy*, that's a good sign! Try these:
    - If the partition has a file system, but no drive letter (for example, D:), see [Change a drive letter](change-a-drive-letter.md) to add a drive letter manually.
...
2. Unplug the disk, plug it back in, and then select **Action** > **Rescan Disks**.
3. Shut down your PC, turn off your external hard disk (if it's an external disk with a power cord), and then turn your PC and the disk back on. <br>To turn off your PC in Windows 10, select the Start button, select the Power button, and then select **Shut down**.
...
5. Try a different cable.<br>It might sound crazy, but cables fail a lot, so try using a different cable to plug the disk in.
6. Check Device Manager for issues.<br>Press and hold (or right-click) the Start button, then select Device Manager from the context menu. Look for any devices with an exclamation point next to it or other issues, double-click the device and then read its status. Here's a list of [Error codes in Device Manager](https://support.microsoft.com/help/310123/error-codes-in-device-manager-in-windows), but one approach that sometimes works is to right-click the problematic device, select **Uninstall device**, and then **Action** > **Scan for hardware changes**.<br>![Device Manager showing an unknown USB device](media\device-manager.PNG)
...
## <a id="partitions-not-appearing"></a>Disk is missing from File Explorer and listed as Foreign in Disk Management

**Cause:** This can happen if the disk is a dynamic disk. Dynamic disks use an older technology in Windows that enabled such things as combining multiple drives into a single volume (nowadays we recommend using [Storage Spaces](https://support.microsoft.com/help/12438/windows-10-storage-spaces) instead of dynamic disks). Dynamic disks appear as **Foreign** when they are added to the system. To use the volumes, you must import the **Foreign** disks and then bring the volumes **Online**.

**Solution:** Right-click any partitions listed as *Foreign*, select **Import Foreign Disks** and then follow the on-screen instructions. <br>You can also see [Change a drive letter](change-a-drive-letter.md) to add a drive letter manually, or try the troubleshooting steps described in [A disk's status is Not Initialized or the disk is missing entirely](disk-management/troubleshooting-disk-management.md#disk-not-initialized).
...
**Solution:** Rescan the disks or restart the computer to see if the disk status changes. Also try the troubleshooting steps described in [A disk's status is Not Initialized or the disk is missing entirely](disk-management/troubleshooting-disk-management.md#disk-not-initialized).
