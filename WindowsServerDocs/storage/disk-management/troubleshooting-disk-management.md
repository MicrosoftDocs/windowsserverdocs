...
Here's an easy way to get to Disk Management, in case you're not there already:

1. In the search box on the taskbar, enter **Computer Management**, select and hold (or right-click) **Computer Management**, and then choose **Run as administrator** > **Yes**.
...
![Screenshot that shows Disk Management highlighting an unknown disk that must be initialized.](media/uninitialized-disk.PNG)
...
1. Look at the disk in Disk Management. If it appears *Offline* as shown here, try selecting it and choosing **Online**.
    ...
    ![Screenshot that shows the disk as offline.](media/offline-disk.png)
...
2. If the disk appears in Disk Management as *Online*, and has a primary partition that's listed as *Healthy*, as shown here, that's a good sign.
    ...
    ![Screenshot that shows a disk that's online and with a healthy volume.](media/healthy-volume.png)
...
7. Check Device Manager for issues.
    ...
    ![Screenshot of Device Manager showing an unknown USB device.](media/device-manager.PNG)
...
2. Import the disks. For more information on how to move and import disks, see [Move disks to another computer](move-disks-to-another-computer.md).
...
**Cause:**  Indicates that the dynamic volume is accessible, but I/O errors are detected on the underlying dynamic disk. If an I/O error is detected on any part of a dynamic disk, all volumes on the disk display the **Healthy (At Risk)** status and a warning icon appears on the volume.
...
**Solution:** If you're using non-Microsoft disk management software that has disabled LDM, contact the vendor on that software to get troubleshooting support for the disk configuration.
...
2. To manage disks on remote computers that support VDS, configure the Windows Defender Firewall on both the local computer (on which you're running Disk Management) and the remote computer.
