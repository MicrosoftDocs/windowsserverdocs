...
description: Describes the Key Derivation Function (KDF) called "KDFv2" for AD FS
...
KDFv2 modes may be changed by an Administrator on an ADFS server via the following PowerShell commands:
...
Depending on the OS version that ADFS servers are running when patched to support KDFv2, KDFv2 may be enabled automatically. Also, OS-version-dependent events may be logged to indicate the state of KDFv2 in the farm. Possible deployment scenarios and expected behaviors follow:
 
Scenario 1: Windows Server 2019 or above is installed on all ADFS servers in a farm. One or more farm nodes are unpatched.
...
Note: Event 182 will not be logged if any servers in a farm are running Windows Server 2016.
