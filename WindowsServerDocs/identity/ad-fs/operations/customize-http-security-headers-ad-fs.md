...
By default, header is set to deny; however, admins can modify the value through the **Set-AdfsResponseHeaders** cmdlet.
...
This HTTP security response header is used to stop web pages from loading when browsers detect cross-site scripting (XSS) attacks. This approach is referred as XSS filtering. The header can be set to one of the following values:

- **0** disables XSS filtering. Not recommended.
- **1** enables XSS filtering. If XSS attack is detected, browser sanitizes the page.
- **1; mode=block** enables XSS filtering. If XSS attack is detected, browser prevents rendering of the page. This setting is the default and recommended setting.
...
Web browser security prevents a web page from making cross-origin requests initiated from within scripts. However, sometimes you might want to access resources in other origins (domains). Cross Origin Resource Sharing (CORS) is a W3C standard that allows a server to relax the same-origin policy. By using CORS, a server can explicitly allow some cross-origin requests while rejecting others.
...
2. After user authentication, the front-end JavaScript included in SPA makes a request to access the web API. The request is redirected to AD FS with following headers:
    ...
    - Access-Control-Request-Method – identifies the HTTP method (for example, DELETE) to be used when actual request is made.
...
3. AD FS verifies that the web API origin included in the header is listed in the trusted origins configured in AD FS. For more information on how to modify trusted origins, see [CORS Customization](#cors-customization). AD FS then responds with following headers:
...
By default, CORS functionality isn't be enabled; however, admins can enable the functionality through the **Set-AdfsResponseHeaders** cmdlet.
