...
## Scenarios 
1. Administrator has enabled **HTTP Strict-Transport-Security** (forces all connections over HTTPS encryption) to protect the users who might access the web app using HTTP from a public wifi access point that might be hacked. She would like to further strengthen security by enabling HSTS for subdomains.  
2. Administrator has configured the **X-Frame-Options** response header (prevents rendering any web page in an iFrame) to protect the web pages from being clickjacked. However, she needs to customize the header value due to a new business requirement to display data (in iFrame) from an application with a different origin (domain).
3. Administrator has enabled **X-XSS-Protection** (prevents cross scripting attacks) to sanitize and block the page if browser detects cross scripting attacks. However, she needs to customize header to allow the page to load once sanitized.  
4. Administrator needs to enable **Cross Origin Resource Sharing (CORS)** and set the origin (domain) on AD FS to allow a Single Page Application to access a web API with another domain.  
...
## Scenario 1:  HTTP Strict-Transport-Security (HSTS) 
HSTS is a web security policy mechanism which helps mitigate protocol downgrade attacks and cookie hijacking for services that have both HTTP and HTTPS endpoints. It allows web servers to declare that web browsers1 (or other complying user agents) should only interact with it using HTTPS and never via the HTTP protocol.  
...
Set- AdfsResponseHeaders 
   - SetHeaderName “Strict-Transport-Security” 
...
This HTTP security response header is used to communicate to the browser2 whether it can render a page in a &lt;frame&gt;/&lt;iframe&gt;. The header can be set to one of the following values: 
...
```PowerShell
Set- AdfsResponseHeaders 
   - RemoveHeaders "X-Frame-Options" 
```

## Scenario 3:  X-XSS-Protection 
...
- **1** – Enables XSS filtering (default setting). If XSS attack is detected, browser will sanitize the page.   
- **1; mode=block** – Enables XSS filtering (default setting). If XSS attack is detected, browser will prevent rendering of the page. This is the default and recommended setting.  
...
```PowerShell
Set- AdfsResponseHeaders 
   - SetHeaderName “X-XSS-Protection” 
...
## Scenario 4: Cross Origin Resource Sharing (CORS) headers 
...
Note – CORS request resembles a standard HTTP request, however, the presence of an origin header signals the incoming request is CORS related. 
...
## Scenario 5: Content Security Policy (CSP) 
This HTTP security response header is used to prevent cross-site scripting, clickjacking and other data injection attacks by preventing browsers5 from inadvertently executing malicious content. Browsers that don’t support CSP simply ignores the CSP response headers.  
...
```PowerShell
Set- AdfsResponseHeaders 
   - SetHeaderName “TestHeader” 
   - SetHeaderValue “TestHeaderValue” 
 ```
