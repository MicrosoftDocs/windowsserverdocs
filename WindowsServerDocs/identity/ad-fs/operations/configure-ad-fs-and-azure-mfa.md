...
1. Configure each new AD FS Microsoft Entra multifactor authentication certificate in the Microsoft Entra tenant.
    ...
    With the Azure AD PowerShell module, for each new certificate (on each AD FS server), update your Microsoft Entra tenant settings as follows. You must first connect to the tenant by using `Connect-MgGraph` to run the following commands.
