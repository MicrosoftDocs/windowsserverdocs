...
- Be aware that AD FS administrators can customize the AD FS styles. Don't hardcode your own styles. Instead, try using AD FS styles whenever possible.
...
- **theme styles** - These styles should not and cannot be used. These styles are meant to define theme across all pages. They're defined by an element ID purposely so that they are not reused.
