...
Reasons why you shouldn't use self\-signed, service communication certificates for a live deployment include the following:

- A self\-signed, SSL certificate must be added to the trusted root store on each of the federation servers in the resource partner organization. While this alone doesn't enable an attacker to compromise a resource federation server, trusting self\-signed certificates does increase the attack surface of a computer and it can lead to security vulnerabilities if the certificate signer isn't trustworthy.

- A self\-signed, service communication certificates creates a bad user experience. Clients will receive Security Alert prompts when they try to access federated resources that display the following message: "The security certificate was issued by a company you've not chosen to trust." This is expected behavior, because the self\-signed certificate isn't trusted.
...
- CAs provide additional certificate\-based features, such as private key archive, renewal, and revocation, that aren't provided by self\-signed certificates.