...
The AD FS sign-on page can be used to test whether or not authentication is working.  This test is done by navigating to the page and signing in.  Also, we can use the sign-in page to verify that all SAML 2.0 relying parties are listed.
...
You can test the seamless sign-in experience by making sure that the URL for your AD FS servers are added the local intranet zone of your internet options.  Use the following procedure:

1. On a Windows 10 client, select start and type internet options and select internet options.

1. Select the security tab, select on local intranet, and select the sites button.
