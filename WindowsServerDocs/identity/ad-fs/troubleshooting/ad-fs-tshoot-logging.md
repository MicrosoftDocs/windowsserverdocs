...
The Tracelog is where detailed messages are logged, and is the most useful log when troubleshooting. Since a lot Tracelog information can be generated in a short amount of time, which can affect system performance, the logs are disabled by default.

### To enable and view the Tracelog

1. Open **Event Viewer** and expand **Applications and Services Log**.
2. Right-click on **Applications and Services Log**, select **View** and select **Show Analytic and Debug Logs** (This pane shows more nodes).
...
|Application Token Failure|1201|A request where  security token issuance failed on the Federation Service. For WS-Federation and SAML-P, this event is logged when the request was processed with the SSO artifact. (such as the SSO cookie).|
...
1. Select Start, point to **Programs**, point to **Administrative Tools**, and then select **Local Security Policy**.
...
3. On the **Local Security Setting** tab, verify that the AD FS service account is listed. If it isn't present, select Add User or Group and add it to the list, and then select OK.
...
To open the AD FS Management snap-in, select Start, point to Programs, point to Administrative Tools, and then select AD FS Management.

6. In the Actions pane, select Edit Federation Service Properties
7. In the Federation Service Properties dialog box, select the Events tab.
...
In addition to trace logging, sometimes you may need to view Windows Communication Foundation (WCF) and Windows Identity Foundation (WIF) messages in order to troubleshoot an issue. You can view these messages by modifying the **Microsoft.IdentityServer.ServiceHost.Exe.Config** file on the AD FS server.
