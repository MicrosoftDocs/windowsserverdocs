...
Active Directory Federation Services (AD FS) requires specific certificates in order to work correctly. Problems can occur if any of these certificates weren't set up or configured properly.
...
- **Federation trust**: Federation trust requires one of the following:
  ...
  - A cross-certification design has been implemented in which each side has exchanged its root CA with its partner.
  - Self-signed certificates have been imported on each side where appropriate.
...
|Event 381: An error occurred during an attempt to build the certificate chain for the configuration certificate.|One of the certificates configured for use on the AD FS server is expired or was revoked.|Ensure that all configured certificates aren't revoked or expired.|
