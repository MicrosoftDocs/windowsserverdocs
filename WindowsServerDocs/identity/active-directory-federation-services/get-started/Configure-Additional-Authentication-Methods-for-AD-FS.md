...
In order to enable multi\-factor authentication \(MFA\), you must select at least one additional authentication method. By default, in Active Directory Federation Services \(AD FS\) in [!INCLUDE[winblue_server_1]()], you can select Certificate Authentication \(in other words, smart card\-based authentication\) as an additional authentication method.
...
You can also configure and enable Microsoft and third\-party authentication methods in [!INCLUDE[nextref_adfs2]()] in [!INCLUDE[winblue_server_1]()]. Once installed and registered with [!INCLUDE[nextref_adfs2]()], you can enforce MFA as part of the global or per\-relying\-party authentication policy.
