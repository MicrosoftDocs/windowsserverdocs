...
A stand\-alone federation server in [!INCLUDE[firstref_adfs2]()] consists of a single server that hosts a Federation Service configured to use the Windows Internal Database \(WID\). This [!INCLUDE[nextref_adfs2]()] topology is for test labs. We do not recommend it for production environments because it has a limit of only one federation server, and it cannot be used to scale up to more servers.  
