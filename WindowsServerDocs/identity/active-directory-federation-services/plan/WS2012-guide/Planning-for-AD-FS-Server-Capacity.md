...
> [!NOTE]  
> The content provided in this topic does not reflect actual testing that was performed on servers running [!INCLUDE[win8_server_2]()]. This topic will be updated once the required testing has been performed.  
  
Capacity planning for [!INCLUDE[firstref_adfs2]()] is the process of forecasting peak usage periods for your Federation Service and planning or scaling\-up your [!INCLUDE[nextref_adfs2]()] server deployment to meet those load requirements.  
  
This section describes deployment guidelines for both the federation server and federation server proxy roles and is based on lab testing that was performed by the [!INCLUDE[nextref_adfs2]()] product team at Microsoft. The purpose of this content is to help you:  
  
-   Closely estimate the hardware needs for your organization’s specific [!INCLUDE[nextref_adfs2]()] deployment, such as the number of [!INCLUDE[nextref_adfs2]()] servers.  
  
-   Accurately project the expected peak usage for sign\-in requests, plan for growth, and ensure that your [!INCLUDE[nextref_adfs2]()] deployment is capable of handling that expected peak usage.  
...
|--------------------|---------------|-------------|  
|Understand the requirements for deploying [!INCLUDE[nextref_adfs2]()] federation servers and federation server proxies|Review important hardware and software requirements necessary for deploying federation server and federation server proxies.|[Appendix A: Reviewing AD FS Requirements](../../../active-directory-federation-services/plan/WS2012-guide/Appendix-A--Reviewing-AD-FS-Requirements.md)|  
|Select the type of [!INCLUDE[nextref_adfs2]()] configuration database that you will deploy in your organization|Before you can begin using capacity planning data in this section, you first have to determine which [!INCLUDE[nextref_adfs2]()] configuration database type you will deploy, either Windows Internal Database \(WID\) or a Structured Query Language \(SQL\) database.|[The Role of the AD FS Configuration Database]();<br /><br />[AD FS Deployment Topology Considerations]()|  
...
|Understand key [!INCLUDE[nextref_adfs2]()]–related capacity planning terms|Review the definitions of common capacity planning terms that are used throughout the [!INCLUDE[nextref_adfs2]()] capacity planning discussion.|See the section titled [AD FS capacity planning terms](../../../active-directory-federation-services/plan/WS2012-guide/Planning-for-AD-FS-Server-Capacity.md#bk_terms) in this topic|  
...
|---------------------|---------------|-------------|  
|Download the [!INCLUDE[nextref_adfs2]()] Capacity Planning Sizing Spreadsheet|The [!INCLUDE[nextref_adfs2]()] Capacity Planning Sizing spreadsheet can help you to determine the number of federation servers required for an AD FS federation server farm deployment. Instructions for how to use this spreadsheet are available in the link provided below for the next task.|[AD FS Capacity Planning Spreadsheet](http://adfsdocs.blob.core.windows.net/adfs/ADFSCapacityPlanning.xlsx)|  
|Gather data about the number of users who will require single sign\-on \(SSO\) access to the target claims\-aware application and the expected peak usage periods associated with this access|This user data you collect will be used for the input values required within the context of the AD FS Capacity Planning Sizing Spreadsheet.|[Estimate the number of federation servers for your organization](#bk_estimatefs)|  
...
The following table describes important terms that are used often in this capacity planning section of the [!INCLUDE[nextref_adfs2]()] Design Guide. For a more complete list of [!INCLUDE[nextref_adfs2]()] terms, see [Understanding Key AD FS Concepts]().  
...
|Windows Internal Database \(WID\)|The default [!INCLUDE[nextref_adfs2]()] configuration database that can be used as an alternative to SQL Server in certain [!INCLUDE[nextref_adfs2]()] deployments.|  
  
## Configuration environment used during AD FS testing  
This section describes the configuration environment that the [!INCLUDE[nextref_adfs2]()] product team used to perform its tests. The team used the following computer hardware, software, and network configuration to gather performance and scalability data in tests of the federation server:  
...
> [!NOTE]  
> Although 16 GB’s of RAM was used on the federation server during testing, a more moderate memory size, such as 4 GB’s of RAM per federation server can be used for most [!INCLUDE[nextref_adfs2]()] deployments. The recommendations that are provided in this [!INCLUDE[nextref_adfs2]()] Capacity Planning content along with the results provided by the [!INCLUDE[nextref_adfs2]()] Capacity Planning Spreadsheet are based on assumptions that each federation server will use approximately 4GB’s of RAM for most [!INCLUDE[nextref_adfs2]()] production environments.  
...
After you have completed the prerequisite tasks and have become familiar with related terms and hardware requirements, you can use the following additional capacity planning content to help you determine the recommended number of [!INCLUDE[nextref_adfs2]()] servers required for your deployment:  
