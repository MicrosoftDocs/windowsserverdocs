...
This topic describes how to publish applications through [!INCLUDE[adfs_webproxy]()] using [!INCLUDE[firstref_adfs2]()] preauthentication.  
  
For all types of application that you can publish using [!INCLUDE[nextref_adfs2]()] preauthentication, you must add a [!INCLUDE[nextref_adfs2]()] relying party trust to the Federation Service.  
  
The general [!INCLUDE[nextref_adfs2]()] preauthentication flow is as follows:  
...
1.  The client device attempts to access a published web application on a particular resource URL; for example https:\/\/app1.contoso.com\/.  
    ...
    The resource URL is a public address on which [!INCLUDE[adfs_webproxy]()] listens for incoming HTTPS requests.  
  
    If HTTP to HTTPS redirection is enabled, [!INCLUDE[adfs_webproxy]()] will also listen for incoming HTTP requests.  
  
2.  [!INCLUDE[adfs_webproxy]()] redirects the HTTPS request to the [!INCLUDE[nextref_adfs2]()] server with URL encoded parameters, including the resource URL and the appRealm \(a relying party identifier\).  
  
    The user authenticates using the authentication method required by the [!INCLUDE[nextref_adfs2]()] server; for example, user name and password, two\-factor authentication with a one\-time password, and so on.  
  
3.  After the user is authenticated, the [!INCLUDE[nextref_adfs2]()] server issues a security token, the ‘edge token’, containing the following information and redirects the HTTPS request back to the [!INCLUDE[adfs_webproxy]()] server:  
...
4.  [!INCLUDE[adfs_webproxy]()] receives the redirected HTTPS request from the [!INCLUDE[nextref_adfs2]()] server with the edge token and validates and uses the token as follows:  
  
    -   Validates that the edge token signature is from the federation service that is configured in the [!INCLUDE[adfs_webproxy]()] configuration.  
...
5.  If the edge token is valid, [!INCLUDE[adfs_webproxy]()] forwards the HTTPS request to the published web application using either HTTP or HTTPS.  
...
7.  [!INCLUDE[adfs_webproxy]()] saves a cookie on the client device. The cookie is used by [!INCLUDE[adfs_webproxy]()] to identify that this session has already been preauthenticated and that no further preauthentication is required.  
...
2.  The web browser sends an HTTPS request to the [!INCLUDE[adfs_webproxy]()] server which redirects the request to the [!INCLUDE[nextref_adfs2]()] server.  
  
3.  The [!INCLUDE[nextref_adfs2]()] server authenticates the user and the device and redirects the request back to [!INCLUDE[adfs_webproxy]()]. The request now contains the edge token. The [!INCLUDE[nextref_adfs2]()] server adds a single sign\-on \(SSO\) cookie to the request because the user has already performed authentication against the [!INCLUDE[nextref_adfs2]()] server.  
  
4.  [!INCLUDE[adfs_webproxy]()] validates the token, adds its own cookie, and forwards the request to the backend server.  
  
5.  The backend server redirects the request to the [!INCLUDE[nextref_adfs2]()] server to get the application security token.  
  
6.  The request is redirected to the backend server by the [!INCLUDE[nextref_adfs2]()] server. The request now contains the application token and the SSO cookie. The user is granted access to the application and is not required to enter a user name or password.  
...
-   Created a relying party trust for the application in the [!INCLUDE[nextref_adfs2]()] Management console.  
  
-   Verified that a certificate on the [!INCLUDE[adfs_webproxy]()] server is suitable for the application you want to publish.  
...
1.  On the [!INCLUDE[adfs_webproxy]()] server, in the Remote Access Management console, in the **Navigation** pane, click **[!INCLUDE[adfs_webproxy]()]**, and then in the **Tasks** pane, click **Publish**.  
  
2.  On the **Publish New Application Wizard**, on the **Welcome** page, click **Next**.  
  
3.  On the **Preauthentication** page, click **[!INCLUDE[firstref_adfs2]()]**, and then click **Next**.  
...
![](../media/Publishing-Applications-using-AD-FS-Preauthentication/PowerShellLogoSmall.gif)**[!INCLUDE[wps_proc_title]()]**  
  
[!INCLUDE[wps_proc_intro]()]  
...
[!INCLUDE[adfs_webproxy]()] can be used to publish applications that uses Integrated Windows authentication; that is, [!INCLUDE[adfs_webproxy]()] performs preauthentication as required, and can then perform SSO to the published application that uses Integrated Windows authentication. To publish an application that uses Integrated Windows authentication you must add a non\-claims\-aware relying party trust for the application to the Federation Service.  
  
To allow [!INCLUDE[adfs_webproxy]()] to perform single sign\-on \(SSO\) and to perform credentials delegation using Kerberos constrained delegation, the [!INCLUDE[adfs_webproxy]()] server must be joined to a domain. See [Plan Active Directory](assetId:///a0d4c023-e75b-4140-b0aa-0fdb7f28a59d#BKMK_AD).  
  
To allow users to access applications that use Integrated Windows authentication, the [!INCLUDE[adfs_webproxy]()] server must be able to provide delegation for users to the published application. You can do this on the domain controller for any application. You can also do this on the backend server if it is running on [!INCLUDE[winblue_server_2]()] or [!INCLUDE[win8_server_2]()]. For more information, see [What's New in Kerberos Authentication](http://technet.microsoft.com/library/hh831747.aspx).  
  
For a walkthrough of how to configure [!INCLUDE[adfs_webproxy]()] to publish an application using Integrated Windows authentication, see [Configure a site to use Integrated Windows authentication](assetId:///b0788958-627f-450f-877c-209b1bd0db52).  
  
When using Integrated Windows authentication to backend servers, the authentication between [!INCLUDE[adfs_webproxy]()] and the published application is not claims\-based, instead it uses Kerberos constrained delegation to authenticate end users to the application. The general flow is described below:  
...
3.  The [!INCLUDE[nextref_adfs2]()] server authenticates the user and redirects the request back to [!INCLUDE[adfs_webproxy]()]. The request now contains the edge token.  
  
4.  [!INCLUDE[adfs_webproxy]()] validates the token.  
  
5.  If the token is valid, [!INCLUDE[adfs_webproxy]()] obtains a Kerberos ticket from the domain controller on behalf of the user.  
  
6.  [!INCLUDE[adfs_webproxy]()] adds the Kerberos ticket to the request as part of the Simple and Protected GSS\-API Negotiation Mechanism \(SPNEGO\) token, and forwards the request to the backend server. The request contains the Kerberos ticket; therefore, the user is granted access to the application without the need for further authentication.  
...
-   Created a non\-claims\-aware relying party trust for the application in the [!INCLUDE[nextref_adfs2]()] Management console.  
  
-   Configured the backend server to support Kerberos constrained delegation on the domain controller or by using the Set\-ADUser cmdlet with the \-PrincipalsAllowedToDelegateToAccount parameter. Note that if the backend server is running on [!INCLUDE[winblue_server_2]()] or [!INCLUDE[win8_server_2]()], you can also run this PowerShell command on the backend server.  
  
-   Made sure that the [!INCLUDE[adfs_webproxy]()] servers are configured for delegation to the service principal names of the backend servers.  
...
3.  [!INCLUDE[adfs_webproxy]()] redirects the request to the [!INCLUDE[nextref_adfs2]()] server, which performs the authentication.  
  
4.  The [!INCLUDE[nextref_adfs2]()] server redirects the request back to [!INCLUDE[adfs_webproxy]()]. The request now contains the edge token.  
  
5.  The [!INCLUDE[nextref_adfs2]()] server adds a single sign\-on \(SSO\) cookie to the request because the user has already performed authentication against the [!INCLUDE[nextref_adfs2]()] server.  
  
6.  [!INCLUDE[adfs_webproxy]()] validates the token and forwards the request to the backend server.  
  
7.  The backend server redirects the request to the [!INCLUDE[nextref_adfs2]()] server to get the application security token.  
...
The steps to publish an application that uses MS\-OFBA are identical to the steps for a claims\-based application or a non\-claims\-based application. For claims\-based applications, see [Publish a Claims-based Application for Web Browser Clients](../web-application-proxy/../web-application-proxy/Publishing-Applications-using-AD-FS-Preauthentication.md#BKMK_1.1), for non\-claims\-based applications, see [Publish an Integrated Windows authenticated-based Application for Web Browser Clients](../web-application-proxy/../web-application-proxy/Publishing-Applications-using-AD-FS-Preauthentication.md#BKMK_1.2). [!INCLUDE[adfs_webproxy]()] automatically detects the client and will authenticate the user as required.  
  
## Publish an Application that uses HTTP Basic  
HTTP Basic is the authorization protocol used by many protocols, including ActiveSync, to connect rich clients, including smartphones, with your Exchange mailbox. For more information on HTTP Basic, see [RFC 2617](http://www.ietf.org/rfc/rfc2617.txt). [!INCLUDE[adfs_webproxy]()] traditionally interacts with [!INCLUDE[nextref_adfs2]()] using redirections which is not supported on ActiveSync clients; most rich clients don’t support cookies or state management. Publishing an app using HTTP basic provides support for ActiveSync clients in [!INCLUDE[adfs_webproxy]()] by caching the token that is received from AD FS and serving the token from the cache to overcome this limitation and avoid a high load on AD FS. In this way [!INCLUDE[adfs_webproxy]()] enables the HTTP app to receive a non\-claims relying party trust for the application to the Federation Service. See [Plan Active Directory](assetId:///a0d4c023-e75b-4140-b0aa-0fdb7f28a59d#BKMK_AD).  
...
2.  The app sends an HTTPS request to the URL published by [!INCLUDE[adfs_webproxy]()].  
  
3.  If the request does not contain credentials, [!INCLUDE[adfs_webproxy]()] returns an HTTP 401 response to the app containing the URL of the authenticating [!INCLUDE[nextref_adfs2]()] server.  
...
5.  Because the device cannot be redirected to [!INCLUDE[nextref_adfs2]()], the [!INCLUDE[adfs_webproxy]()] sends an authentication request to [!INCLUDE[nextref_adfs2]()] with the credentials that it has: username, password and, if available, device certificate. The token is acquired on behalf of the device.  
  
6.  In order to minimize the number of requests sent to the [!INCLUDE[nextref_adfs2]()], [!INCLUDE[adfs_webproxy]()], validates subsequent client requests using cached tokens for as long as the token is valid. [!INCLUDE[adfs_webproxy]()] periodically cleans the cache. You can view the size of the cache using the performance counter.  
  
7.  If the token is valid, [!INCLUDE[adfs_webproxy]()] forwards the request to the backend server and the user is granted access to the published web application.  
...
> [!NOTE]  
> [!INCLUDE[adfs_webproxy]()] supports publishing only for Windows Store apps that use the OAuth 2.0 protocol.  
  
In the [!INCLUDE[nextref_adfs2]()] Management console, you must make sure that the OAuth endpoint is proxy enabled. To check if the OAuth endpoint is proxy enabled, open the AD FS Management console, expand **Service**, click **Endpoints**, in the **Endpoints** list, locate the OAuth endpoint and make sure that the value in the **Proxy Enabled** column is **Yes**.  
  
The authentication flow for clients that use Windows Store apps is described below:  
  
> [!NOTE]  
> The [!INCLUDE[adfs_webproxy]()] redirects to the [!INCLUDE[nextref_adfs2]()] server for authentication. Because Windows Store apps do not support redirects, if you use Windows Store apps, it is necessary to set the URL of the [!INCLUDE[nextref_adfs2]()] server using the Set\-WebApplicationProxyConfiguration cmdlet and the OAuthAuthenticationURL parameter.  
>   
> Windows Store apps can be published only using [!INCLUDE[wps_2]()].  
  
1.  The client attempts to access a published web application using a Windows Store app.  
  
2.  The app sends an HTTPS request to the URL published by [!INCLUDE[adfs_webproxy]()].  
  
3.  [!INCLUDE[adfs_webproxy]()] returns an HTTP 401 response to the app containing the URL of the authenticating [!INCLUDE[nextref_adfs2]()] server. This process is known as ‘discovery’.  
  
    > [!NOTE]  
    > If the app knows the URL of the authenticating [!INCLUDE[nextref_adfs2]()] server and already has a combo token containing the OAuth token and the edge token, steps 2 and 3 are skipped in this authentication flow.  
  
4.  The app sends an HTTPS request to the [!INCLUDE[nextref_adfs2]()] server.  
  
5.  The app uses the web authentication broker to generate a dialog box in which the user enters credentials to authenticate to the [!INCLUDE[nextref_adfs2]()] server. For information about web authentication broker, see [Web authentication broker](http://msdn.microsoft.com/library/windows/apps/hh750287.aspx).  
  
6.  After successful authentication, the [!INCLUDE[nextref_adfs2]()] server creates a combo token that contains the OAuth token and the edge token and sends the token to the app.  
  
7.  The app sends an HTTPS request containing the combo token to the URL published by [!INCLUDE[adfs_webproxy]()].  
  
8.  [!INCLUDE[adfs_webproxy]()] splits the combo token into its two parts and validates the edge token.  
  
9. If the edge token is valid, [!INCLUDE[adfs_webproxy]()] forwards the request to the backend server with only the OAuth token. The user is granted access to the published web application.  
  
This procedure describes how to publish an application for OAuth2. This type of application can be published only using [!INCLUDE[wps_2]()]. Before you begin, make sure that you have done the following:  
  
-   Created a relying party trust for the application in the [!INCLUDE[nextref_adfs2]()] Management console.  
  
-   Made sure that the OAuth endpoint is proxy enabled in the [!INCLUDE[nextref_adfs2]()] Management console and make a note of the URL Path.  
...
To set the OAuth authentication URL for a [!INCLUDE[adfs2_fs]()] address of fs.contoso.com and a URL path of \/adfs\/oauth2\/:  
