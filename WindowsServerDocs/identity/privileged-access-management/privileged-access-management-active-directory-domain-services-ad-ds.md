...
Privileged Access Management \(PAM\) for Active Directory Domain Services \(AD DS\) is a solution that is based on Microsoft Identity Manager \(MIM\) and Windows Server 2012 R2 and Windows Server 2016 Technical Preview. It helps organizations restrict privileged access within an existing Active Directory environment.  
...
![](../media/privileged-access-management-active-directory-domain-services-ad-ds/mim-pam-pamsetupprocess.png)  
...
![](../media/privileged-access-management-active-directory-domain-services-ad-ds/mim-pam-howpamworksv2.png)  
...
Assuming the request is approved, the Action workflow communicates directly with bastion forest Active Directory to put a user in a group. For example, when Jen requests to administer the HR database, the administrative account for Jen is added to the privileged group in the bastion forest within seconds. Her administrative account’s membership in that group will expire after a time limit, such as an hour, or a day, or a week, or whatever is appropriate for that request. With Windows Server 2016 Technical Preview, that membership is associated in Active Directory with a time limit; with Windows Server 2012 R2 in the bastion forest, that time limit is enforced by MIM.  
