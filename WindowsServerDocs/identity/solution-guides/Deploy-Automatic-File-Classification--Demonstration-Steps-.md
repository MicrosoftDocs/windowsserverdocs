...
-   A content classification rule that searches a set of files for the string â€œContoso Confidential.â€ If the string is found in a file, the Impact resource property is set to High on the file.  
...
```  
Set-ADResourceProperty â€"Enabled:$true â€"Identity:â€CN=Impact_MS,CN=Resource Properties,CN=Claims Configuration,CN=Services,CN=Configuration,DC=contoso,DC=comâ€   
Set-ADResourceProperty â€"Enabled:$true â€"Identity:â€CN=PII_MS,CN=Resource Properties,CN=Claims Configuration,CN=Services,CN=Configuration,DC=contoso,DC=comâ€  
```  
  
## <a name="BKMK_Step2"></a>Step 2: Create a string content classification rule  
A string content classification rule scans a file for a specific string. If the string is found, the value of a resource property can be configured. In this example, we will scan each file on a network shared folder and look for the string â€œContoso Confidential.â€ If the string is found, the associated file is classified as having high business impact.  
...
New-FSRMClassificationRule -Name â€œContoso Confidentialâ€ -Property "Impact_MS" -PropertyValue "3000" -Namespace @(â€œD:\Finance Documentsâ€) -ClassificationMechanism "Content Classifier" -Parameters @("StringEx=Min=1;Expr=Contoso Confidential") -ReevaluateProperty Overwrite  
```  
...
```  
New-FSRMClassificationRule -Name "PII Rule" -Property "PII_MS" -PropertyValue "5000" -Namespace @(â€œD:\Finance Documentsâ€) -ClassificationMechanism "Content Classifier" -Parameters @("RegularExpressionEx=Min=10;Expr=^(?!000)([0-7]\d{2}|7([0-7]\d|7[012]))([ -]?)(?!00)\d\d\3(?!0000)\d{4}$") -ReevaluateProperty Overwrite  
```  
...
1.  On the file server, run the classification rules by using File Server Resource Manager.  
    ...
    4.  You can do this by using Windows PowerShell with the following command: **Start-FSRMClassification â€"RunDuration 0 -Confirm:$false**  
