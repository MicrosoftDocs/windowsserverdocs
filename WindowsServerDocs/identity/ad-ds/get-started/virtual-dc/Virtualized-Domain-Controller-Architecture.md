...
![](media/ADDS_VDC_InitialCloningProcess.png)  
  
**Cloning retry operation**  
  
![](media/ADDS_VDC_CloningRetryProcess.png)  
...
2.  The virtual machine then reads the VM\-Generation ID provided by the VMGenerationCounter driver. It compares the two VM\-Generation IDs.  
    ...
    2.  If the two IDs do not match, this is a new virtual machine that contains an NTDS.DIT from a previous domain controller \(or it is a restored snapshot\). If a DCCloneConfig.xml file exists, the domain controller proceeds with cloning operations. If not, it continues with snapshot restoration operations. See [Virtualized domain controller safe restore architecture](Virtualized-Domain-Controller-Architecture.md#BKMK_SafeRestoreArch).  
...
![](media/ADDS_VDC_VirtualizationSafeguardsDuringNormalBoot.png)  
...
![](media/ADDS_VDC_VirtualizationSafeguardsDuringSnapShotRestore.png)  
