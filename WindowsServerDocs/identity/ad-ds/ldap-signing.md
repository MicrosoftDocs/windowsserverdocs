...
When channel binding is enabled:

1. The client establishes an SSL/TLS connection to the LDAP server
2. A Channel Binding Token is generated based on the SSL/TLS session parameters
3. The client includes this token when authenticating
4. The server validates that the token matches the current SSL/TLS session
5. If tokens mismatch or are missing when required, the connection is rejected
...
In Windows Server versions prior to 2025, LDAP security features are available but not enforced by default:
...
This permissive default allowed compatibility with legacy applications and devices but required administrators to manually enable security features to protect against man-in-the-middle attacks and session hijacking.

### Windows Server 2025 and later

Windows Server 2025 and later significantly strengthens default security posture for new deployments:

- **LDAP signing**: Mandatory by default for all new Active Directory deployments through the "Domain controller: LDAP server signing requirements enforcement" policy
...
These enhanced defaults protect new environments immediately upon deployment, compatible with clients that support modern security features.
...
- **Gradual enforcement recommended**: Evaluate client compatibility using event monitoring before enabling signing or channel binding requirements
...
To learn more about the evolution of these requirements and specific deployment timelines, see [2020, 2023, and 2024 LDAP channel binding and LDAP signing requirements for Windows](https://support.microsoft.com/topic/2020-2023-and-2024-ldap-channel-binding-and-ldap-signing-requirements-for-windows-kb4520412-ef185fb8-00f7-167d-744c-f299a66fc00a).
...
LDAP signing and channel binding generate specific events that help you monitor security status and troubleshoot connectivity issues. These events appear in Event Viewer under the Directory Service category.

### LDAP signing events

| Event ID | Name | Description | Recommended action |
|----------|------|-------------|-------------------|
| 2886 | LDAP signing not required | Domain controller allows unsigned LDAP communications | Evaluate client compatibility and consider requiring signing |
...
| 2888 | LDAP signing disabled | Domain controller allows all unsigned traffic (highly vulnerable) | Immediately enable LDAP signing to protect environment |
...
2. **Monitor logs regularly**: Check Event Viewer > Directory Service for events 2886-2889 and 3039-3041
3. **Identify problematic clients**: Investigate sources of Event 2887 or 3039; update client configurations to support signing and channel binding
4. **Implement gradually**: Start with "Negotiate signing" and "When supported" policies, then move to full enforcement after verifying client compatibility
5. **Use audit mode**: Deploy changes in audit-only mode first to identify incompatible clients before enforcing
6. **Update legacy systems**: Upgrade or replace clients that can't support signing or channel binding; isolate them on separate network segments if necessary
...
To learn more about LDAP client performance counters, see [Active Directory LDAP client performance counters](ldap-client-performance-counters.md).
...
- [2020, 2023, and 2024 LDAP channel binding and LDAP signing requirements for Windows](https://support.microsoft.com/topic/2020-2023-and-2024-ldap-channel-binding-and-ldap-signing-requirements-for-windows-kb4520412-ef185fb8-00f7-167d-744c-f299a66fc00a)
- [Frequently asked questions about changes to Lightweight Directory Access Protocol](https://support.microsoft.com/topic/frequently-asked-questions-about-changes-to-lightweight-directory-access-protocol-41a40287-810a-e799-d067-f578fca055fc)
