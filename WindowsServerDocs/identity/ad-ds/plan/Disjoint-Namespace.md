...
- Manually configured Service Principal Names (SPNs) may no longer match DNS names after a namespace change. This can cause authentication failures.
    ...
    - If you use Windows Server 2003-based computers with constrained delegation, those computers may require you to manually edit the **msDS-AllowedToDelegateTo** attribute in Active Directory. additional configuration to change SPNs. For more information, see the [ms-DS-Allowed-To-Delegate-To attribute](/windows/win32/adschema/a-msds-allowedtodelegateto).
