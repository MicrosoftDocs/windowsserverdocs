...
After you have restored or re-installed all DCs, you can verify that AD DS and SYSVOL are recovered and replicating correctly by using **repadmin /replsum**. **repadmin /replsum** runs on any version of Windows Server.
...
Check the DFS Replication log in Event Viewer for Event ID 4602 (or File Replication Service event ID 13516). This log event indicates SYSVOL has been initialized.

If the first recovered DC logs Event ID 4614 (“the domain controller is waiting to perform initial replication. The replicated folder will remain in the initial synchronization state until it has replicated with its partner”) in the DFS Replication log, then Event ID 4602 doesn't appear and you need to perform the following manual steps to recover SYSVOL if it is replicated by DFSR:
