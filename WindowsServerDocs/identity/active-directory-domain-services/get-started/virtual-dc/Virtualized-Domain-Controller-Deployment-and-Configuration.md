...
-   Step 2: Verify the PDC emulator role is hosted by a domain controller that runs [!INCLUDE[win8_server_2]()] and that it is online and reachable by the cloned domain controller during cloning.  
...
If the hypervisor is Microsoft Hyper\-V, ensure it is running on [!INCLUDE[win8_server_2]()]. You can validate this using Device Management  
...
Alternatively, run the sample [FixVDCPermissions.ps1](#BKMK_FixPDCPerms) in a Windows PowerShell console, where the console starts as an elevated administrator on a domain controller in the affected domain. It automatically set the permissions. The sample is located in the appendix of this module.  
...
Review the previous [Platform Requirements](../../../active-directory-domain-services/get-started/virtual-dc/../../../active-directory-domain-services/get-started/virtual-dc/../../../active-directory-domain-services/get-started/virtual-dc/../../../active-directory-domain-services/get-started/virtual-dc/Virtualized-Domain-Controller-Deployment-and-Configuration.md#BKMK_PlatformReqs) section for known VM\-Generation ID support.  
