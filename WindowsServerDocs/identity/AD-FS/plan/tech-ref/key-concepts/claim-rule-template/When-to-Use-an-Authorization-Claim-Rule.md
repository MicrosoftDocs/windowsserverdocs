...
-   In the [!INCLUDE[nextref_adfs2]()] Management snap\-in, claim rules can only be created using claim rule templates
...
[!INCLUDE[nextref_adfs2]()]defines two claim types that are used to determine whether a user is permitted or denied. These claim type Uniform Resource Identifiers \(URIs\) are as follows:
...
If a claim should be sent only when the claim value matches a custom pattern, you must use a custom rule. For more information, see [When to Use a Custom Claim Rule](../../../../../active-directory-federation-services/plan/tech-ref/key-concepts/claim-rule-template/When-to-Use-a-Custom-Claim-Rule.md).
