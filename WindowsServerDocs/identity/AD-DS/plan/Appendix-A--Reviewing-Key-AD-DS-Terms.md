...
The following terms are relevant to the deployment process for [!INCLUDE[nextref_longhorn]()] Active Directory Domain Services \(AD DS\).  
...
The process of upgrading the operating systems of all domain controllers in a given domain, for example, upgrading Windows Server 2003 to [!INCLUDE[nextref_longhorn]()], and raising the functional level of the domain, if applicable, while leaving domain objects, such as users and groups, in place.  
