...
16. Clear the **Enable remote control** flag. It should never be necessary for support staff to connect to this account’s sessions to implement fixes.  
    ...
    > [!NOTE]  
    > Every object in Active Directory should have a designated IT owner and a designated business owner, as described in [Planning for Compromise](). If you are tracking ownership of AD DS objects in Active Directory \(as opposed to an external database\), you should enter appropriate ownership information in this object’s properties.  
    >   
...
For more information about recommended audit configuration settings for domain controllers, see [Monitoring Active Directory for Signs of Compromise](). Domain controller\-specific configuration settings are provided in [Monitoring Active Directory for Signs of Compromise]().  
