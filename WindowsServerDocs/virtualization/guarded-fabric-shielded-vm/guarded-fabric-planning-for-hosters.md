...
Deploy the Host Guardian Service (HGS) in a highly secure environment, whether that be on dedicated physical server, a VM on an isolated Hyper-V host (separated from the fabric it's protecting), or one logically separated, for example, by a subscription in Azure, or as a shielded VM running in a barnch office.  
