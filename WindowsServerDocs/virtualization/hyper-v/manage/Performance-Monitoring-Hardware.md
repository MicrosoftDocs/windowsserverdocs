---
title: Exposing Intel Performance Monitoring Hardware to a Hyper-V Virtual Machine
description: "Desribes how to expose Intel's Performance Monitorning Hardware to a Hyper-V Machine. Also touches on how enablement effects live migration."
...
ms.date: 09/20/2019
---
# Exposing Intel Performance Monitoring Hardware to a Hyper-V Virtual Machine
...
## Requirements 
To expose performance monitoring hardware into a virtual machine, you'll need:
...
- A Hyper-V virtual machine _without_ [nested virtualization](https://docs.microsoft.com/virtualization/hyper-v-on-windows/user-guide/nested-virtualization) that is in the stopped state
 
## Exposing the PMU capabilities (PMU, LBR, PEBS) to virtual machines via Powershell's Set-VMProcessor Cmdlet
...
>Note: When enabling the perfmon components, if `"pebs"` is specified, then `"pmu"` must be specified.  Also, enabling a perfmon component that is not supported by the host's physical processors will result in a virtual machine start failure.
 
## Effect of PMU enabelement on save/restore, export, and live migration
