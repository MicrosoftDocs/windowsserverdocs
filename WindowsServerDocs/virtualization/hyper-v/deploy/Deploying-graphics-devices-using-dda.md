...
The first step in the solution is to address DDA restrictions to the VMs. Configure the `Automatic Stop Action` of a VM to enable **TurnOff** with the following PowerShell cmdlet: `Set-VM -Name VMName -AutomaticStopAction TurnOff`.
