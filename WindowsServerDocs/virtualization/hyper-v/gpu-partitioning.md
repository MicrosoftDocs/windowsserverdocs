...
AMD EPYC 7002 (Rome) processors support GPU partitioning; however, due to a silicon limitation, AMD EPYC 7002 doesn't support live migration with GPU partitioning. To use live migration with GPU partitioning, your hosts must use processors that support IOMMU DMA bit tracking. Examples of processors that support IOMMU DMA bit tracking for live migration with GPU partitioning include:
...
> [!NOTE]
> AMD V710 supports full GPU live migration (where the entire GPU is assigned to a single VM), but doesn't support live migration with GPU partitioning on Windows Server 2025.
