...
You can use this procedure to configure hash publication for BranchCache using local computer Group Policy on a file server that is running [!INCLUDE[winthreshold_server_1](includes/winthreshold_server_1_md.md)] with the **BranchCache for Network Files** role service of the File Services server role installed.  
