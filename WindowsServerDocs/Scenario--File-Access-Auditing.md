...
|-----------------|---------------------------------|
|Active Directory Doman Services role|AD DS in [!INCLUDE[win8_server_2](includes/win8_server_2_md.md)] introduces a claims\-based authorization platform that enables creating user claims and device claims, compound identity, \(user plus device claims\), new central access policies \(CAP\) model, and the use of file classification information in authorization decisions.|
|File and Storage Services role|File servers in [!INCLUDE[win8_server_2](includes/win8_server_2_md.md)] provide a user interface where administrators can view the effective permissions for users for a file or folder and troubleshoot access issues and grant access as required.|
