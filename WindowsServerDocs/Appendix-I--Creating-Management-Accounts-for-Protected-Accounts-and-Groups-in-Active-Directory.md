...
16. Clear the **Enable remote control** flag. It should never be necessary for support staff to connect to this account’s sessions to implement fixes.
    ...
    > [!NOTE]
    > Every object in Active Directory should have a designated IT owner and a designated business owner, as described in [Planning for Compromise](Planning-for-Compromise.md). If you are tracking ownership of AD DS objects in Active Directory \(as opposed to an external database\), you should enter appropriate ownership information in this object’s properties.
    > 
...
1.  To test enabling a management account and resetting its password, log on to a secure administrative workstation with an account that is a member of the group you created in [Appendix I: Creating Management Accounts for Protected Accounts and Groups in Active Directory](Appendix-I--Creating-Management-Accounts-for-Protected-Accounts-and-Groups-in-Active-Directory.md).
