...
There are two HNV implementations which will be available in [!INCLUDE[winthreshold_server_2](includes/winthreshold_server_2_md.md)]: HNVv1 and HNVv2.

-   **HNVv1**

    HNVv1 is compatible with [!INCLUDE[winblue_server_2](includes/winblue_server_2_md.md)] and [!INCLUDE[vmmblue_1](includes/vmmblue_1_md.md)] \(VMM\). Configuration for HNVv1 relies on WMI management and [!INCLUDE[wps_2](includes/wps_2_md.md)] cmdlets \(facilitated through System Center VMM\) to define isolation settings and Customer Address \(CA\) – virtual network – to Physical Address \(PA\) mappings and routing. No additional features have been added to HNVv1 in [!INCLUDE[winthreshold_server_2](includes/winthreshold_server_2_md.md)] and no new features are planned.
...
Gateways can come in different physical form factors. They can be built on [!INCLUDE[winthreshold_server_2](includes/winthreshold_server_2_md.md)], incorporated into a Top of Rack \(TOR\) switch acting as a VXLAN Gateway, accessed through a Virtual IP \(VIP\) advertised by a load balancer, put into other existing network appliances, or can be a new stand\-alone network appliance.

For more information about Windows RAS Gateway options, see [RAS Gateway](RAS-Gateway.md).
...
|**Related Technologies**|-   For Hyper\-V Network Virtualization  technical details in [!INCLUDE[winblue_server_2](includes/winblue_server_2_md.md)], see [Hyper-V Network Virtualization technical details](https://technet.microsoft.com/library/jj134174.aspx)<br />-   [Network Controller](Network-Controller.md)|
