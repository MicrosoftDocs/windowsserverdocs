---
title: How CAU Plug-ins Work in Windows Server Threshold
ms.custom: na
ms.reviewer: na
ms.suite: na
ms.tgt_pltfrm: na
ms.topic: article
ms.assetid: 638669c0-9092-4460-a445-fe932a6affaa
---
# How CAU Plug-ins Work in Windows Server Threshold
To preview or apply updates by using the Cluster\-Aware Updating \(CAU\) user interface \(UI\) or the CAU [!INCLUDE[wps_2](includes/wps_2_md.md)] cmdlets, you must specify one or more CAU plug\-ins. A plug\-in installs updates across all the nodes in a failover cluster by using a specific updating tool and update source. This topic provides information about using the built\-in CAU plug\-ins or other plug\-ins that you install for CAU.
...
To install a plug\-in, follow the instructions from the plug\-in publisher. To manually register a plug\-in with CAU, run the [Register-CauPlugin](http://technet.microsoft.com/library/hh847225) cmdlet on each computer where the plug\-in is installed.
...
> [!TIP]
> In the CAU UI, you can only specify a single plug\-in for CAU to use to preview or to apply updates during an Updating Run. By using the CAU [!INCLUDE[wps_2](includes/wps_2_md.md)] cmdlets, you can specify one or more plug\-ins. If you need to install multiple types of updates on the cluster, it is usually more efficient to specify multiple plug\-ins in one Updating Run, rather than using a separate Updating Run for each plug\-in. For example, fewer node restarts will typically occur.
...
By using the CAU [!INCLUDE[wps_2](includes/wps_2_md.md)] cmdlets that are listed in the following table, you can specify one or more plug\-ins for an Updating Run or scan by passing the **–CauPluginName** parameter. You can specify multiple plug\-in names by separating them with commas. If you specify multiple plug\-ins, you can also control how the plug\-ins influence each other during an Updating Run by specifying the **\-RunPluginsSerially**, **\-StopOnPluginFailure**, and **–SeparateReboots** parameters. For more information about using multiple plug\-ins, use the links provided to the cmdlet documentation in the following table.

|Cmdlet|Description|
|----------|---------------|
|[Add-CauClusterRole](http://technet.microsoft.com/library/hh847235)|Adds the CAU clustered role that provides the self\-updating functionality to the specified cluster.|
|[Invoke-CauRun](http://technet.microsoft.com/library/hh847223)|Performs a scan of cluster nodes for applicable updates and installs those updates through an Updating Run on the specified cluster.|
|[Invoke-CauScan](http://technet.microsoft.com/library/hh847228)|Performs a scan of cluster nodes for applicable updates and returns a list of the initial set of updates that would be applied to each node in the specified cluster.|
|[Set-CauClusterRole](http://technet.microsoft.com/library/hh847234)|Sets configuration properties for the CAU clustered role on the specified cluster.|
...
To specify plug\-in arguments by using the CAU [!INCLUDE[wps_2](includes/wps_2_md.md)] cmdlets that support the **–CauPluginParameters** parameter, pass a parameter of the form:

**\-CauPluginArguments @{Name1\=Value1;Name2\=Value2;Name3\=Value3}**

You can also use a predefined [!INCLUDE[wps_2](includes/wps_2_md.md)] hash table. To specify plug\-in arguments for more than one plug\-in, pass multiple hash tables of arguments, separated with commas. Pass the plug\-in arguments in the plug\-in order that is specified in **CauPluginName**.

### Specify optional plug\-in arguments
The plug\-ins that CAU installs \(**Microsoft.WindowsUpdatePlugin** and **Microsoft.HotfixPlugin**\) provide additional options that you can select. In the CAU UI, these appear on an **Additional Options** page after you configure Updating Run options for the plug\-in. If you are using the CAU [!INCLUDE[wps_2](includes/wps_2_md.md)] cmdlets, these options are configured as optional plug\-in arguments. For more information, see [Use the Microsoft.WindowsUpdatePlugin](#BKMK_WUP) and [Use the Microsoft.HotfixPlugin](#BKMK_HFP) later in this topic.

## Manage plug\-ins using Windows PowerShell cmdlets

|Cmdlet|Description|
|----------|---------------|
|[Get-CauPlugin](http://technet.microsoft.com/library/hh847233)|Retrieves information about one or more software updating plug\-ins that are registered on the local computer.|
|[Register-CauPlugin](http://technet.microsoft.com/library/hh847225)|Registers a CAU software updating plug\-in on the local computer.|
|[Unregister-CauPlugin](http://technet.microsoft.com/library/hh847225)|Removes a software updating plug\-in from the list of plug\-ins that can be used by CAU. **Note:** The plug\-ins that are installed with CAU \(**Microsoft.WindowsUpdatePlugin** and the **Microsoft.HotfixPlugin**\) cannot be unregistered.|
...
2.  To enable the necessary WMI permissions for the account, start the WMI Management Console on the SMB server. Start [!INCLUDE[wps_2](includes/wps_2_md.md)] and then type the following command:
...
-   If you are using the CAU [!INCLUDE[wps_2](includes/wps_2_md.md)] cmdlets, configure the **DisableAclChecks\='True'** argument in the **CauPluginArguments** parameter for the hotfix plug\-in.
...
-   To enable SMB Encryption for the SMB shared folder, run the following [!INCLUDE[wps_2](includes/wps_2_md.md)] cmdlet on the SMB server:
...
Optionally, to enforce the use of SMB Encryption in the connections to the SMB server, select the **Require SMB Encryption in accessing the hotfix root folder** option in the CAU UI, or configure the **RequireSMBEncryption\='True'** plug\-in argument by using the CAU [!INCLUDE[wps_2](includes/wps_2_md.md)] cmdlets.
...
You must enable the **File Server Remote Management \(SMB\-in\)** rule in Windows Firewall on the SMB file server. This is enabled by default in [!INCLUDE[winblue_server_2](includes/winblue_server_2_md.md)] and [!INCLUDE[win8_server_2](includes/win8_server_2_md.md)].

## See also

-   [Cluster-Aware Updating Overview](assetId:///a8e6dfbb-9d98-4130-86ac-9f6f00241e02)

-   [Cluster-Aware Updating Windows PowerShell Cmdlets](http://go.microsoft.com/fwlink/p/?LinkId=237675)
