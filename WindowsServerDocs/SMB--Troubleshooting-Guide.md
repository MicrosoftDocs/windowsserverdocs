...
> [!NOTE]
> With the new Virtual Receive\-side scaling feature in [!INCLUDE[winblue_server_2](includes/winblue_server_2_md.md)], virtual network adapters are now RSS\-capable and provide four TCP connections for each network adapter. In [!INCLUDE[win8_server_2](includes/win8_server_2_md.md)], there was only one TCP connection for each virtual network adapter.
> 
> In [!INCLUDE[winblue_server_2](includes/winblue_server_2_md.md)], SMB client connections are tracked per file share \(instead of per server\), and clients are then redirected to the cluster node with the best access to the volume used by the file share. This improves efficiency by reducing redirection traffic between file server nodes. Clients are redirected following an initial connection and when cluster storage is reconfigured.
