---
title: Manage Software Inventory Logging in Windows Server 2012 R2
ms.custom: na
...
This document describes how to manage Software Inventory Logging in [!INCLUDE[winblue_server_1](includes/winblue_server_1_md.md)].

The Software Inventory Logging feature that was introduced in [!INCLUDE[winblue_server_2](includes/winblue_server_2_md.md)], is intended to help datacenter administrators easily log Microsoft software asset management data for their deployments over time. This document describes how to manage Software Inventory Logging. Before using Software Inventory Logging, make sure that Windows Update [KB 3000850](http://support.microsoft.com/kb/3000850) and [KB 3060681](http://support.microsoft.com/kb/3060681) are installed on each [!INCLUDE[winblue_server_2](includes/winblue_server_2_md.md)] system needing to be inventoried. This feature runs locally on each server to be inventoried. It does not collect data from remote servers.
...
-   [Starting and Stopping Software Inventory Logging](Manage-Software-Inventory-Logging-in-Windows-Server-2012-R2.md#BKMK_Step1)

-   [Software Inventory Logging over time](Manage-Software-Inventory-Logging-in-Windows-Server-2012-R2.md#BKMK_Step2)

-   [Displaying Software Inventory Logging data](Manage-Software-Inventory-Logging-in-Windows-Server-2012-R2.md#BKMK_Step3)

-   [Deleting data logged by Software Inventory Logging](Manage-Software-Inventory-Logging-in-Windows-Server-2012-R2.md#BKMK_Step4)

-   [Backing up and restoring data logged by Software Inventory Logging](Manage-Software-Inventory-Logging-in-Windows-Server-2012-R2.md#BKMK_Step5)

-   [Reading data logged and published by Software Inventory Logging](Manage-Software-Inventory-Logging-in-Windows-Server-2012-R2.md#BKMK_Step6)

-   [Software Inventory Logging Security](Manage-Software-Inventory-Logging-in-Windows-Server-2012-R2.md#BKMK_Step7)

-   [Working with date and time settings in Windows Server Software Inventory Logging](Manage-Software-Inventory-Logging-in-Windows-Server-2012-R2.md#BKMK_Step8)

-   [Enabling and Configuring Software Inventory Logging in a Mounted Virtual Hard Disk](Manage-Software-Inventory-Logging-in-Windows-Server-2012-R2.md#BKMK_Step10)

-   [Overview of Using Software Inventory Logging in Windows Server 2012 R2 without KB 3000850](Manage-Software-Inventory-Logging-in-Windows-Server-2012-R2.md#BKMK_Step11)

-   [Using Software Inventory Logging in a Windows Server 2012 R2 Hyper\-V environment without KB 3000850](Manage-Software-Inventory-Logging-in-Windows-Server-2012-R2.md#BKMK_Step12)

> [!NOTE]
> [!INCLUDE[wps_howtorun](includes/wps_howtorun_md.md)]

## <a name="BKMK_Step1"></a>Starting and Stopping Software Inventory Logging
Software Inventory Logging daily collection and forward over the network must be enabled on a computer running [!INCLUDE[winblue_server_2](includes/winblue_server_2_md.md)] to log software inventory.
...
When using Software Inventory Logging on a [!INCLUDE[winblue_server_2](includes/winblue_server_2_md.md)] Hyper\-V host, it is possible to retrieve SIL data from [!INCLUDE[winblue_server_2](includes/winblue_server_2_md.md)] guests that are running locally, if SIL logging has been started in the guest\(s\).  However, this is only possible when using the Get\-SilData and Publish\-SilData Powershell cmdlets, and only possible with [!INCLUDE[winblue_server_2](includes/winblue_server_2_md.md)] in both host and guest.  The purpose of this capability is to allow data center administrators that provide guest VMs to tenants, or other entities of a large corporation, to capture software inventory data at the hypervisor host and subsequently forward all of this data to an aggregator \(or target URI\).

Below are two examples of what the output on the Powershell console would look like \(much abbreviated\) on a Windows Server 2012 R2 Hyper\-V host running one [!INCLUDE[winblue_server_2](includes/winblue_server_2_md.md)] guest VM with SIL logging started.  You will notice the first example, which uses Get\-SilData alone, will output all data from the host as expected.  Also included is all SIL data from the guest, but in a collapsed format.  To expand and view this data from the guest, simply cut and paste the snippet used in the second example below.  SIL data objects from the guest will always have the VM GUID associated within the object.
...
![](media/SILHyper-VExample1.png)

**Output Example 2** \(w\/Expand\-SilData function\)

![](media/SILHyper-VExample2.png)

## See Also
[Software Inventory Logging Overview](Software-Inventory-Logging-Overview.md)
