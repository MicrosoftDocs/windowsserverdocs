...
If a WSUS server is running in replica mode, you will not be able to perform this task. For more information about replica mode, see [Running WSUS Replica Mode](Running-WSUS-Replica-Mode.md), and [Step 1: Prepare for Your WSUS Deployment](Step-1--Prepare-for-Your-WSUS-Deployment.md).  
...
-   You can specify a custom port for synchronization. For information about configuring ports, see [Step 3: Configure WSUS](Step-3--Configure-WSUS.md) in the WSUS deployment guide.  
  
-   You can use Secure Socket Layers \(SSL\) to secure synchronization of update information between WSUS servers. For more information about using SSL, see section "3.5. Secure WSUS with the Secure Sockets Layer Protocol" of [Step 3: Configure WSUS](Step-3--Configure-WSUS.md) in the WSUS deployment guide.  
