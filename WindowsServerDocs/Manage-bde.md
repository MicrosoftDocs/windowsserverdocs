...
|[Manage-bde: tpm](Manage-bde--tpm.md)|Configures the computer's Trusted Platform Module \(TPM\). This command is not supported on computers running [!INCLUDE[win8_client_2](includes/win8_client_2_md.md)] or **win8\_server\_2**. To manage the TPM on these computers, use either the TPM Management MMC snap\-in or the TPM Management cmdlets for Windows PowerShell.|  
