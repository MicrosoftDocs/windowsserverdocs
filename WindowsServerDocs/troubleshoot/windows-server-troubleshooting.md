---
title: Troubleshooting Windows Server
description: Get links to troubleshooting articles for Windows Server issues
ms.date: 1/24/2020
author: kaushika-msft
ms.author: kaushika
ms.topic: troubleshooting
---
# Troubleshooting Windows Server components

> [!TIP]
> Looking for information about older versions of Windows Server? Check out our other [Windows Server libraries](/previous-versions/windows/) on docs.microsoft.com.
>
> You can also [search this site](/search/index?dataSource=previousVersions&search=Windows+Server) for specific information.

Microsoft regularly releases both updates for Windows Server. To ensure your servers can receive future updates, including security updates, it's important to keep your servers updated. Check out [Windows 10 and Windows Server 2016 update history](https://support.microsoft.com/help/4000825/windows-10-windows-server-2016-update-history) for a complete list of released updates.

This section contains advanced troubleshooting topics and links to help you resolve issues with Windows Server. Additional topics will be added as they become available.

## Troubleshoot activation

- [Troubleshooting Windows volume activation](../get-started/activation-troubleshooting-guide.md)
- [Guidelines for troubleshooting KMS](../get-started/activation-troubleshoot-kms-general.md)
- [Slmgr.vbs options for obtaining volume activation information](../get-started/activation-slmgr-vbs-options.md)
- [Resolve Windows activation error codes](../get-started/activation-error-codes.md)
- [KMS activation known issues](../get-started/activation-troubleshoot-kms-issues.md)
- [MAK activation known issues](../get-started/activation-troubleshoot-mak-issues.md)
- [Guidelines for troubleshooting DNS-related activation issues](../get-started/common-troubleshooting-procedures-kms-dns.md)
- [Rebuild the Tokens.dat file](../get-started/activation-rebuild-tokens-dat-file.md)
- [Troubleshooting ADBA clients](../get-started/activation-troubleshoot-adba-clients.md)

## Troubleshoot startup and restart

- [Advanced troubleshooting for Windows startup](/windows/client-management/troubleshoot-windows-startup)
- [How to determine the appropriate page file size for 64-bit versions of Windows](/windows/client-management/determine-appropriate-page-file-size)
- [Generate a kernel or complete crash dump](/windows/client-management/generate-kernel-or-complete-crash-dump)
- [Introduction to the page file](/windows/client-management/introduction-page-file)
- [Configure system failure and recovery options in Windows](/windows/client-management/system-failure-recovery-options)
- [Advanced troubleshooting for Windows boot problems](/windows/client-management/advanced-troubleshooting-boot-problems)
- [Advanced troubleshooting for Windows-based computer freeze](/windows/client-management/troubleshoot-windows-freeze)
- [Advanced troubleshooting for stop error or blue screen error](/windows/client-management/troubleshoot-stop-errors)
- [Advanced troubleshooting for stop error 7B or Inaccessible_Boot_Device](/windows/client-management/troubleshoot-inaccessible-boot-device)
- [Advanced troubleshooting for Event ID 41 "The system has rebooted without cleanly shutting down first"](/windows/client-management/troubleshoot-event-id-41-restart)
- [Stop error occurs when you update the in-box Broadcom network adapter driver](/windows/client-management/troubleshoot-stop-error-on-broadcom-driver-update)

## Troubleshoot AD forest recovery

- [AD Forest Recovery - FAQ](../identity/ad-ds/manage/ad-forest-recovery-faq.yml)

## Troubleshoot AD replication

- [Troubleshooting Active Directory Replication Problems](../identity/ad-ds/manage/troubleshoot/troubleshooting-active-directory-replication-problems.md)
- [Virtualized Domain Controller Troubleshooting](../identity/ad-ds/manage/virtual-dc/virtualized-domain-controller-troubleshooting.md)
- [Troubleshooting Domain Controller Deployment](../identity/ad-ds/deploy/troubleshooting-domain-controller-deployment.md)
- [Configuring a Computer for Troubleshooting](../identity/ad-ds/manage/troubleshoot/configuring-a-computer-for-troubleshooting.md)

## Troubleshoot AD FS

- [Troubleshooting AD FS](../identity/ad-fs/troubleshooting/ad-fs-tshoot-overview.md)
- [AD FS Troubleshooting - Auditing Events and Logging](../identity/ad-fs/troubleshooting/ad-fs-tshoot-logging.md)
- [AD FS Troubleshooting - SQL Connectivity](../identity/ad-fs/troubleshooting/ad-fs-tshoot-sql.md)
- [AD FS Troubleshooting - Claims Issuance](../identity/ad-fs/troubleshooting/ad-fs-tshoot-claims-issuance.md)
- [AD FS Troubleshooting - Loop Detection](../identity/ad-fs/troubleshooting/ad-fs-tshoot-loop.md)
- [AD FS Troubleshooting - Certificates](../identity/ad-fs/troubleshooting/ad-fs-tshoot-certs.md)
- [AD FS Troubleshooting - Fiddler](../identity/ad-fs/troubleshooting/ad-fs-tshoot-fiddler.md)
- [AD FS Troubleshooting - Fiddler - WS-Federation](../identity/ad-fs/troubleshooting/ad-fs-tshoot-fiddler-ws-fed.md)
- [AD FS Troubleshooting - Claims Rules](../identity/ad-fs/troubleshooting/ad-fs-tshoot-claims-rules.md)
- [AD FS Troubleshooting - Integrated Windows Authentication](../identity/ad-fs/troubleshooting/ad-fs-tshoot-iwa.md)
- [AD FS Troubleshooting - Azure AD](../identity/ad-fs/troubleshooting/ad-fs-tshoot-azure.md)
- [AD FS FAQ](../identity/ad-fs/overview/ad-fs-faq.yml)
- [AD FS Help Diagnostics Analyzer](../identity/ad-fs/troubleshooting/ad-fs-diagnostics-analyzer.md)

## Troubleshoot AoVPN

- [Troubleshoot Always On VPN](../remote/remote-access/vpn/always-on-vpn/deploy/always-on-vpn-deploy-troubleshooting.md)

## Troubleshoot DFSR

- [DFS Replication: Frequently Asked Questions (FAQ)](../storage/dfs-replication/dfsr-faq.yml)

## Troubleshoot DirectAccess

- [Troubleshooting DirectAccess](../remote/remote-access/directaccess/troubleshooting-directaccess.md)

## Troubleshoot disk management

- [Troubleshooting Disk Management](../storage/disk-management/troubleshooting-disk-management.md)

## Troubleshoot DNS

- [Troubleshooting Domain Name System (DNS) issues](../networking/dns/troubleshoot/troubleshoot-dns-data-collection.md)
- [Troubleshooting DNS clients](../networking/dns/troubleshoot/troubleshoot-dns-client.md)
- [Support policy for DNS client-side caching on DNS clients](../networking/dns/troubleshoot/support-policy-for-dns-client-side-caching.md)
- [Troubleshooting DNS Servers](../networking/dns/troubleshoot/troubleshoot-dns-server.md)

## Troubleshoot failover cluster

- [Troubleshooting a Failover Cluster using Windows Error Reporting](../failover-clustering/troubleshooting-using-wer-reports.md)
- [Cluster-Aware Updating - Frequently Asked Questions](../failover-clustering/cluster-aware-updating-faq.yml)
- [Troubleshooting cluster issue with Event ID 1135](./troubleshooting-cluster-event-id-1135.md)
- [Having a problem with nodes being removed from active Failover Cluster membership](./problem-nodes-failover-cluster.md)
- [Nodes being removed from Failover Cluster membership on VMware ESX](./nodes-failover-cluster-vmware.md)
- [IaaS with SQL AlwaysOn - Tuning Failover Cluster Network Thresholds](./iaas-sql-failover-cluster.md)

## Troubleshoot DHCP

- [Troubleshooting guide for Dynamic Host Configuration Protocol (DHCP)](./troubleshoot-dhcp-issue.md)
- [DHCP (Dynamic Host Configuration Protocol) Basics](./dynamic-host-configuration-protocol-basics.md)
- [General guidance to troubleshoot DHCP](./general-guidance-to-troubleshoot-dhcp.md)
- [How to use automatic TCP/IP addressing without a DHCP server](./how-to-use-automatic-tcpip-addressing-without-a-dh.md)
- [Troubleshoot problems on the DHCP client](./troubleshoot-problems-on-dhcp-client.md)
- [Troubleshoot problems on the DHCP server](./troubleshoot-problems-on-dhcp-server.md)

## Troubleshoot FSRM

- [Troubleshooting File Server Resource Manager](../storage/fsrm/troubleshooting-file-server-resource-manager.md)

## Troubleshoot guarded fabric

- [Troubleshooting Using the Guarded Fabric Diagnostic Tool](../security/guarded-fabric-shielded-vm/guarded-fabric-troubleshoot-diagnostics.md)
- [Troubleshooting the Host Guardian Service](../security/guarded-fabric-shielded-vm/guarded-fabric-troubleshoot-hgs.md)
- [Troubleshooting the Host Guardian Service](../security/guarded-fabric-shielded-vm/guarded-fabric-troubleshoot-hosts.md)

## Troubleshoot multi-site RAS

- [Troubleshooting Enabling Multisite](../remote/remote-access/ras/multisite/troubleshoot/troubleshooting-enabling-multisite.md)
- [Troubleshooting Adding Entry Points](../remote/remote-access/ras/multisite/troubleshoot/troubleshooting-adding-entry-points.md)
- [Troubleshooting Setting the Entry Point Domain Controller](../remote/remote-access/ras/multisite/troubleshoot/troubleshooting-setting-the-entry-point-domain-controller.md)
- [Troubleshooting Web Probe URLs](../remote/remote-access/ras/multisite/troubleshoot/troubleshooting-setting-the-entry-point-domain-controller.md)

## Troubleshoot Nano Server

- [Troubleshooting Nano Server](../get-started/troubleshooting-nano-server.md)

## Troubleshoot NIC teaming

- [Troubleshooting NIC Teaming](../networking/technologies/nic-teaming/troubleshooting-nic-teaming.md)

## Troubleshoot OTP authentication

- [Troubleshooting Authentication Issues](../remote/remote-access/ras/otp/troubleshoot/troubleshooting-authentication-issues.md)
- [Troubleshooting Enabling OTP](../remote/remote-access/ras/otp/troubleshoot/troubleshooting-enabling-otp.md)

## Troubleshoot QoS

- [QoS Frequently Asked Questions](../networking/technologies/qos/qos-policy-faq.yml)

## Troubleshoot S2D

- [Storage Spaces Direct troubleshooting](../storage/storage-spaces/troubleshooting-storage-spaces.md)
- [Storage Spaces Direct - Frequently asked questions](../storage/storage-spaces/storage-spaces-direct-faq.yml)
- [Storage Spaces Direct health and operational states](../storage/storage-spaces/storage-spaces-states.md)
- [Collect diagnostic data with Storage Spaces Direct](../storage/storage-spaces/data-collection.md)
- [Storage-class Memory (NVDIMM-N) Health Management in Windows](../storage/storage-spaces/storage-class-memory-health.md)

## Troubleshoot SDN

- [Troubleshoot SDN](../networking/sdn/troubleshoot/troubleshoot-software-defined-networking.md)
- [Troubleshoot the Windows Server Software Defined Networking Stack](../networking/sdn/troubleshoot/troubleshoot-windows-server-software-defined-networking-stack.md)

## Troubleshoot RDS session connectivity

- [General Remote Desktop connection troubleshooting](../remote/remote-desktop-services/troubleshoot/rdp-error-general-troubleshooting.md)
- [Clients can't connect and get the Class not registered error](../remote/remote-desktop-services/troubleshoot/rdp-error-class-not-registered.md)
- [Clients can't connect and see No licenses available error](../remote/remote-desktop-services/troubleshoot/rdp-error-no-licenses-available.md)
- [User can't authenticate or must authenticate twice](../remote/remote-desktop-services/troubleshoot/cannot-authenticate-or-must-authenticate-twice.md)
- [On connecting, user receives Remote Desktop Service is currently busy message](../remote/remote-desktop-services/troubleshoot/remote-desktop-service-currently-busy.md)
- [Remote Desktop client disconnects and can't reconnect to the same session](../remote/remote-desktop-services/troubleshoot/rdp-client-disconnects-cannot-reconnect-same-session.md)
- [Remote laptop disconnects from wireless network](../remote/remote-desktop-services/troubleshoot/remote-laptop-disconnects-wireless-network.md)
- [Poor performance or application problems during remote desktop connection](../remote/remote-desktop-services/troubleshoot/poor-performance-or-application-problems.md)

## Troubleshoot shielded VM

- [Troubleshoot Shielded VMs](../security/guarded-fabric-shielded-vm/guarded-fabric-troubleshoot-shielded-vms.md)

## Troubleshoot software restriction policies

- [Troubleshoot Software Restriction Policies](../identity/software-restriction-policies/troubleshoot-software-restriction-policies.md)

## Troubleshoot storage migration

- [Storage Migration Service known issues](../storage/storage-migration-service/known-issues.md)
- [Storage Migration Service frequently asked questions (FAQ)](../storage/storage-migration-service/faq.yml)

## Troubleshoot Storage Replica

- [Known issues with Storage Replica](../storage/storage-replica/storage-replica-known-issues.md)
- [Frequently Asked Questions about Storage Replica](../storage/storage-replica/storage-replica-frequently-asked-questions.yml)

## Troubleshoot user profiles

- [Troubleshoot user profiles with events](../storage/folder-redirection/troubleshoot-user-profiles-events.md)

## Troubleshoot WebProxy

- [Troubleshooting Web Application Proxy](../remote/remote-access/web-application-proxy/troubleshooting-web-application-proxy.md)

## Troubleshoot Windows Admin Center

- [Windows Admin Center Common Troubleshooting Steps](../manage/windows-admin-center/support/troubleshooting.md)
- [Windows Admin Center Known Issues](../manage/windows-admin-center/support/known-issues.md)
- [Windows Admin Center Frequently Asked Questions](../manage/windows-admin-center/understand/faq.yml)