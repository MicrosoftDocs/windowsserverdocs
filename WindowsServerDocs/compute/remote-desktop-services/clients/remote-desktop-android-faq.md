...
The following information provides answer to frequently asked questions about the Android client.
...
- **VPN is misconfigured.** A misconfigured VPN server can be the reason why the VPN connections never worked or stopped working after some time. Ensure testing with the Android device's web browser or a PC or Mac on the same network if this happens.
