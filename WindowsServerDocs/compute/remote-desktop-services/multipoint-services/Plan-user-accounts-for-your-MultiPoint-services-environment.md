...
> [!NOTE]  
> The following sections discuss scenarios that you might implement for local user accounts in [!INCLUDE[win8_server_multipoint_2](./includes/win8_server_multipoint_2_md.md)]. If you are using domain user accounts, see the “One or more MultiPoint servers  in a domain network environment” scenario in [Example scenarios: MultiPoint Services user accounts](Example-scenarios--MultiPoint-Services-user-accounts.md). For information about setting up domain user accounts in Active Directory Domain Services \(AD DS\), see [Create a New User Account](http://technet.microsoft.com/library/dd861308.aspx) \(http:\/\/technet.microsoft.com\/library\/dd861308.aspx\).  
