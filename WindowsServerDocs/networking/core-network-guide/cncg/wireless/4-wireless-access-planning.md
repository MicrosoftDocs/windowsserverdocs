...
- **Wireless AP IP address \(static\)**. On each wireless AP, configure a unique static IP address. If the subnet is serviced by a DHCP server, ensure that all AP IP addresses fall within a DHCP exclusion range so that the DHCP server does not try to issue the same IP address to another computer or device. Exclusion ranges are documented in the Windows Server 2016 Core Network Guide procedure [Create and Activate a New DHCP Scope](https://technet.microsoft.com/windows-server-docs/networking/core-network-guide/core-network-guide#BKMK_newscopeDHCP). If you are planning to configure APs as RADIUS clients by group in NPS, each AP in the group must have an IP address from the same IP address range.
