...
ms.date: 01/28/2026
#customer intent: As a network administrator, I want to enable DNS over HTTPS on Windows DNS Server so that I can encrypt DNS traffic between clients and the server.
---
...
This article explains how to enable and configure DNS over HTTPS (DoH) on Windows DNS Server. You'll import a certificate, bind it to the DNS Server, enable the DoH service, and verify the configuration.

## Prerequisites

Before you begin, ensure you have:

- Windows Server 2025 with the 2026-02 Security Update [KB000000](https://support.microsoft.com/help/000000) or later installed
...
- To enable DNS over HTTPS on the DNS Server service, a registry key obtained through an opt-in process. Request access by using [DoH on Windows DNS Server: Public Preview Registration](https://aka.ms/doh-preview), follow the instructions provided before continuing.
...
1. Place the certificate's `.pfx` file (containing both certificate and private key) on the server hosting Windows DNS Server.
...
1. Generate a new GUID by running the following command:

   ```powershell
   New-Guid
   ```

   Save the output GUID for use in a later step.

1. List available certificates and their thumbprints by running the following command:

   ```powershell
   Get-ChildItem -Path Cert:\LocalMachine\My | Select-Object Subject, Thumbprint, NotAfter
   ```

1. Locate your server certificate in the output and save its thumbprint.

1. Bind the certificate to the server port by running the following command. Replace `<certificate-thumbprint>` with your certificate thumbprint and `<guid>` with the GUID you generated:

   ```powershell
   netsh http add sslcert ipport=0.0.0.0:443 certhash=<certificate-thumbprint> appid={<guid>}
   ```

> [!TIP]
> To have Windows DNS Server service DoH traffic on a specific IP address instead of all addresses, replace `0.0.0.0` with your desired IP address. The IP address must resolve to the SAN in your certificate. You can also replace `443` with a different port number.
...
After binding the certificate, enable the DoH service on your Windows DNS Server.
...
1. Verify the DNS query resolves successfully over the encrypted connection.

## Next steps

> [!div class="nextstepaction"]
> [Monitor DNS over HTTPS performance and events](monitor-dns-over-https.md)