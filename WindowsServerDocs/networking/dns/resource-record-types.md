...
| Service location (SRV) records | Specifies the host, port and protocol for a service, such as Active Directory domain controllers. Service location (SRV) resource records are required when clients use DNS to locate location services such as Active Directory domain controllers. |
