...
> [!NOTE]  
> Windows Server 2019 and Windows 10 1809 apply cumulative updates from KB5006744 onwards, so they use the formula for Windows Server 2016 and later versions.
>
...
|**ChainEntryTimeout** | |Specifies the maximum number of seconds that an entry can remain in the chaining table before the entry is considered to be expired. Expired entries can be removed when the next request or response is processed. The default value is **16**. |
