...
- **Software virtual Receive Side Scaling \(vRSS\)**. Software vRSS is used to spread incoming traffic destined for a VM across multiple logical processors \(LPs\) of the VM.  This enables the VM to handle more networking traffic than a single LP would be able to handle. For more information, see [Virtual Receive-side Scaling in Windows Server 2012 R2](https://technet.microsoft.com/library/dn383582.aspx).
