...
SMB in [!INCLUDE[win8_server_2](includes/win8_server_2_md.md)] includes the new SMB 3.0 protocol and many new improvements which are described in the following table.
...
-   A failover cluster running [!INCLUDE[win8_server_2](includes/win8_server_2_md.md)] with at least two nodes configured. The cluster must pass the cluster validation tests included in the validation wizard.
...
-   Client computers must be running [!INCLUDE[win8_client_1](includes/win8_client_1_md.md)] or [!INCLUDE[win8_server_2](includes/win8_server_2_md.md)], both of which include the updated SMB client that supports continuous availability.
...
-   At least two computers running [!INCLUDE[win8_server_2](includes/win8_server_2_md.md)] are required. No extra features need to be installed—the technology is on by default.
