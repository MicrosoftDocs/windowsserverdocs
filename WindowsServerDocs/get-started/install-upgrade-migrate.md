...
- **Version gap** — Beginning with Windows Server 2025, nonclustered systems can skip up to four versions. Earlier versions support skipping up to two. Cluster rolling upgrades can only advance one version at a time. See the [supported paths table](#supported-in-place-upgrade-paths-by-version) for details.
...
|---|---|---|---|
| **In-place upgrade** | Installs a newer version of Windows Server over the existing one, keeping your settings, server roles, features, and data. | You want the fastest path to a newer version on the same hardware, and your roles and features [support in-place upgrade](upgrade-migrate-roles-features.md). | [Perform an in-place upgrade](perform-in-place-upgrade.md) |
