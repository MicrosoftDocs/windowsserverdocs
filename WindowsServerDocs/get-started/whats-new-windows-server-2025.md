...
- **Kerberos changes for supported encryption type configuration**: Kerberos will no longer honor the legacy registry key REG_DWORD `SupportedEncryptionTypes` found under `HKEY_LOCAL_MACHINE\CurrentControlSet\Control\Lsa\Kerberos\Parameters`. Microsoft recommends to leverage the group policy [Network security: Configure encryption types allowed for Kerberos
](https://learn.microsoft.com/en-us/previous-versions/windows/it-pro/windows-10/security/threat-protection/security-policy-settings/network-security-configure-encryption-types-allowed-for-kerberos) instead.
