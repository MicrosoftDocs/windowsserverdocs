...
| New EKUs for VBS enclaves | New enclaves signed with the new EKU are blocked from running on Windows Server 2022. This restriction applies to both newly created enclaves and existing enclaves that receive updates signed with the new EKU. Existing enclaves remain functional unless they are updated to use the new EKU. |
...
| New EKUs for VBS enclaves | New enclaves signed with the new EKU are blocked from running on Windows Server 2019. This restriction applies to both newly created enclaves and existing enclaves that receive updates signed with the new EKU. Existing enclaves remain functional unless they are updated to use the new EKU. |
