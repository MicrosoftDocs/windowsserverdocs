...
- The *Client Machine ID (CMID)*, which should be unique in every KMS client. As mentioned in [KMS host](#kms-host), a common issue related to count is if the count doesn't increase beyond **1** no matter how many KMS hosts or clients you activate in the environment. For more information, see [The KMS current count doesn't increase when you add new Windows Vista or Windows 7-based client computers to the network](https://support.microsoft.com/help/929829/the-kms-current-count-does-not-increase-when-you-add-new-windows-vista).
