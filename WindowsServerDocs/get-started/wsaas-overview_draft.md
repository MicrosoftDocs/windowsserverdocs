...
description: Starting with [RS3], Microsoft has streamlined servicing to make operating system updates simpler to test, manage, and deploy.
...
Starting with the [RS3] release of Windows Server [SOMETHING], we are changing the release model to support Windows Server as a service and align with similar release and servicing models for Windows 10 [LINK] and [Office 365 ProPlus](https://support.office.com/en-us/article/Overview-of-the-upcoming-changes-to-Office-365-ProPlus-update-management-78b33779-9356-4cdf-9d2c-08350ef05cca). If you've been working with Windows 10, these improvements might already be familiar to you, since the new release model for Windows Server is similar, though not identical. Microsoft has reimagined each part of the process to simplify the lives of IT pros and maintain a consistent Windows Server experience for its customers. If you've been working with Windows 10, these improvements might already be familiar to you. 

## What do these changes mean for me?

In summary, there are two key features to the new release plan:

- **Two updates a year**  Server Core installations of Windows Server will have semi-annual feature updates, as will Windows 10 and Office 365 ProPlus. These are targeted for March and September.

- **18 months of support**  We will extend the support period for these semi-annual feature updates from 12 to 18 months (starting from the initial release) so that IT professionals can choose to update once or twice a year.

### Release channels

#### Semi-annual Channel
Similar to the Current Branch for Business channel currently in use for Nano Server, products in the Semi-annual Channel will have updates available twice a year, in March and September. For [RS3], Server Core installations will be serviced by the Semi-annual Channel.

- **Semi-annual Channel (pilot)**: the first release of the update is the "pilot," intended for IT pilots, application validation, and end-user acceptance testing before broad deployment.
- **Semi-annual Channel (broad)**: two to three months later, the pilot release is declared as the "broad" release and the pilot release on all media sources is replaced by a current version that includes all security updates that have been issued since the pilot was released. The broad release is appropriate for broad deployment in production.

#### Long-term Servicing Branch
[What do we want to say about it? I think basically no changes?]
...
**Starting with [RS3]** Nano Server will be available only as a **container operating system**. You must run it as a container in a container host, such as a Server Core installation of [RS3]. You get it [HERE]; it has [THESE FEATURE CHANGES].

- Doesn't have [SOMETHING]
- Doesn't support [SOME OTHER THING]
- [ETC]

### Server Core installations of [RS3]
**Starting with [RS3]**, Server Core installations will be serviced on the new **Semi-annual Channel** model. Server Core will be available only as **volume-licensed** operating systems. You will also have to participate in Software Assurance [LINK].

### Server with Desktop Experience installations of [RS3]
Server with Desktop Experience installations will be serviced on the [Long Term Servicing Branch](https://support.microsoft.com/lifecycle#gp%2Fgp_msl_policy) model, comprising 5 years of mainstream support and 5 years of extended support.

--------------------------------------
BELOW HERE IS MATERIAL COPIED OUT OF THE WIN10 TOPIC AND THE CURRENT NANO TOPIC. I DON'T THINK WE WANT ALL (OR MAYBE EVEN MUCH) OF IT, BUT IT'S HERE SO YOU CAN POINT OUT THE BITS YOU THINK ARE MOST HELPFUL
...
Deploying Windows Server is simpler than with previous versions of Windows. When migrating from earlier versions of Windows, an easy in-place upgrade process [IS THIS GOING TO BE TRUE FOR SERVER THOUGH?] can be used to automatically preserve all apps, settings, and data. And once running Windows 10, deployment of Windows 10 feature updates will be equally simple.
...
New hardware is not needed for Windows Server 2016 version 1607. The minimum hardware requirements to run Windows Server version 1607 are the same as those required for Windows Server 2016. Most hardware drivers will continue to function in this release. 

-----------------------------------------------

## Servicing

Starting with Windows Server version RS3, there are two release types: feature updates that add new functionality twice each year, and quality updates that provide security and reliability fixes at least once a month. 

With Windows Server, organizations will need to change the way they approach deploying updates. Servicing branches are the first way to separate users into deployment groups for feature and quality updates. With the introduction of servicing branches comes the concept of a [deployment ring](waas-deployment-rings-windows-10-updates.md), which is simply a way to categorize the combination of a deployment group and a servicing branch to group devices for successive waves of deployment. For more information about developing a deployment strategy that leverages servicing branches and deployment rings, see [Plan servicing strategy for Windows 10 updates](waas-servicing-strategy-windows-10-updates.md). 
