...
Credential information is encrypted twice. The first encryption is based on the account' password. Credentials are encrypted again when they are sent across the Internet. The data that is stored is not available to other Microsoft or non-Microsoft services.
