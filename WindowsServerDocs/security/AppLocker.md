...
---
# AppLocker
This topic describes the controls on the AppLocker user interface to help you create and maintain application control policies by using AppLocker in [!INCLUDE[winblue_server_2](includes/winblue_server_2_md.md)] and [!INCLUDE[winblue_client_2](includes/winblue_client_2_md.md)].
...
The AppLocker user interface is accessed through the Microsoft Management Console \(MMC\), and it is organized into rule collections, which are Executable files, Scripts, Windows Installer files, [!INCLUDE[applocker_modernapps](includes/applocker_modernapps_md.md)], [!INCLUDE[applocker_pkgs](includes/applocker_pkgs_md.md)] and DLL files. These collections give the administrator an easy way to differentiate the rules for different types of applications. The following table lists the file formats that are included in each rule collection.
...
|Appx \([!INCLUDE[applocker_modernapps](includes/applocker_modernapps_md.md)] and [!INCLUDE[applocker_pkgs](includes/applocker_pkgs_md.md)]\)|.appx|
...
This condition identifies an application based on its digital signature and extended attributes when available. The digital signature contains information about the company that created the application \(the publisher\). Executable files, DLLs, Windows installers, [!INCLUDE[applocker_modernapps](includes/applocker_modernapps_md.md)], and [!INCLUDE[applocker_pkgs](includes/applocker_pkgs_md.md)] also have extended attributes, which are obtained from the binary resource. Attributes for executable files, DLLs, and Windows installers contain the name of the product that the file is a part of, the original name of the file as supplied by the publisher, and the version number of the file. In [!INCLUDE[applocker_modernapps](includes/applocker_modernapps_md.md)] and [!INCLUDE[applocker_pkgs](includes/applocker_pkgs_md.md)], these extended attributes contain the name and the version of the application package.

> [!NOTE]
> Rules that are created in the [!INCLUDE[applocker_modernapp](includes/applocker_modernapp_md.md)] and [!INCLUDE[applocker_pkg](includes/applocker_pkg_md.md)] rule collection can only have the Publisher condition because Windows does not support unsigned [!INCLUDE[applocker_modernapps](includes/applocker_modernapps_md.md)] or [!INCLUDE[applocker_pkgs](includes/applocker_pkgs_md.md)].
...
The [!INCLUDE[applocker_modernapp](includes/applocker_modernapp_md.md)] default rule types:

-   Allow members of the **Everyone** group to install and run all signed [!INCLUDE[applocker_modernapps](includes/applocker_modernapps_md.md)] and [!INCLUDE[applocker_pkgs](includes/applocker_pkgs_md.md)].
...
> [!IMPORTANT]
> If you want to allow any [!INCLUDE[applocker_modernapps](includes/applocker_modernapps_md.md)] in your environment while continuing to control executables, you should create default rules for [!INCLUDE[applocker_modernapps](includes/applocker_modernapps_md.md)] and set the enforcement mode to Audit\-only for the [!INCLUDE[applocker_modernapp](includes/applocker_modernapp_md.md)] rule collection.
...
1.  [!INCLUDE[winblue_start](includes/winblue_start_md.md)]**secpol.msc**, and then press ENTER.
...
2.  The Automatically Generate Rules Wizard allows you to create multiple rules at one time. You can select a folder and let the wizard create rules for the relevant files within that folder, or in case of [!INCLUDE[applocker_modernapps](includes/applocker_modernapps_md.md)], you can let the wizard create rules for all [!INCLUDE[applocker_modernapps](includes/applocker_modernapps_md.md)] that are installed on the computer. You can also specify the user or group to which to apply the rules. This wizard automatically generates Allow rules only.
...
-   AppLocker rules cannot be used to manage computers running a Windows operating system earlier than [!INCLUDE[nextref_server_7](includes/nextref_server_7_md.md)] or [!INCLUDE[nextref_client_7](includes/nextref_client_7_md.md)]. Software Restriction Policies must be used instead. If AppLocker rules are defined in a Group Policy Object \(GPO\), only those rules are applied. To ensure interoperability between Software Restriction Policies rules and AppLocker rules, define Software Restriction Policies rules and AppLocker rules in different GPOs.

-   The [!INCLUDE[applocker_modernapp](includes/applocker_modernapp_md.md)] and [!INCLUDE[applocker_pkg](includes/applocker_pkg_md.md)] rule collection is not available in [!INCLUDE[nextref_server_7](includes/nextref_server_7_md.md)] or [!INCLUDE[nextref_client_7](includes/nextref_client_7_md.md)].

-   When the rules for the executable rule  collection are enforced and the [!INCLUDE[applocker_modernapp](includes/applocker_modernapp_md.md)] and [!INCLUDE[applocker_pkg](includes/applocker_pkg_md.md)] rule collection does not contain any rules, no [!INCLUDE[applocker_modernapps](includes/applocker_modernapps_md.md)] or [!INCLUDE[applocker_pkgs](includes/applocker_pkgs_md.md)] are allowed to run. To allow [!INCLUDE[applocker_modernapps](includes/applocker_modernapps_md.md)] or [!INCLUDE[applocker_pkgs](includes/applocker_pkgs_md.md)], you must create rules for the [!INCLUDE[applocker_modernapp](includes/applocker_modernapp_md.md)] and [!INCLUDE[applocker_pkg](includes/applocker_pkg_md.md)] rule collection.
...
|Resource|Windows Server 2008 R2 and Windows 7|[!INCLUDE[winblue_server_2](includes/winblue_server_2_md.md)], [!INCLUDE[win8_server_2](includes/win8_server_2_md.md)], [!INCLUDE[winblue_client_2](includes/winblue_client_2_md.md)] and [!INCLUDE[win8_client_2](includes/win8_client_2_md.md)]|
|------------|----------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
|Procedures|[AppLocker Operations Guide](http://technet.microsoft.com/library/ee791916(WS.10).aspx)|[Administer AppLocker](Administer-AppLocker.md)<br /><br />[Manage Packaged Apps with AppLocker](Manage-Packaged-Apps-with-AppLocker.md)|
