...
The Kerberos Key Distribution Center \(KDC\) is integrated with other Windows Server??security services that run on the domain controller. The KDC uses the domain' Active Directory Domain Services database as its security account database. Active Directory Domain Services is required for default Kerberos implementations within the domain or forest.
