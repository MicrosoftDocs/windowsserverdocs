...
The [!INCLUDE[wfas](includes/wfas_md.md)] Administration with [!INCLUDE[wps_2](includes/wps_2_md.md)] Guide provides essential scriptlets for automating [!INCLUDE[wfas](includes/wfas_md.md)] management in [!INCLUDE[win8_server_2](includes/win8_server_2_md.md)]. It is designed for IT pros, system administrators, IT managers, and others who use and need to automate [!INCLUDE[wfas](includes/wfas_md.md)] management in Windows.

In [!INCLUDE[win8_server_2](includes/win8_server_2_md.md)] and [!INCLUDE[win8_client_2](includes/win8_client_2_md.md)], administrators can use [!INCLUDE[wps_2](includes/wps_2_md.md)] to manage their firewall and IPsec deployments. This object\-oriented scripting environment will make it easier for administrators to manage policies and monitor network conditions than was possible in Netsh. [!INCLUDE[wps_2](includes/wps_2_md.md)] allows network settings to be self\-discoverable through the syntax and parameters in each of the cmdlets. This guide demonstrates how common tasks were performed in Netsh and how you can use [!INCLUDE[wps_2](includes/wps_2_md.md)] to accomplish them.

> [!IMPORTANT]
> The netsh commands for [!INCLUDE[wfas](includes/wfas_md.md)] have not changed since the previous operating system version. The netsh commands for [!INCLUDE[wfas](includes/wfas_md.md)] in [!INCLUDE[win8_server_2](includes/win8_server_2_md.md)] are identical to the commands that are provided in [!INCLUDE[firstref_server_7](includes/firstref_server_7_md.md)].

In future versions of Windows, Microsoft might remove the netsh functionality for [!INCLUDE[wfas](includes/wfas_md.md)]. Microsoft recommends that you transition to [!INCLUDE[wps_2](includes/wps_2_md.md)] if you currently use netsh to configure and manage [!INCLUDE[wfas](includes/wfas_md.md)].

[!INCLUDE[wps_2](includes/wps_2_md.md)] and netsh command references are at the following locations.

-   [Netsh Commands for Windows Firewall with Advanced Security](http://technet.microsoft.com/library/cc771920(v=ws.10))

## Scope
This guide does not teach you the fundamentals of [!INCLUDE[wfas](includes/wfas_md.md)], which can be found in [Windows Firewall with Advanced Security Overview](Windows-Firewall-with-Advanced-Security-Overview.md). It does not teach the fundamentals of [!INCLUDE[wps_2](includes/wps_2_md.md)], and it assumes that you are familiar with the [!INCLUDE[wps_2](includes/wps_2_md.md)] language and the basic concepts of [!INCLUDE[wps_2](includes/wps_2_md.md)]. For more information about [!INCLUDE[wps_2](includes/wps_2_md.md)] concepts and usage, see the reference topics in the [Additional resources](Windows-Firewall-with-Advanced-Security-Administration-with-Windows-PowerShell.md#BKMK_AdditionalResources) section of this guide.

## Audience and user requirements
This guide is intended for IT pros, system administrators, and IT managers, and it assumes that you are familiar with [!INCLUDE[wfas](includes/wfas_md.md)], the [!INCLUDE[wps_2](includes/wps_2_md.md)] language, and the basic concepts of [!INCLUDE[wps_2](includes/wps_2_md.md)].
...
-   [!INCLUDE[wps_2](includes/wps_2_md.md)] 3.0 \(included in [!INCLUDE[win8_server_2](includes/win8_server_2_md.md)]\)

-   Windows NetSecurity Module for [!INCLUDE[wps_2](includes/wps_2_md.md)] \(included in [!INCLUDE[win8_server_2](includes/win8_server_2_md.md)]\)

-   [!INCLUDE[ise_2](includes/ise_2_md.md)] \(optional feature in [!INCLUDE[wps_2](includes/wps_2_md.md)] 3.0, which is installed by using [!INCLUDE[sm](includes/sm_md.md)]\)

> [!NOTE]
> In [!INCLUDE[wps_2](includes/wps_2_md.md)] 3.0, modules are imported automatically when you get or use any cmdlet in the module. You can still use the **Import\-Module** cmdlet to import a module.
> 
> Use **Import\-Module** if you are using [!INCLUDE[wps_2](includes/wps_2_md.md)] 2.0, or if you need to use a feature of the module before you use any of its cmdlets. For more information, see [Import\-Module](http://go.microsoft.com/fwlink/?LinkID=141553).
> 
> Use **Import\-PSSnapIn** to use cmdlets in a [!INCLUDE[wps_2](includes/wps_2_md.md)] snap\-in, regardless of the version of [!INCLUDE[wps_2](includes/wps_2_md.md)] that you are running.
...
|[Additional resources](Windows-Firewall-with-Advanced-Security-Administration-with-Windows-PowerShell.md#BKMK_AdditionalResources)|More information about [!INCLUDE[wps_2](includes/wps_2_md.md)]|

## <a name="BKMK_ProfileGlobalDefaults"></a>Set profile global defaults
Global defaults set the system behavior in a per profile basis. [!INCLUDE[wfas](includes/wfas_md.md)] supports Domain, Private, and Public profiles.
...
The global default settings can be defined through the command\-line interface. These modifications are also available through the [!INCLUDE[wfas](includes/wfas_md.md)] MMC snap\-in.
...
Adding a firewall rule in [!INCLUDE[wps_2](includes/wps_2_md.md)] looks a lot like it did in Netsh, but the parameters and values are specified differently.
...
### GPO Caching
To reduce the burden on busy domain controllers, [!INCLUDE[wps_2](includes/wps_2_md.md)] allows you to load a GPO to your local session, make all your changes in that session, and then save it back at all once.
...
Note that this does not batch your individual changes, it loads and saves the entire GPO at once. So if any other changes are made by other administrators, or in a different [!INCLUDE[wps_2](includes/wps_2_md.md)] window, saving the GPO overwrites those changes.

### Modify an existing firewall rule
When a rule is created, Netsh and [!INCLUDE[wps_2](includes/wps_2_md.md)] allow the administrator to change rule properties and influence, but the rule maintains its unique identifier \(in [!INCLUDE[wps_2](includes/wps_2_md.md)] this is specified with the *\-Name* parameter\).
...
Netsh requires you to provide the name of the rule for it to be changed and we do not have an alternate way of getting the firewall rule. In [!INCLUDE[wps_2](includes/wps_2_md.md)], you can query for the rule using its known properties.
...
You can change the remote endpoint of the **Allow Web 80** rule \(as done previously\) using filter objects. Using [!INCLUDE[wps_2](includes/wps_2_md.md)] you query by port using the port filter, then assuming additional rules exist affecting the local port, you build with further queries until your desired rule is retrieved.
...
In the following example, we add both inbound and outbound Telnet firewall rules to the group **Telnet Management**.  In [!INCLUDE[wps_2](includes/wps_2_md.md)], group membership is specified when the rules are first created so we re\-create the previous example rules.  Adding rules to a custom rule group is not possible in Netsh.
...
If the group is not specified at rule creation time, the rule can be added to the rule group using dot notation in [!INCLUDE[wps_2](includes/wps_2_md.md)]. You cannot specify the group using `Set-NetFirewallRule` since the command allows querying by rule group.
...
Rule objects can be disabled so that they are no longer active. In [!INCLUDE[wps_2](includes/wps_2_md.md)], the **Disable\-NetFirewallRule** cmdlet will leave the rule on the system, but put it in a disabled state so the rule no longer is applied and impacts traffic. A disabled firewall rule can be re\-enabled by **Enable\-NetFirewallRule**. This is different from the **Remove\-NetFirewallRule**, which permanently removes the rule definition from the system.
...
Remote management using WinRM is enabled by default on [!INCLUDE[win8_server_2](includes/win8_server_2_md.md)]. The cmdlets that support the *CimSession* parameter use WinRM and can be managed remotely by default. This is important because the default and recommended installation mode for [!INCLUDE[win8_server_2](includes/win8_server_2_md.md)] is Server Core which does not include a graphical user interface.
...
[!INCLUDE[wps_2](includes/wps_2_md.md)] can create powerful, complex IPsec policies like in Netsh and the [!INCLUDE[wfas](includes/wfas_md.md)] MMC snap\-in. However, because [!INCLUDE[wps_2](includes/wps_2_md.md)] is object\-based rather than string token\-based, configuration in [!INCLUDE[wps_2](includes/wps_2_md.md)] offers greater control and flexibility.

In Netsh, the authentication and cryptographic sets were specified as a list of comma\-separated tokens in a specific format. In [!INCLUDE[wps_2](includes/wps_2_md.md)], rather than using default settings, you first create your desired authentication or cryptographic proposal objects and bundle them into lists in your preferred order. Then, you create one or more IPsec rules that reference these sets. The benefit of this model is that programmatic access to the information in the rules is much easier. See the following sections for clarifying examples.

![](media/CreateIPsecRule.gif)
...
You can leverage IKEv2 capabilities in [!INCLUDE[win8_server_2](includes/win8_server_2_md.md)] by simply specifying IKEv2 as the key module in an IPsec rule. This can only be done using computer certificate authentication and cannot be used with phase 2 authentication.
...
Copying individual rules is a task that is not possible through the Netsh interface. Here is how you can accomplish it with [!INCLUDE[wps_2](includes/wps_2_md.md)].
...
### Handling [!INCLUDE[wps_2](includes/wps_2_md.md)] errors

To handle errors in your [!INCLUDE[wps_2](includes/wps_2_md.md)] scripts, you can use the *–ErrorAction* parameter.  This is especially useful with the **Remove** cmdlets.  If you want to remove a particular rule, you will notice that it fails if the rule is not found. When removing rules, if the rule isn’t already there, it is generally acceptable to ignore that error. In this case, you can do the following to suppress any “rule not found” errors during the remove operation.
...
### Monitor
The following [!INCLUDE[wps_2](includes/wps_2_md.md)] commands are useful in the update cycle of a deployment phase.
...
In situations where only secure traffic can be allowed through the Windows Firewall, a combination of manually configured firewall and IPsec rules are necessary. The firewall rules determine the level of security for allowed packets, and the underlying IPsec rules secure the traffic. The scenarios can be accomplished in [!INCLUDE[wps_2](includes/wps_2_md.md)] and in Netsh, with many similarities in deployment.
...
For more information about [!INCLUDE[wps_2](includes/wps_2_md.md)] concepts, see the following topics.
