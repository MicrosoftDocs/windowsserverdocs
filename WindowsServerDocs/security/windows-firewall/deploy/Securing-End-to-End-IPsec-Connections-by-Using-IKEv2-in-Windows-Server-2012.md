...
In [!INCLUDE[win8_server_2](includes/win8_server_2_md.md)], Internet Key Exchange version 2 \(IKEv2\) support is broadened from previous Windows versions.

For example, in [!INCLUDE[win8_server_2](includes/win8_server_2_md.md)], IKEv2 does the following:
...
-   Uses the [!INCLUDE[wps_2](includes/wps_2_md.md)] interface exclusively for configuration. You cannot configure IKEv2 through the user interface.

-   Uses certificates for the authentication mechanism

In [!INCLUDE[nextref_server_7](includes/nextref_server_7_md.md)], IKEv2 is available as a virtual private network \(VPN\) tunneling protocol that supports automatic VPN reconnection. IKEv2 allows the security association to remain unchanged despite changes in the underlying connection.
...
The following [!INCLUDE[wps_2](includes/wps_2_md.md)] script establishes a connection security rule that uses IKEv2 for communication between two computers \(CLIENT1 and SERVER1\) that are joined to the corp.contoso.com domain as shown in Figure 1.
...
![](media/PowerShellLogoSmall.gif)**Windows PowerShell commands**
...
Use a [!INCLUDE[wps_2](includes/wps_2_md.md)] script similar to the following to create a local IPsec policy on the computers that you want to include in the secure connection.
...
1.  [!INCLUDE[clickstart](includes/clickstart_md.md)]**wf.msc**, and then press ENTER.
...
#### Use [!INCLUDE[wps_2](includes/wps_2_md.md)] cmdlets to display the security associations.

1.  Open a [!INCLUDE[wps_2](includes/wps_2_md.md)] command prompt.
