...
To configure Windows Firewall to suppress the display of a notification when it blocks a program that tries to listen for network traffic and to prohibit locally defined rules, use the [!INCLUDE[wfas](includes/wfas_md.md)] node  in the Group Policy Management MMC snap\-in.
