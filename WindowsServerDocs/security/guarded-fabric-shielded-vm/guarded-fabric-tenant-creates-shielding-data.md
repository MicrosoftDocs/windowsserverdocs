...
Also, note that the networking-related substitution strings towards the end of the table are only used if you are leveraging VMM Static IP Address Pools. For more information about static IP addresses in VMM templates see [Static IP Address Pools](https://technet.microsoft.com/library/jj721568.aspx#BKMK_StaticIPAddressPools) in the VMM documentation. Your hosting service provider should be able to tell you if these substitution strings are required.
