...
The output of [Get-HgsClientConfiguration](/powershell/module/hgsclient/get-hgsclientconfiguration) on that host will show you information about why that host failed attestation.
...
NotConfigured             | The host is not configured to use a HGS for attestation and key protection. It is configured for local mode, instead. If this host is in a guarded fabric, use [Set-HgsClientConfiguration](/powershell/module/hgsclient/set-hgsclientconfiguration) to provide it with the URLs for your HGS server.
