...
ms.tgt_pltfrm: na
ms.topic: article
ms.assetid: 87791a4e-f649-402c-bea5-65310b59839b
...
-   [Credential provider architecture](#BKMK_CredProviderArch)

-   [Smart card subsystem architecture](#BKMK_SubsystemArchitecture)

For more information, see [Windows Authentication Architecture](../windows-authentication/Windows-Authentication-Architecture.md). This topic in the Windows Authentication Technical Overview explains the basic architectural scheme for Windows authentication for past and current versions of Windows.

## <a name="BKMK_CredProviderArch"></a>Credential provider architecture
...
![Chart showing the architecture for credential providers in the Windows operating system](../media/Smart-Card-Architecture/WindowsSmartCardTechnicalReference_01.gif)
...
## <a name="BKMK_SubsystemArchitecture"></a>Smart card subsystem architecture
Vendors provide smart cards and smart card readers, and in many cases the vendors are different for the smart card and the smart card reader. Drivers for smart card readers are written to the Personal Computer/Smart Card (PC/SC) standard version 1.0. Each smart card must have a Credential Service Provider (CSP) that uses the CryptoAPI interfaces to enable cryptographic operations, and the WinSCard APIs to enable communications with smart card hardware.
...
![Figure showing the relationship between the CryptoAPI, CSPs, the Smart Card Base Cryptographic Service Provider (Base CSP), and smart card minidrivers](../media/Smart-Card-Architecture/WindowsSmartCardTechnicalReference_03.gif)
...
-   [Data caching](#BKMK_DataCaching)
    ...
    The data cache provides for a single process to minimize smart card I/O operations.

-   [PIN caching](#BKMK_PinCache)

    The PIN cache helps the user from having to reenter a PIN each time the smart card is unauthenticated.

#### <a name="BKMK_DataCaching"></a>Data caching
...
Three types of objects or data are cached by the CSP: pins (for more information, see [PIN caching](#BKMK_PinCache)), certificates, and files. If any of the cached data changes, the corresponding object is read from the smart card in successive operations. For example, if a file is written to the smart card, the CSP cache becomes out-of-date for the files, and other processes read the smart card at least once to refresh their CSP cache.
...
-   [Container specification levels](#BKMK_ContainerSpecificationLevels)

-   [Container operations](#BKMK_ContainerOperations)

-   [Context flags](#BKMK_ContextFlags)

-   [Create a new container in silent context](#BKMK_CreatingaNewContainerInSilentContext)

-   [Smart card selection behavior](#BKMK_SmartCardSelectionBehavior)

-   [Make a smart card reader match](#BKMK_MakingaReaderMatch)

-   [Make a smart card match](#BKMK_MakingaSmartCardMatch)

-   [Open an existing default container (no reader specified)](#BKMK_OpenAnExistingDefaultContainerNoReaderSpecified)

-   [Open an existing GUID-named container (no reader specified)](#BKMK_OpenAnExistingGUIDnamedContainerNoReaderSpecified)

-   [Create a new container (no reader specified)](#BKMK_CreateaNewContainerNoReaderSpecified)

-   [Delete a container](#BKMK_DeleteaContainer)

#### <a name="BKMK_ContainerSpecificationLevels"></a>Container specification levels
...
#### <a name="BKMK_ContainerOperations"></a>Container operations
...
#### <a name="BKMK_ContextFlags"></a>Context flags
...
#### <a name="BKMK_CreatingaNewContainerInSilentContext"></a>Create a new container in silent context
...
#### <a name="BKMK_SmartCardSelectionBehavior"></a>Smart card selection behavior
...
![Flowchart showing the selection steps performed by the Windows operating system](../media/Smart-Card-Architecture/WindowsSmartCardTechnicalReference_04.PNG)
...
#### <a name="BKMK_MakingaReaderMatch"></a>Make a smart card reader match
...
#### <a name="BKMK_MakingaSmartCardMatch"></a>Make a smart card match
...
#### <a name="BKMK_OpenAnExistingDefaultContainerNoReaderSpecified"></a>Open an existing default container (no reader specified)
...
#### <a name="BKMK_OpenAnExistingGUIDnamedContainerNoReaderSpecified"></a>Open an existing GUID-named container (no reader specified)
...
#### <a name="BKMK_CreateaNewContainerNoReaderSpecified"></a>Create a new container (no reader specified)
...
#### <a name="BKMK_DeleteaContainer"></a>Delete a container
...
![Chart showing the Cryptography architecture that is used by the Windows operating system](../media/Smart-Card-Architecture/WindowsSmartCardTechnicalReference_05.gif)
...
For more information about how to write a smart card minidriver, CSP, or KSP, see [Enterprise Smart Card Deployment in the Microsoft Windows Smart Card Framework](http://go.microsoft.com/fwlink/?LinkId=93349).
