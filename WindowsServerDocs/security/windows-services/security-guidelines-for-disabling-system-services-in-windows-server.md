...
| **Comments** | On a domain controller, the installation of the DC role adds a thread to the spooler service that is responsible for performing print pruning – removing the stale print queue objects from the Active Directory. If the spooler service is not running on at least one DC in each site, then the AD has no means to remove old queues that no longer exist. [Ask the performance team blog](https://techcommunity.microsoft.com/t5/ask-the-performance-team/printing-and-active-directory/ba-p/373900). |
