...
|--------------------------------|-------|-------------|
|Scope|SRP policies can be applied to all Windows operating systems beginning with Windows XP and Windows Server 2003.|AppLocker policies apply only to the support versions of Windows listed in [Requirements to Use AppLocker](../get-started/Requirements-to-Use-AppLocker.md).|
...
|Enforcement mode|SRP works in the â€œdeny list modeâ€? where administrators can create rules for files that they do not want to allow in this Enterprise whereas the rest of the file are allowed to run by default.<br /><br />SRP can also be configured in the â€œallow list modeâ€? such that the by default all files are blocked and administrators need to create allow rules for files that they want to allow.|AppLocker by default works in the â€œallow list modeâ€? where only those files are allowed to run for which there is a matching allow rule.|
...
|Support for rule exceptions|SRP does not support rule exceptions|AppLocker rules can have exceptions which allow administrators to create rules such as â€œAllow everything from Windows except for Regedit.exeâ€?.|
...
For information about the capabilities of Software Restriction Policies, see [Software Restriction Policies Technical Overview](../../software-restriction-policies/Software-Restriction-Policies-Technical-Overview.md).
