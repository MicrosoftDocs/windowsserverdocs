...
For more information about Microsoft' implementation of TLS and SSL in the Schannel SSP, see the [TLS/SSL Technical Reference (2003)](http://technet.microsoft.com/library/cc784149(v=ws.10).aspx).
...
There are three Client Authentication Trust Modes supported by the Schannel provider. The trust mode controls how validation of the client' certificate chain is performed and is a system-wide setting controlled by the REG_DWORD ???ClientAuthTrustMode??? under HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\SecurityProviders\Schannel.
