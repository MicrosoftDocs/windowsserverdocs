...
This architectural approach builds on the protections found in the [!INCLUDE[winthreshold_client_1](includes/winthreshold_client_1_md.md)][Credential Guard](https://technet.microsoft.com/library/mt483740%28v=vs.85%29.aspx) and [Device Guard](https://technet.microsoft.com/library/dn986865(v=vs.85).aspx) features and goes beyond those protections for sensitive accounts and tasks.  
...
> [!NOTE]  
> Organizations without access to [!INCLUDE[winthreshold_client_2](includes/winthreshold_client_2_md.md)] Enterprise can use [!INCLUDE[winthreshold_client_2](includes/winthreshold_client_2_md.md)] Pro, which includes many of the critical foundational technologies for PAWs, including Trusted Boot, BitLocker, and Remote Desktop.  Education customers can use [!INCLUDE[winthreshold_client_2](includes/winthreshold_client_2_md.md)] Education.  [!INCLUDE[winthreshold_client_2](includes/winthreshold_client_2_md.md)] Home should not be used for a PAW.  
>   
> For a comparison matrix of the different editions of [!INCLUDE[winthreshold_client_2](includes/winthreshold_client_2_md.md)], read [this article](https://www.microsoft.com/WindowsForBusiness/Compare).  
...
-   **Admin OS** \- The physical host runs [!INCLUDE[winthreshold_client_2](includes/winthreshold_client_2_md.md)] on the PAW host for Administrative tasks  
  
-   **User OS** \- A [!INCLUDE[winthreshold_client_2](includes/winthreshold_client_2_md.md)] client [!INCLUDE[nextref_virtualname](includes/nextref_virtualname_md.md)] virtual machine guest runs a corporate image  
  
With [!INCLUDE[winthreshold_client_2](includes/winthreshold_client_2_md.md)][!INCLUDE[nextref_virtualname](includes/nextref_virtualname_md.md)], a guest virtual machine \(also running [!INCLUDE[winthreshold_client_2](includes/winthreshold_client_2_md.md)]\) can have a rich user experience including sound, video, and Internet communications applications such as  Skype for Business.  
  
In this configuration, daily work that does not require administrative privileges is done in the user OS virtual machine which has a regular corporate [!INCLUDE[winthreshold_client_2](includes/winthreshold_client_2_md.md)] image and is not subject to restrictions applied to the PAW host. All administrative work is done on the Admin OS.  
  
To configure this, follow the instructions in this guidance for the PAW host, add Client [!INCLUDE[nextref_virtualname](includes/nextref_virtualname_md.md)] features, create a User VM, and then install a [!INCLUDE[winthreshold_client_2](includes/winthreshold_client_2_md.md)] corporate image on the User VM.  
...
1.  Complete the Prerequisites  
    ...
    4.  Acquire and validate the required Windows 10 Enterprise Edition and application software. Obtain the software required for PAW and validate it using the guidance in [Clean Source for installation media](http://aka.ms/cleansource).  
        ...
        -   [!INCLUDE[winthreshold_client_2](includes/winthreshold_client_2_md.md)] Enterprise Edition  
  
        -   [Remote Server Administration Tools](http://www.microsoft.com/en-us/download/details.aspx?id=45520.) for [!INCLUDE[winthreshold_client_2](includes/winthreshold_client_2_md.md)]  
...
8.  Deploy your PAW\(s\)  
    ...
    1.  Install [!INCLUDE[winthreshold_client_2](includes/winthreshold_client_2_md.md)] using the clean source installation media that you obtained earlier.  
    ...
    3.  Install Remote Server Administration Tools for [!INCLUDE[winthreshold_client_2](includes/winthreshold_client_2_md.md)] using the clean source installation media.  
...
1.  **\(Recommended\)** Enable **RestrictedAdmin** mode – Enable this feature on your existing servers and workstations, then enforce the use of this feature. This feature will require the target servers to be running [!INCLUDE[nextref_server_7](includes/nextref_server_7_md.md)] or later and target workstations to be running [!INCLUDE[nextref_client_7](includes/nextref_client_7_md.md)] or later.  
...
5.  **\(Optional\)** Enable Connectivity to Cloud Services. This step allows management of cloud services like Azure and Office 365 with appropriate security assurances. This step is also required for Microsoft Intune to manage the PAWs.  
    ...
    2.  Apply [!INCLUDE[winthreshold_client_2](includes/winthreshold_client_2_md.md)] Security baselines and Cloud Service Access Link the security baselines for Windows and for cloud service access \(if required\) to the correct OUs using the steps below:  
  
        1.  Extract the contents of the [!INCLUDE[winthreshold_client_2](includes/winthreshold_client_2_md.md)] Security Baselines ZIP file.  
        ...
        2.  Create these GPOs, [import the policy](https://technet.microsoft.com/library/cc753786.aspx) settings, and [link](https://technet.microsoft.com/library/cc732979.aspx) per this table. Link each policy to each location and ensure the order follows the table \(lower entries in table should be applied later and higher priority\):  
            ...
            **Policies:**  
            ...
            |||  
            |-|-|  
            |SCM [!INCLUDE[winthreshold_client_2](includes/winthreshold_client_2_md.md)] \- Domain Security|N\/A \- Do Not Link Now|  
            |SCM [!INCLUDE[winthreshold_client_2](includes/winthreshold_client_2_md.md)] TH2 \- Computer|Admin\\Tier 0\\Devices|  
            ||Admin\\Tier 1\\Devices|  
            ||Admin\\Tier 2\\Devices|  
            |SCM [!INCLUDE[winthreshold_client_2](includes/winthreshold_client_2_md.md)] TH2\- BitLocker|Admin\\Tier 0\\Devices|  
            ||Admin\\Tier 1\\Devices|  
            ||Admin\\Tier 2\\Devices|  
            |SCM [!INCLUDE[winthreshold_client_2](includes/winthreshold_client_2_md.md)] \- Credential Guard|Admin\\Tier 0\\Devices|  
            ||Admin\\Tier 1\\Devices|  
            ||Admin\\Tier 2\\Devices|  
            ...
            ||Admin\\Tier 1\\Devices|  
            ||Admin\\Tier 2\\Devices|  
            |SCM [!INCLUDE[winthreshold_client_2](includes/winthreshold_client_2_md.md)] \- User|Admin\\Tier 0\\Accounts|  
            ...
            > [!NOTE]  
            > The “SCM [!INCLUDE[winthreshold_client_2](includes/winthreshold_client_2_md.md)] \- Domain Security” GPO may be linked to the domain independently of PAW, but will affect the entire domain.  
