...
2.  [Create security groups for Windows 7 DirectAccess clients on DC1](assetId:///205ca795-93ce-4e53-aa6b-b44c87f0e14a#SecGroup)—When DirectAccess is configured, it automatically creates [!INCLUDE[gpos](includes/gpos_md.md)] \(GPOs\) and GPO settings that are applied to DirectAccess clients and servers. The DirectAccess client GPO is applied to specific Active Directory security groups.  
...
Create the DirectAccess security groups for [!INCLUDE[firstref_client_7](includes/firstref_client_7_md.md)] with the following procedure.  
  
[!INCLUDE[nextref_client_7](includes/nextref_client_7_md.md)] client computers must be members of separate security groups because they are able to connect to internal resources through a single entry point only. When enabling Multisite support or adding entry points, if [!INCLUDE[nextref_client_7](includes/nextref_client_7_md.md)] support is requested, then a separate GPO will be automatically created by DirectAccess for [!INCLUDE[nextref_client_7](includes/nextref_client_7_md.md)] clients for each entry point.  
...
3.  On the **New Object \- Group** dialog box, under **Group name**, enter **Win7\_Clients\_Site1**.  
  
4.  Under **Group scope**, click **Global**, under **Group type**, click **Security**, and then click **OK**.  
  
5.  Double\-click the **Win7\_Clients\_Site1** security group, and on the **Win7\_Clients\_Site1 Properties** dialog box, click the **Members** tab.  
...
8.  In **Enter the object names to select**, type **client2**, and then click **OK**, and then on the **Win7\_Clients\_Site1 Properties** dialog box click **OK**.  
...
10. On the **New Object \- Group** dialog box, under **Group name**, enter **Win7\_Clients\_Site2**.  
...
## <a name="ADSite"></a>To add a new AD DS site  
  
1.  [!INCLUDE[clickstart](includes/clickstart_md.md)]**dssite.msc**, and then press ENTER.  
