...
2.  To install the Storage Replica feature, run the following cmdlet from a management computer:  
    ...
    Using the `Test-SRTopology` cmdlet with Nano Server in [!INCLUDE[winthreshold_server_2](includes/winthreshold_server_2_md.md)] requires remote script invocation with CredSSP. Unlike other Storage Replica cmdlets, `Test-SRTopology` requires running locally on the source server.   
...
Not in [!INCLUDE[winthreshold_server_2](includes/winthreshold_server_2_md.md)]. This release only supports one to one replication of a server, cluster, or stretch cluster node. This may change in a later release.  
  
## <a name="FAQ5"></a> Can I expand or shrink replicated volumes replicated by Storage Replica?  
  
Not in [!INCLUDE[winthreshold_server_2](includes/winthreshold_server_2_md.md)]. You must remove replication, resize volumes, and then add replication back to the volumes. This may change in a later release.  
  
## <a name="FAQ6"></a>Can I bring a destination volume online for read\-only access?  
Not in [!INCLUDE[winthreshold_server_2](includes/winthreshold_server_2_md.md)].   Storage Replica dismounts the destination volume and its drive letter or mount point when replication begins. This may change in a later release.  
  
## <a name="FAQ7"></a> Can I configure Scale\-out File Server \(SOFS\) in a stretch cluster?  
While technically possible, this is not a recommended configuration  in [!INCLUDE[winthreshold_server_2](includes/winthreshold_server_2_md.md)] due to the lack of site awareness in the compute nodes contacting the SOFS. If using campus-distance networking, where latencies are typically sub-millisecond, this configuration will probably work without issues.   
...
You can use the `Grant-SRDelegation` cmdlet in [!INCLUDE[winthreshold_server_2](includes/winthreshold_server_2_md.md)]. This allows you to set specific users in server to server, cluster to cluster, and stretch cluster replication scenarios as having the permissions to create, modify, or remove replication, without being a member of the local administrators group. For example:  
