...
ms.assetid: 15db311f-a3d8-40e7-b255-1d0e907d993b
author: jpjofre
---
# Install and Use Windows PowerShell Web Access
[!INCLUDE[pswa_1](includes/pswa_1_md.md)], first introduced in [!INCLUDE[win8_server_1](includes/win8_server_1_md.md)], acts as a [!INCLUDE[wps_2](includes/wps_2_md.md)] gateway, providing a web\-based [!INCLUDE[wps_2](includes/wps_2_md.md)] console that is targeted at a remote computer. It enables IT Pros to run [!INCLUDE[wps_2](includes/wps_2_md.md)] commands and scripts from a [!INCLUDE[wps_2](includes/wps_2_md.md)] console in a web browser, with no [!INCLUDE[wps_2](includes/wps_2_md.md)], remote management software, or browser plug\-in installation necessary on the client device. All that is required to run the web\-based [!INCLUDE[wps_2](includes/wps_2_md.md)] console is a properly\-configured [!INCLUDE[pswa_2](includes/pswa_2_md.md)] gateway, and a client device browser that supports JavaScript® and accepts cookies.  
...
After successful gateway setup and configuration, users can access a [!INCLUDE[wps_2](includes/wps_2_md.md)] console by using a web browser. When users open the secured [!INCLUDE[pswa_2](includes/pswa_2_md.md)] website, they can run a web\-based [!INCLUDE[wps_2](includes/wps_2_md.md)] console after successful authentication.  
  
[!INCLUDE[pswa_2](includes/pswa_2_md.md)] setup and configuration is a three\-step process:  
  
1.  Installing [!INCLUDE[pswa_2](includes/pswa_2_md.md)]  
  
2.  Configuring the gateway  
  
3.  Configuring authorization rules that allow users access to the web\-based [!INCLUDE[wps_2](includes/wps_2_md.md)] console  
  
Before you install and configure [!INCLUDE[pswa_2](includes/pswa_2_md.md)], we recommend that you read this entire guide, which includes instructions about how to install, secure, and uninstall [!INCLUDE[pswa_2](includes/pswa_2_md.md)]. The [Use the Web-based Windows PowerShell Console_1](Use-the-Web-based-Windows-PowerShell-Console_1.md) topic describes how users sign in to the web\-based console, and covers limitations and differences between the web\-based [!INCLUDE[wps_2](includes/wps_2_md.md)] console and the **powershell.exe** console. End users of the web\-based console should read [Use the Web-based Windows PowerShell Console_1](Use-the-Web-based-Windows-PowerShell-Console_1.md), but do not need to read the rest of this guide.  
  
This topic does not provide in\-depth Web Server \(IIS\) operations guidance; only those steps required to configure the [!INCLUDE[pswa_2](includes/pswa_2_md.md)] gateway are described in this topic. For more information about configuring and securing websites in IIS, see the IIS documentation resources in the See Also section.  
  
The following diagram shows how [!INCLUDE[pswa_2](includes/pswa_2_md.md)] works.  
  
![](media/wpwa_diagram.jpg)  
...
[!INCLUDE[pswa_2](includes/pswa_2_md.md)] in [!INCLUDE[winthreshold_server_1](includes/winthreshold_server_1_md.md)] requires Web Server \(IIS\), .NET Framework 4.6, and [!INCLUDE[wps_2](includes/wps_2_md.md)] 3.0 or [!INCLUDE[wps_2](includes/wps_2_md.md)] 4.0 to be running on the server on which you want to run the gateway. You can install [!INCLUDE[pswa_2](includes/pswa_2_md.md)] on a server that is running [!INCLUDE[winthreshold_server_2](includes/winthreshold_server_2_md.md)] by using either the [!INCLUDE[arfw](includes/arfw_md.md)] in [!INCLUDE[sm](includes/sm_md.md)], or [!INCLUDE[wps_2](includes/wps_2_md.md)] deployment cmdlets for [!INCLUDE[sm](includes/sm_md.md)]. When you install [!INCLUDE[pswa_2](includes/pswa_2_md.md)] by using [!INCLUDE[sm](includes/sm_md.md)] or its deployment cmdlets, required roles and features are automatically added as part of the installation process.  
  
[!INCLUDE[pswa_2](includes/pswa_2_md.md)] allows remote users to access computers in your organization by using [!INCLUDE[wps_2](includes/wps_2_md.md)] in a web browser. Although [!INCLUDE[pswa_2](includes/pswa_2_md.md)] is a convenient and powerful management tool, the web\-based access poses security risks, and should be configured as securely as possible. We recommend that administrators who configure the [!INCLUDE[pswa_2](includes/pswa_2_md.md)] gateway use available security layers, both the cmdlet\-based authorization rules included with [!INCLUDE[pswa_2](includes/pswa_2_md.md)], and security layers that are available in Web Server \(IIS\) and third\-party applications. This documentation includes both unsecure examples that are only recommended for test environments, as well as examples that are recommended for secure deployments.  
...
You can install the [!INCLUDE[pswa_2](includes/pswa_2_md.md)] gateway on a server that is running [!INCLUDE[winthreshold_server_2](includes/winthreshold_server_2_md.md)] by using either [!INCLUDE[wps_2](includes/wps_2_md.md)] cmdlets, or by using the [!INCLUDE[arfw](includes/arfw_md.md)] that is opened from within [!INCLUDE[sm](includes/sm_md.md)]. For quick installation and configuration, use [!INCLUDE[wps_2](includes/wps_2_md.md)] cmdlets, as described in this section.  
...
1.  Do one of the following to open a [!INCLUDE[wps_2](includes/wps_2_md.md)] session with elevated user rights.  
    ...
    > [!NOTE]  
    > In [!INCLUDE[wps_2](includes/wps_2_md.md)] 3.0 and later releases, there is no need to import the [!INCLUDE[sm](includes/sm_md.md)] cmdlet module into the [!INCLUDE[wps_2](includes/wps_2_md.md)] session before running cmdlets that are part of the module. A module is automatically imported the first time you run a cmdlet that is part of the module. Also, [!INCLUDE[wps_2](includes/wps_2_md.md)] cmdlets are not case\-sensitive.  
  
2.  Type the following, and then press **Enter**, where *computer\_name* represents a remote computer on which you want to install [!INCLUDE[pswa_2](includes/pswa_2_md.md)], if applicable. The `Restart` parameter automatically restarts destination servers if required.  
    ...
    > [!NOTE]  
    > Installing [!INCLUDE[pswa_2](includes/pswa_2_md.md)] by using [!INCLUDE[wps_2](includes/wps_2_md.md)] cmdlets does not add Web Server \(IIS\) management tools by default. If you want to install the management tools on the same server as the [!INCLUDE[pswa_2](includes/pswa_2_md.md)] gateway, add the `IncludeManagementTools` parameter to the installation command \(as provided in this step\). If you are managing the [!INCLUDE[pswa_2](includes/pswa_2_md.md)] website from a remote computer, install the IIS Manager snap\-in by installing [Remote Server Administration Tools for Windows 10 Technical Preview](http://go.microsoft.com/fwlink/?LinkID=404281) on the computer from which you want to manage the gateway.  
...
2.  Type the following, and then press **Enter**.  
    ...
    **Install\-PswaWebApplication \-UseTestCertificate**  
    ...
    Running the cmdlet installs the [!INCLUDE[pswa_2](includes/pswa_2_md.md)] web application within the IIS Default Web Site container. The cmdlet creates the infrastructure required to run [!INCLUDE[pswa_2](includes/pswa_2_md.md)] on the default website, https:\/\/<server\_name>\/pswa. To install the web application in a different website, provide the website name by adding the `WebSiteName` parameter. To change the name of the web application \(the default is `pswa`\), add the `WebApplicationName` parameter.  
...
4.  In the IIS Manager tree pane, expand the node for the server on which [!INCLUDE[pswa_2](includes/pswa_2_md.md)] is installed until the **Sites** folder is visible. Expand the **Sites** folder.  
  
5.  Select the website in which you have installed the [!INCLUDE[pswa_2](includes/pswa_2_md.md)] web application. In the **Actions** pane, click **Bindings**.  
...
8.  In the **SSL certificate** field, select your signed certificate from the drop\-down menu. Click **OK**. See [To configure an SSL certificate in IIS Manager](#BKMK_cert) in this topic for more information about how to obtain a certificate.  
    ...
    The [!INCLUDE[pswa_2](includes/pswa_2_md.md)] web application is now configured to use your signed SSL certificate. You can access [!INCLUDE[pswa_2](includes/pswa_2_md.md)] by opening https:\/\/<server\_name>\/pswa in a browser window.  
...
For more detail about [!INCLUDE[pswa_2](includes/pswa_2_md.md)] authorization rules and security, see [Authorization Rules and Security Features of Windows PowerShell Web Access_1](Authorization-Rules-and-Security-Features-of-Windows-PowerShell-Web-Access_1.md).  
...
After you have configured an authorization rule, you are ready for authorized users to sign in to the web\-based console and begin using [!INCLUDE[pswa_2](includes/pswa_2_md.md)].  
  
## <a name="BKMK_custom"></a>Custom deployment  
You can install the [!INCLUDE[pswa_2](includes/pswa_2_md.md)] gateway on a server that is running [!INCLUDE[winthreshold_server_2](includes/winthreshold_server_2_md.md)] by using the [!INCLUDE[arfw](includes/arfw_md.md)] in [!INCLUDE[sm](includes/sm_md.md)]. After [!INCLUDE[pswa_2](includes/pswa_2_md.md)] is installed, you can customize the configuration of the gateway in IIS Manager.  
...
4.  On the **Select destination server** page, select a server from the server pool, or select an offline VHD. To select an offline VHD as your destination server, first select the server on which to mount the VHD, and then select the VHD file. For information about how to add servers to your server pool, see the [!INCLUDE[sm](includes/sm_md.md)] Help. After you have selected the destination server, click **Next**.  
  
5.  On the **Select features** page of the wizard, expand **Windows PowerShell**, and then select **Windows PowerShell Web Access**.  
  
6.  Note that you are prompted to add required features, such as .NET Framework 4.6, and role services of Web Server \(IIS\). Add required features and continue.  
  
    > [!NOTE]  
    > Installing [!INCLUDE[pswa_2](includes/pswa_2_md.md)] by using the [!INCLUDE[arfw](includes/arfw_md.md)] also installs Web Server \(IIS\), including the IIS Manager snap\-in. The snap\-in and other IIS management tools are installed by default if you use [!INCLUDE[arfw](includes/arfw_md.md)]. If you install [!INCLUDE[pswa_2](includes/pswa_2_md.md)] by using [!INCLUDE[wps_2](includes/wps_2_md.md)] cmdlets as described in the following procedure, management tools are not added by default.  
  
7.  On the **Confirm installation selections** page, if the feature files for [!INCLUDE[pswa_2](includes/pswa_2_md.md)] are not stored on the destination server that you selected in step 4, click **Specify an alternate source path**, and provide the path to the feature files. Otherwise, click **Install**.  
  
8.  After you click **Install**, the **Installation progress** page displays installation progress, results, and messages such as warnings, failures, or post\-installation configuration steps that are required for [!INCLUDE[pswa_2](includes/pswa_2_md.md)]. After [!INCLUDE[pswa_2](includes/pswa_2_md.md)] is installed, you are prompted to review the readme file, which contains basic, required setup instructions for the gateway. These instructions are also included in this topic. The path to the readme file is C:\\Windows\\Web\\PowerShellWebAccess\\wwwroot\\README.txt.  
  
### Step 2: Configure the gateway  
Instructions in this section are for installing the [!INCLUDE[pswa_2](includes/pswa_2_md.md)] web application in a subdirectory—and not in the root directory—of your website. This procedure is the GUI\-based equivalent of the actions performed by the `Install-PswaWebApplication` cmdlet. This section also includes instructions for how to use IIS Manager to configure the [!INCLUDE[pswa_2](includes/pswa_2_md.md)] gateway as a root website.  
...
2.  Create a new application pool for [!INCLUDE[pswa_2](includes/pswa_2_md.md)]. Expand the node of the gateway server in the IIS Manager tree pane, select **Application Pools**, and click **Add Application Pool** in the **Actions** pane.  
  
3.  Add a new application pool with the name **pswa\_pool**, or provide another name. Click **OK**.  
  
4.  In the IIS Manager tree pane, expand the node for the server on which [!INCLUDE[pswa_2](includes/pswa_2_md.md)] is installed until the **Sites** folder is visible. Select the **Sites** folder.  
  
5.  Right\-click the website \(for example, **Default Web Site**\) to which you would like to add the [!INCLUDE[pswa_2](includes/pswa_2_md.md)] website, and then click **Add Application**.  
...
10. **Optional security step:** With the website selected in the tree pane, double\-click **SSL Settings** in the content pane. Select **Require SSL**, and then in the **Actions** pane, click **Apply**. Optionally, in the **SSL Settings** pane, you can require that users connecting to the [!INCLUDE[pswa_2](includes/pswa_2_md.md)] website have client certificates. Client certificates help to verify the identity of a client device user. For more information about how requiring client certificates can increase the security of [!INCLUDE[pswa_2](includes/pswa_2_md.md)], see [Authorization Rules and Security Features of Windows PowerShell Web Access_1](Authorization-Rules-and-Security-Features-of-Windows-PowerShell-Web-Access_1.md) in this guide.  
...
12. Open the new [!INCLUDE[pswa_2](includes/pswa_2_md.md)] website, https:\/\/< *gateway\_server\_name*>\/pswa.  
  
    The browser should display the [!INCLUDE[pswa_2](includes/pswa_2_md.md)] console sign\-in page.  
  
    > [!NOTE]  
    > You cannot sign in until users have been granted access to the website by adding authorization rules.  
  
13. In a [!INCLUDE[wps_2](includes/wps_2_md.md)] session that has been opened with elevated user rights \(Run as Administrator\), run the following script, in which *application\_pool\_name* represents the name of the application pool that you created in step 3, to give the application pool access rights to the authorization file.  
...
2.  In the IIS Manager tree pane, expand the node for the server on which [!INCLUDE[pswa_2](includes/pswa_2_md.md)] is installed until the **Sites** folder is visible. Select the **Sites** folder.  
...
15. Open the new [!INCLUDE[pswa_2](includes/pswa_2_md.md)] website.  
  
    Because the root website points to the [!INCLUDE[pswa_2](includes/pswa_2_md.md)] folder, the browser should display the [!INCLUDE[pswa_2](includes/pswa_2_md.md)] sign\-in page when you open https:\/\/< *gateway\_server\_name*>. You should not need to add **\/pswa** to the URL.  
...
1.  In the IIS Manager tree pane, select the server on which [!INCLUDE[pswa_2](includes/pswa_2_md.md)] is installed.  
...
After [!INCLUDE[pswa_2](includes/pswa_2_md.md)] is installed and the gateway configuration is finished as described in this topic, the [!INCLUDE[wps_2](includes/wps_2_md.md)] web\-based console is ready to use. For more information about getting started in the web\-based console, see [Use the Web-based Windows PowerShell Console_1](Use-the-Web-based-Windows-PowerShell-Console_1.md).  
