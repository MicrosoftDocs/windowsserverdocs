---
title: Step 1: Configure the DirectAccess Infrastructure_3
ms.custom: na
...
author: coreyp
---
# Step 1: Configure the DirectAccess Infrastructure_3
...
1.  On the Remote Access server: [!INCLUDE[startscreen](includes/startscreen_md.md)]**mmc.exe**, and then press ENTER.  
...
To deploy Remote Access, you require a minimum of two [!INCLUDE[gpos](includes/gpos_md.md)]: one [!INCLUDE[gpo](includes/gpo_md.md)] contains settings for the Remote Access server and one contains settings for DirectAccess client computers. When you configure Remote Access, the wizard automatically creates the required [!INCLUDE[gpos](includes/gpos_md.md)]. However, if your organization enforces a naming convention, or you do not have the required permissions to create or edit [!INCLUDE[gpos](includes/gpos_md.md)], they must be created prior to configuring Remote Access.  
...
> [!IMPORTANT]  
> The administrator can manually link the DirectAccess [!INCLUDE[gpos](includes/gpos_md.md)] to an Organizational Unit using these steps:  
>   
...
> 4.  If the linking to the OU has not been done before running the DirectAccess wizard, then after the configuration is complete, the domain administrator can link the DirectAccess [!INCLUDE[gpos](includes/gpos_md.md)] to the required Organizational Units. The link to the domain can be removed. Steps for linking a [!INCLUDE[gpo](includes/gpo_md.md)] to an Organization Unit can be found [here](http://technet.microsoft.com/library/cc732979.aspx).  
  
> [!NOTE]  
> If a [!INCLUDE[gpo](includes/gpo_md.md)] was created manually, it is possible during the DirectAccess configuration that the [!INCLUDE[gpo](includes/gpo_md.md)] will not be available. The [!INCLUDE[gpo](includes/gpo_md.md)] may not have been replicated to the closest Domain Controller to the management computer. In this event, the administrator can wait for replication to complete, or force the replication.  
  
## <a name="ConfigSGs"></a>Configure security groups  
The DirectAccess settings contained in the client computer [!INCLUDE[gpo](includes/gpo_md.md)] are applied only to computers that are members of the security groups that you specify when configuring Remote Access. In addition, if you are using security groups to manage your application servers, create a security group for these servers.  
