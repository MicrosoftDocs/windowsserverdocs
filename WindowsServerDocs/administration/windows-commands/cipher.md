...
| /w:`<directory>` | Removes data from available unused disk space on the entire volume using the DoD 5220.22-M standard. If you use the **/w** parameter, all other parameters are ignored. The directory specified can be located anywhere in a local volume. If it is a mount point or points to a directory in another volume, the data on that volume is removed. |
