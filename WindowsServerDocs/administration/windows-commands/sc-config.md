...
| `binpath= <binarypathname>` | Specifies a path to the service binary file. There is no default for **binpath=**, and this string must be supplied.<br />Additionally, **ntsd -d** can be specified in front of the string for debugging. For more information, see [Debugging using CDB and NTSD](/windows-hardware/drivers/debugger/debugging-using-cdb-and-ntsd).|
