...
| netsh `wfp show options` | Specifies which global WFP option to display in `optionfor=` format. Options are: <br><br><li> `NETEVENTS`: Shows whether network event diagnostics are enabled or disabled. <li> `KEYWORDS`: Displays the types of network events (such as broadcast or multicast) that are being captured for diagnostics. <li> `TXNWATCHDOG`: Displays the current timeout value (in milliseconds) for the transaction watchdog event. </li> |
...
```cmd
netsh wfp show options optionfor=NETEVENTS
```
