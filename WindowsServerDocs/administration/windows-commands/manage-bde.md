...
To displays the drives on the computer and identifies whether or not they are BitLocker-protected and the current encryption status.
```
manage-bde -status
```
To illustrates enabling BitLocker on drive C with the option of a recovery password. The recovery password will be generated by BitLocker and displayed on the screen so that you can record it.
```
manage-bde –on C: -recoverypassword
```
To illustrates unlocking a BitLocker-protected drive by using a recovery password.
```
