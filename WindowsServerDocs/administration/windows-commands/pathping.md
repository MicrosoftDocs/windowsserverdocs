---
title: pathping
description: Reference topic for the pathping command, which gets info about network latency and network loss at intermediate hops between a source and a destination.
...
>
> Additionally, this command identifies which routers are on the path, same as using the [tracecert command](tracecert.md). Howevever, this command also sends pings periodically to all of the routers over a specified time period and computes statistics based on the number returned from each.
...
- [tracecert command](tracecert.md)
