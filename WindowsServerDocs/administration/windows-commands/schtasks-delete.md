...
description: Reference article for the schtasks delete command, which deletes a scheduled task.
ms.topic: reference
ms.author: lizross
author: eross-msft
manager: mtillman
...
| /p `<password>` | Specifies the password of the user account specified in the **/u** parameter. If you use the **/u** parameter without the **/p** parameter or the password argument, schtasks will prompt you for a password. |
| /? | Displays help at the command prompt. The **/u** and **/p** parameters are valid only when you use **/s**. |
