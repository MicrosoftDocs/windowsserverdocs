...
1. Via a user's configuration file at _%userprofile%\\.ssh\config_.
1. Via the system-wide configuration file at _%programdata%\ssh\ssh\_config_.
...
Windows also includes PowerShell, and third-party command shells are also available for Windows and might be configured as the default shell for a server.
...
In Windows, sshd reads configuration data from _%programdata%\ssh\sshd\_config_ by default. You can specify a different configuration file by running `sshd.exe` with the `-f` parameter.
...
Windows allows multiple formats for specifying domain principals, but might conflict with standard Linux patterns.
For that reason, `*` is used to cover FQDNs.
...
The following example denies contoso\admin from the host 192.168.2.23 and blocks all users from contoso domain. It also allows users who are members of the contoso\sshusers and contoso\serveroperators groups.
...
The default is `.ssh/authorized_keys`. If the path isn't absolute, it's taken relative to the user's
...
> account must be granted full control. The BUILTIN\Administrators security group is required to enable
