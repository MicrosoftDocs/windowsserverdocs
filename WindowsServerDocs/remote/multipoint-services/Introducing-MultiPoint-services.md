...
The MultiPoint Services role in Windows Server 2016 allows multiple users, each with their own independent and familiar Windows experience, to simultaneously share one computer. There are several ways users can access their sessions. One way is via remoting into the server using [remote desktop applications](../remote-desktop-services/clients/remote-desktop-clients.md) with any device. Another means is through physical stations that are attached to the MultiPoint server via:
