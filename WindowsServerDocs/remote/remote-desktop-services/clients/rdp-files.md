...
| enablerdsaadauth:i:value | Determines whether the client will use Azure AD to authenticate to the remote PC if it is available. | - 0: RDP will not use Azure AD authentication, even if the remote PC supports it</br>- 1: RDP will use Azure AD authentication if the remote PC supports it | 0 | Yes |
