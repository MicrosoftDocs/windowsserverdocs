...
| compression:i:value | This setting determines whether bulk compression is enabled when it is transmitted by RDP to the local computer.|(0) Disable RDP bulk compression; (1) Enable RDP bulk compression | 1 || x | x |
...
|remoteapplicationname:s:value | Specifies the name of the RemoteApp in the client interface while starting the RemoteApp.||x|x|x|
|remoteapplicationprogram:s:value | Specifies the alias or executable name of the RemoteApp. ||x|x|x|
...
|smart sizing:i:value | This setting determines whether or not the client computer can scale the content on the remote computer to fit the window size of the client computer.|(0) The client window display will not be scaled when resizsed; (1) The client window display will be scaled when resized|0|x|x||
