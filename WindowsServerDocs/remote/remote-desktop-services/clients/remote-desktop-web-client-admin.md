...
1. Finally, run this cmdlet to publish the Remote Desktop web client:
    ...
    >[!NOTE]
    >When running the **Publish-RDWebClientPackage** cmdlet, you may see a warning that says per-device CALs are not supported, even if your deployment is configured for per-user CALs. You can ignore this warning. We display it to make sure you’re aware of the configuration limitation.
