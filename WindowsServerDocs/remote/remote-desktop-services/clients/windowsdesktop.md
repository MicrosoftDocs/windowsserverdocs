...
1. From the Connection Center, select the **Settings** icon below the desktop connection.
2. The Settings panel appears on the right side of the client displaying the name of the desktop.
...
- **Select the displays to use for the session** specifies which local displays to use for the session. All selected displays must be adjacent. This settings is automatically disabled when using a single display.
- **Start in full screen** determines whether the session will launch in full-screen or windowed mode. This setting is automatically enabled when using multiple displays.
- **Update the resolution on resize** changes the behavior when you resize the session in windowed mode. If enabled, the resolution of the remote desktop will update to match the size of the local window. If disabled, the session will retain the resolution specified in **Resolution** for its entire duration. This setting is automatically enabled when using multiple displays.
