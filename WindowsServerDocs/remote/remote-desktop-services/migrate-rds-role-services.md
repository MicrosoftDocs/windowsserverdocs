---
title: Migrate your Remote Desktop Services deployment to the latest Windows Server version
...
manager: scottman
---
# Migrate your Remote Desktop Services deployment to the latest Windows Server version
...
This is the first and most important step for migrating: migrating your RD Connection Brokers to destination servers running Windows Server 2025.
...
2. [Upgrade](./upgrade-to-rds.md) the remaining RD Connection Broker server in the deployment to Windows Server 2025.

3. Add Windows Server 2025 RD Connection Broker servers into the high availability deployment.
...
Follow these steps to migrate a session collection in an earlier version of Windows Server to a session collection in Windows Server 2025.
...
1. [Upgrade the session collection](./upgrade-to-rdsh.md) to Windows Server 2025.

2. Add the new RD Session Host server running Windows Server 2025 to the session collection.
...
Follow these steps to migrate a virtual desktop collection from a source server running the earlier version of Windows Server to a destination server running Windows Server 2025.
...
1. [Upgrade the virtual desktop collection](./upgrade-to-rdvh.md) from the server running the earlier version of Windows Server to Windows Server 2025.

2. Add the new Windows Server 2025 RD Virtualization Host servers to the virtual desktop collection.
...
> If the UVHD template (UVHD-template.vhdx) is enabled in the session collection and the file server has been migrated to a new server, update the User Profile Disks: Location collection property with the new path. The User Profile Disks must be available at the same relative path in the new location as they were on the source server.
>
> A virtual desktop collection of RD Virtualization Host servers with a mix of servers running earlier versions of Windows Server and Windows Server 2025 is not supported.

## Migrate RD Web Access servers

Follow these steps to migrate RD Web Access servers:

- Join the destination servers running Windows Server 2025 to the Remote Desktop Services deployment and install the RD Web role

- Use [IIS Web Deploy tool](https://www.iis.net/) to migrate the RD Web website settings from the current RD Web Access servers to the destination servers running Windows Server 2025.

- [Migrate certificates](#migrate-certificates) to the destination servers running Windows Server 2025.
...
- Join the destination servers running Windows Server 2025 to the Remote Desktop Services deployment and install the RD Gateway role

- Use the [IIS Web Deploy tool](https://www.iis.net/) to migrate the RD Gateway endpoint settings from the current RD Gateway servers to the destination servers running Windows Server 2025.
...
Follow these steps to migrate an RD Licensing server from a source server running an earlier version of Windows Server to a destination server running Windows Server 2025.
...
2. Edit the **Deployment Properties** in **Server Manager** on the Remote Desktop management server (which is typically being run on the first RD Connection Broker server) to include only the new RD Licensing servers running Windows Server 2025.
