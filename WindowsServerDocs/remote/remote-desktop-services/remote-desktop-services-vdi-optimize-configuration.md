...
|Store|Turn off the offer to update to the latest version of Windows| N/A |**Enabled** (Enables or disables the Store offer to update to the latest version of Windows. If you enable this setting, the Store application don't offer updates to the latest version of Windows.)|
...
|Windows Security \ Notifications|Hide non-critical notifications| N/A |**Enabled** (With this setting enabled, local users only see critical notifications from Windows Security. They don't see other types of notifications, such as regular PC or device health information.)|
...
Many services that may seem like good candidates to disable are set to manual service start type. This means that the service doesn't automatically start and start only if an event triggers a request to the service. Services that are already set to start type manual are not listed here.
