...
2.  Plan a range of static IP addresses on the internal perimeter network to meet that requirement (that is, the _static address pool_).
...
| Are you just deploying Always On VPN | Refer to [STEP 3: Configure the Server Infrastructure](vpn-deploy-server-infrastructure.md). Install and configure the server-side components necessary to support the VPN, including configuring PKI to distribute the certificates used by users, the VPN server, and the NPS server. |