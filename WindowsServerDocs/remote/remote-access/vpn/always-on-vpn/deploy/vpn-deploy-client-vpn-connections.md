...
6. Under the **Base VPN** tab, verify or set the following settings:
    ...
    - **Default server:** If this is the default VPN server, set to **True**. Doing this enables this server as the default server that devices use to establish the connection. Set only one server as the default.
...
|Migrate from DirectAccess to Always On VPN  |[Remove devices from the DirectAccess security group](../../../da-always-on-vpn-migration/da-always-on-migration-deploy.md#remove-devices-from-the-directaccess-security-group): In this step, you remove user devices from the DirectAccess security group, after each deployment, so that you can later remove DirectAccess. Both Intune and System Center Configuration Manager contains user device assignment information to help you determine each user's device.       |
