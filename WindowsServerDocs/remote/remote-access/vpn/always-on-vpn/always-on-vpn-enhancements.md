...
| **Exclusion routes** | Always On VPN supports the ability to specify exclusion routes that specifically control routing behavior to define which traffic should traverse the VPN only and not go over the physical network interface.<p>_**Notes.**_<p>- Exclusion routes currently work for traffic within the same subnet as the client, for example, LinkLocal.<p>- Exclusion routes only work in a Split Tunnel setup. |
