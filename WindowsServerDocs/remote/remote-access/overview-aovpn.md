...
| **Configure fall back to SSTP from IKEv2** when clients are behind firewalls or proxy servers. You can cojnfigure fall back by using the automatic tunnel/protocol type within the VPN profile.<p><p>***Note:***<br>User Tunnel supports SSTP and IKEv2, and Device Tunnel supports IKEv2 only, with no support for SSTP fallback.|
