...
The next step in deploying Remote Access in a multisite topology is to complete the multisite infrastructure planning; including, Active Directory, security groups, and [!INCLUDE[gpos](includes/gpos_md.md)].
...
|[2.3 Plan Group Policy Objects](assetId:///bd7b4f5e-91b5-49ac-81bb-53c0008b7f4f#bkmk_2_3_GPO)|Plan additional [!INCLUDE[gpos](includes/gpos_md.md)] \(GPOs\) for your deployment.|
...
2.  A multisite entry point can only be associated with a single Active Directory site. When client computers running [!INCLUDE[win8_client_1](includes/win8_client_1_md.md)] connect to a specific entry point, they are considered as belonging to the Active Directory site associated with that entry point.
...
During deployment of a single server with advanced settings, all client computers accessing the internal network via DirectAccess were gathered into a security group. In a multisite deployment, this security group is used for [!INCLUDE[win8_client_2](includes/win8_client_2_md.md)] client computers only. For a multisite deployment, [!INCLUDE[nextref_client_7](includes/nextref_client_7_md.md)] client computers will be gathered into separate security groups for each entry point in the multisite deployment. For example, if you previously grouped all client computers in the group DA\_Clients, you must now remove any [!INCLUDE[nextref_client_7](includes/nextref_client_7_md.md)] computers from that group and put them in a different security group. For example, in the multiple Active Directory sites, multiple entry points topology, you create a security group for the United States entry point \(DA\_Clients\_US\) and one for the Europe entry point \(DA\_Clients\_Europe\). Place any Windows 7 client computers located in the United States in the DA\_Clients\_US group and any located in Europe in the DA\_Clients\_Europe group. If you do not have any [!INCLUDE[nextref_client_7](includes/nextref_client_7_md.md)] client computers, you do not need to plan security groups for [!INCLUDE[nextref_client_7](includes/nextref_client_7_md.md)] computers.

Required security groups are as follows:

-   One security group for all [!INCLUDE[win8_client_2](includes/win8_client_2_md.md)] client computers. It is recommended to create a unique security group for these clients for each domain.

-   A unique security group containing [!INCLUDE[nextref_client_7](includes/nextref_client_7_md.md)] client computers for each entry point. It is recommended to create a unique group for each domain. For example: Domain1\\DA\_Clients\_Europe; Domain2\\DA\_Clients\_Europe; Domain1\\DA\_Clients\_US; Domain2\\DA\_Clients\_US.
...
-   A GPO for each domain containing client computers running [!INCLUDE[win8_client_2](includes/win8_client_2_md.md)].

-   A GPO for each entry point and each domain containing [!INCLUDE[nextref_client_7](includes/nextref_client_7_md.md)] client computers.
...
-   The following GPOs should be created manually for the multisite deployment:
    ...
    -   **Client GPO \(Windows 7\)**—A GPO for each entry point and each domain containing [!INCLUDE[nextref_client_7](includes/nextref_client_7_md.md)] client computers that will connect to entry points in the multisite deployment. For example Domain1\\DA\_W7\_Clients\_GPO\_Europe; Domain2\\DA\_W7\_Clients\_GPO\_Europe; Domain1\\DA\_W7\_Clients\_GPO\_US; Domain2\\DA\_W7\_Clients\_GPO\_US. If no [!INCLUDE[nextref_client_7](includes/nextref_client_7_md.md)] client computers will connect to entry points, GPOs are not required.

-   There is no requirement to create additional GPOs for [!INCLUDE[win8_client_2](includes/win8_client_2_md.md)] client computers. A GPO for each domain containing client computers was already created when the single Remote Access server was deployed. In a multisite deployment these client GPOs will function as the GPOs for [!INCLUDE[win8_client_2](includes/win8_client_2_md.md)] clients.
