...
When you deploy multiple instances of RAS Gateway VMs that provide high availability and failover, you are deploying a gateway pool. In [!INCLUDE[winblue_server_1](includes/winblue_server_1_md.md)], all the gateway VMs formed a single pool, which made a logical separation of the gateway deployment a little difficult. [!INCLUDE[winblue_server_2](includes/winblue_server_2_md.md)] gateway  offered a 1:1 redundancy deployment for the gateway VMs, which resulted in under\-utilization of the available capacity for site\-to\-site \(S2S\) VPN connections.  
  
This issue is resolved in [!INCLUDE[winthreshold_server_1](includes/winthreshold_server_1_md.md)], which provides multiple Gateway Pools \- which can be of different types for logical separation. The new mode of M\+N redundancy allows for a more efficient failover configuration.  
  
For more overview information about RAS Gateway, see [RAS Gateway](RAS-Gateway.md).  
  
## <a name="bkmk_pools"></a>Gateway Pools Overview  
In [!INCLUDE[winthreshold_server_2](includes/winthreshold_server_2_md.md)], you can deploy gateways in one or more pools.  
...
RAS Gateway is fully integrated with Network Controller in [!INCLUDE[winthreshold_server_2](includes/winthreshold_server_2_md.md)]. When RAS Gateway and Network Controller are deployed, Network Controller performs the following functions.  
