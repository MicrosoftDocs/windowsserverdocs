...
- S2D (Storage Spaces Direct) campus cluster
- S2D stretch cluster
- SAN stretch cluster
...
| Characteristic | Simple Cluster | S2D Campus Cluster | S2D Stretch Cluster | SAN Stretch Cluster | Multi-Cluster |
|---|---|---|---|---|---|
...
- **S2D stretch cluster**: Each site maintains its own Storage Spaces Direct storage pool, with Storage Replica providing replication between sites
...
In an S2D stretch cluster configuration, each site operates its own independent Storage Spaces Direct storage pool. Storage Replica handles replication between sites, and you can configure it as synchronous or asynchronous depending on the distance and latency between locations.
