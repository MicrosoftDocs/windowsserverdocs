...
Simplified SMB Multichannel and Multi- NIC (Network Interface Card) Cluster Networks is a feature that enables the use of multiple NICs on the same cluster network subnet, and automatically enables SMB Multichannel.
...
**Figure 3: Two clusters (Scale-out File Server for storage, SQL Server  FCI (Failover Clustering Instance) for workload) both use multiple NICs in the same subnet to leverage SMB Multichannel and achieve better network throughput.**
