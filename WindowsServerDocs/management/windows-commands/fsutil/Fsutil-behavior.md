...
|Bugcheckoncorrupt {1&#124;0}|Allows \(**1**\) or disallows \(**0**\) generation of a bug check when there is corruption on an NTFS volume. This feature can be used to prevent NTFS from silently deleting data when used with the Self\-Healing NTFS feature.<br /><br />You must reboot your computer for this parameter to take effect.<br /><br />This parameter applies to: [!INCLUDE[nextref_server_7](includes/nextref_server_7_md.md)] and [!INCLUDE[nextref_client_7](includes/nextref_client_7_md.md)].|
