...
The following command creates a counter called perf\_log using the % Processor time counter from the Processor\(\_Total\) counter category.  
  
```  
logman create counter perf_log -c "\Processor(_Total)\% Processor time"  
```  
  
The following command creates a counter called perf\_log using the % Processor time counter from the Processor\(\_Total\) counter category, creating a log file with a maximum size of 10 MB and collecting data for 1 minute and 0 seconds.  
