...
creates and configures a new namespace. You should use this option when you have only the Transport Server role service installed. if you have both the deployment Server role service and the Transport Server role service installed \(which is the default\), use [Using the new-MulticastTransmission Command](using-newmulticasttransmission-command.md). Note that you must register the content provider before you use this option.
