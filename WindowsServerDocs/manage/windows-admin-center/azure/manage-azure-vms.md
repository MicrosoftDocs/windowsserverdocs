...
To instead use Windows Admin Center directly in the Azure portal to manage the operating system on an single Azure VM, see [Use Windows Admin Center in Azure](../wacinazure/manage-vm.md). You can also use scripts to [set up a Windows Admin Center server in an Azure VM](deploy-wac-in-azure.md).
