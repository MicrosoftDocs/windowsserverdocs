...
This topic provides step-by-step instructions for configuring DirectAccess in [!INCLUDE[wsessentials](../windows-server-essentials-manage/includes/wsessentials_md.md)] to enable your mobile workforce to seamlessly connect to your organization’s network from any Internet-equipped remote location without establishing a virtual private network (VPN) connection. DirectAccess can offer mobile workers the same connectivity experience inside and outside the office from their [!INCLUDE[winblue_client_2](../windows-server-essentials-manage/includes/winblue_client_2_md.md)], [!INCLUDE[win8_client_2](../windows-server-essentials-manage/includes/win8_client_2_md.md)], and [!INCLUDE[nextref_client_7](../windows-server-essentials-manage/includes/nextref_client_7_md.md)] computers.  
  
 [!INCLUDE[wse_all](../windows-server-essentials-manage/includes/wse_all_md.md)] In [!INCLUDE[wseblue_2](../windows-server-essentials-manage/includes/wseblue_2_md.md)], if the domain contains more than one [!INCLUDE[wsessentials](../windows-server-essentials-manage/includes/wsessentials_md.md)] server, DirectAccess must be configured on the domain controller.  
  
> [!NOTE]
>  This topic provides instructions for configuring DirectAccess when your [!INCLUDE[wsessentials](../windows-server-essentials-manage/includes/wsessentials_md.md)] server is the domain controller. If the [!INCLUDE[wsessentials](../windows-server-essentials-manage/includes/wsessentials_md.md)] server is a domain member, follow the instructions to configure DirectAccess on a domain member in [Add DirectAccess to an Existing Remote Access (VPN) Deployment](http://technet.microsoft.com/library/jj574220.aspx) instead.  
  
## Process overview  
 To configure DirectAccess in [!INCLUDE[wsessentials](../windows-server-essentials-manage/includes/wsessentials_md.md)], complete the following steps.  
  
> [!IMPORTANT]
>  Before you use the procedures in this guide to configure DirectAccess in [!INCLUDE[wsessentials](../windows-server-essentials-manage/includes/wsessentials_md.md)], you must enable VPN on the server. For instructions, see [Manage VPN](../windows-server-essentials-manage/Manage-VPN-in-Windows-Server-Essentials.md).  
...
1.  On the server, in the bottom left corner of the Start page, click the **Server Manager** icon.  
     ...
     In [!INCLUDE[wseblue_2](../windows-server-essentials-manage/includes/wseblue_2_md.md)], you will need to search for Server Manager to open it. On the Start page, type **Server Manager**, and then click **Server Manager** in the search results. To pin Server Manager to the Start page, right-click Server Manager in the search results, and click **Pin to Start**.  
...
4.  In the Add Roles and Features Wizard, do the following:  
    ...
    2.  On the **Server Selection page** (or the **Select destination server** page in [!INCLUDE[sbs_sbs8web_2](../windows-server-essentials-manage/includes/sbs_sbs8web_2_md.md)]), click **Select a server from the server pool**.  
...
###  <a name="BKMK_MapNewHosttoServerAddress"></a> Step 3c: Add a new host on the DNS server and map it to the Windows Server Essentials server address  
 To complete the DNS configuration, add a new host on the DNS server and map it to the [!INCLUDE[wsessentials](../windows-server-essentials-manage/includes/wsessentials_md.md)] server address.  
...
##  <a name="BKMK_EnableConfigureDA"></a> Step 5: Enable and configure DirectAccess  
 To enable and configure DirectAccess in [!INCLUDE[wsessentials](../windows-server-essentials-manage/includes/wsessentials_md.md)], you must complete the following steps:  
...
###  <a name="BKMK_EnableDA"></a> Step 5a: Enable DirectAccess by using the Remote Access Management Console  
 This section provides step-by-step instructions to enable DirectAccess in [!INCLUDE[wsessentials](../windows-server-essentials-manage/includes/wsessentials_md.md)]. If you have not configured VPN on the server yet, you should do that before you start this procedure. For instructions, see [Manage VPN](../windows-server-essentials-manage/Manage-VPN-in-Windows-Server-Essentials.md).  
...
###  <a name="BKMK_RemoveIPv6"></a> Step 5b: Remove the invalid IPv6Prefix in RRAS GPO (Windows Server 2012 Essentials only)  
 [!INCLUDE[wse_section](../windows-server-essentials-manage/includes/wse_section_md.md)]  
...
###  <a name="BKMK_Step4cWindows7Setup"></a> Step 5c: Enable client computers running Windows 7 Enterprise to use DirectAccess  
 If you have client computers running [!INCLUDE[nextref_client_7ent](../windows-server-essentials-manage/includes/nextref_client_7ent_md.md)], complete the following procedure to enable DirectAccess from those computers.  
  
##### To enable [!INCLUDE[nextref_client_7ent](../windows-server-essentials-manage/includes/nextref_client_7ent_md.md)] computers to use DirectAccess  
...
3.  On the **Authentication** tab, choose the certification authority (CA) certificate that will be the trusted root certificate (you can choose the CA certificate of the [!INCLUDE[wsessentials](../windows-server-essentials-manage/includes/wsessentials_md.md)] server). Click **Enable Windows 7 client computers to connect via DirectAccess**, and then click **Next**.  
  
4.  Follow the instructions to complete the wizard.  
  
> [!IMPORTANT]
>  There is a known issue for [!INCLUDE[nextref_client_7ent](../windows-server-essentials-manage/includes/nextref_client_7ent_md.md)] computers connecting over DirectAccess if the [!INCLUDE[sbs_sbs8web_2](../windows-server-essentials-manage/includes/sbs_sbs8web_2_md.md)] server did not come with UR1 pre-installed. To enable DirectAccess connections in that environment, you must perform these additional steps:  
>   
...
> 2.  Then install the hotfix described in [Microsoft Knowledge Base (KB) article 2615847](http://support.microsoft.com/kb/2615847) on each [!INCLUDE[nextref_client_7](../windows-server-essentials-manage/includes/nextref_client_7_md.md)] computer.  
>   
>      This issue was resolved in [!INCLUDE[wseblue_2](../windows-server-essentials-manage/includes/wseblue_2_md.md)].  
...
##  <a name="BKMK_ExemptPort"></a> Step 9: Reserve ports for the WinNat service  
 Use the following Windows PowerShell command to reserve ports for the WinNat service. Replace "192.168.1.100" with the actual IPv4 address of your [!INCLUDE[sbs_sbs8web_2](../windows-server-essentials-manage/includes/sbs_sbs8web_2_md.md)] server.  
...
> [!IMPORTANT]
>  In [!INCLUDE[wseblue_2](../windows-server-essentials-manage/includes/wseblue_2_md.md)], you do not need to remove the unnecessary IPv6 prefix GPO. Delete the code section with the following label: `# [WINDOWS SERVER 2012 ESSENTIALS ONLY] Remove the unnecessary IPv6 prefix GPO`.  
