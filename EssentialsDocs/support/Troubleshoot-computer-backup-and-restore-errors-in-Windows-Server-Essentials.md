...
5. Make sure that Windows Server Client Computer Backup Provider Service is running on the client computer.
...
4. Check alerts to determine if there are errors in the backup database. If there are errors, follow the instructions in the alert to repair the backup database.

5. Uninstall the Windows Server Essentials Connector software from the computer, and then reinstall it. For more information, see the topics [Uninstall the Connector software](../use/Get-Connected-in-Windows-Server-Essentials.md#BKMK_13) and [Install the Connector software](../use/Get-Connected-in-Windows-Server-Essentials.md#BKMK_11).

## Troubleshoot a backup that did not complete properly
...
6. Run **vssadmin** in a command prompt to troubleshoot Volume Shadow Copy Service (VSS) issues. For information about **vssadmin**, see [VSSADMIN](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc754968(v=ws.10)).

## Troubleshoot backup health alert issues
...
4. Check event logs on the client computer or server for issues relating to backup services or drivers.

5. Reboot the server or client computer where you received the error message.

6. Check health alerts for other issues that may have an effect on client backup.

## Troubleshoot a file or folder restore
...
## Troubleshoot a full system restore
