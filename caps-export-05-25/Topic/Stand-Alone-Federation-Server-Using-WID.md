...
A stand\-alone federation server in [!INCLUDE[firstref_adfs2](../Token/firstref_adfs2_md.md)] consists of a single server that hosts a Federation Service configured to use the Windows Internal Database \(WID\). This [!INCLUDE[nextref_adfs2](../Token/nextref_adfs2_md.md)] topology is for test labs. We do not recommend it for production environments because it has a limit of only one federation server, and it cannot be used to scale up to more servers.  
