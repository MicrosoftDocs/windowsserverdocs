...
ms.tgt_pltfrm: na
ms.topic: article
ms.assetid: ab4d0404-366c-49d4-a95e-e3e72aa9b5a9
author: vhorne
---
...
-   [Router Versions in Windows Server 2012 R2](#bkmk_versions)  
  
-   [BGP Supported Deployment Topologies](#bkmk_top)  
  
-   [BGP Features](#bkmk_features)  
  
When configured on a [!INCLUDE[winblue_server_1](../Token/winblue_server_1_md.md)] Routing and Remote Access Service \(RRAS\) Multitenant Gateway, Border Gateway Protocol \(BGP\) provides you with the ability to manage the routing of network traffic between your tenants’ VM networks and their remote sites.  
...
## <a name="bkmk_versions"></a>Router Versions in Windows Server 2012 R2  
Two different versions of the BGP router are available in [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] – the RRAS Multitenant Gateway and Windows Server Gateway. Although the routers have the same functionality and capabilities, you can use different methods to manage each router, depending on whether you are using [!INCLUDE[sc2012r2_1](../Token/sc2012r2_1_md.md)].  
  
**RRAS Multitenant Gateway**. The RRAS Multitenant Gateway BGP router can be used for multitenant or non\-multitenant deployments, and is a full featured BGP router. To deploy an RRAS Multitenant Gateway BGP Router, you must use Windows PowerShell commands. For more information, see [Remote Access Cmdlets in Windows PowerShell](http://technet.microsoft.com/library/hh918399.aspx) and [Windows Server 2012 R2 RRAS Multitenant Gateway Deployment Guide](../Topic/Windows-Server-2012-R2-RRAS-Multitenant-Gateway-Deployment-Guide.md).  
  
**Windows Server Gateway**. To deploy Windows Server Gateway, you must use [!INCLUDE[sc2012r2_1](../Token/sc2012r2_1_md.md)] and Virtual Machine Manager \(VMM\). The Windows Server Gateway BGP router is designed for use with multitenant deployments. With the [!INCLUDE[sc2012r2_1](../Token/sc2012r2_1_md.md)] VMM Windows Server Gateway router, only a very limited set of configuration options are available in the VMM software interface, including Local BGP IP Address and Autonomous System Numbers \(ASN\), List of BGP Peer IP Addresses, and ASN, values. You can, however, use Remote Access Windows PowerShell BGP commands to configure all other features of Windows Server Gateway. For more information, see [Windows Server Gateway](assetId:///e39e8b8c-a23c-4dbd-b895-d1e3f2885e9e) and [Virtual Machine Manager](http://technet.microsoft.com/library/gg610610.aspx).  
...
-   [RRAS VPN Site\-to\-Site Gateway with BGP at Enterprise site edge](#bkmk_top1)  
...
![](../Image/02_bgp_3rd_party_edge.jpg)  
...
### <a name="bkmk_top3"></a>Multiple Enterprise sites with third party gateways  
...
For additional information, see the Networking blog [Border Gateway Protocol \(BGP\) with Windows Server 2012 R2](http://blogs.technet.com/b/networking/archive/2013/10/11/border-gateway-protocol-bgp-with-windows-server-2012-r2.aspx).  
  
## See Also  
[Routing and Remote Access Service &#40;RRAS&#41;](../Topic/Routing-and-Remote-Access-Service--RRAS-.md)  
