...
-   [Create the referral service files](../Topic/Replace-the-List-of-Domain-Name-Providers.md#BKMK_ReferralFiles)  
  
-   [Add an entry to the registry on the reference computer](../Topic/Replace-the-List-of-Domain-Name-Providers.md#BKMK_AddRegistry)  
...
6.  Type the HTTPS URL that is used to access the referral files that you created in [Create the referral service files](../Topic/Replace-the-List-of-Domain-Name-Providers.md#BKMK_ReferralFiles), and then click **OK**.  
  
    > [!IMPORTANT]  
    > A slash \(\/\) is required at the end of the URL.  
  
### <a name="BKMK_ReplaceDomainNameProviders"></a>Domain name status issues  
If a partner adds domain name providers and uses an application programming interface \(API\) in the [!INCLUDE[sbs_sbs8web_2](../Token/sbs_sbs8web_2_md.md)] SDK to set the Unknown, Failed, and CertificateRequestNotSubmitted statuses for the certificate, the customer receives an incorrect message and configuration result. This is because the cases are handled by exceptions rather than returning a status.  
