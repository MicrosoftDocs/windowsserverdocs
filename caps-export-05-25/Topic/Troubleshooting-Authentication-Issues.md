...
On the DirectAccess server, run the following [!INCLUDE[wps_2](../Token/wps_2_md.md)] commands:  
...
-   Make sure that the domain controller is configured as a management server and that the client machine can reach the domain controller over the infrastructure tunnel. See [3.2 Plan the OTP certificate template](../Topic/Step-3--Plan-OTP-Certificate-Deployment.md#bkmk_3_2_OTP_Cert).  
...
The OTP provider used requires the user to provide additional credentials in the form of a RADIUS challenge\/response exchange, which is not supported by [!INCLUDE[win8_server_2](../Token/win8_server_2_md.md)] DirectAccess OTP.  
...
To create the OTP signing certificate template see [3.3 Plan the registration authority certificate](../Topic/Step-3--Plan-OTP-Certificate-Deployment.md#bkmk_3_3_RA_Cert).  
