...
a21aa6d9-7e8a-4a8f-bebf-c3e38d0b733a </code>
  ...
  <section address="BKMK_Solution">
    ...
    <content>
      <para>If replication of a lingering object is detected, you can remove the object from AD DS, along with any read-only replicas of the object, by identifying the domain controllers that might store this object (including global catalog servers) and running a <system>repadmin</system> command to remove lingering objects on these servers (<system>repadmin /removelingeringobjects</system>). This command is available on domain controllers that are running <token>nextref_longhorn</token>. It is also available on domain controllers that are not running <token>nextref_longhorn</token> but are running the version of Repadmin.exe that is included with Windows Support Tools in Windows Server 2003.</para>
    ...
    <sections>
      <section address="BKMK_Identify_GUID">
        ...
        <content>
          ...
          <list class="bullet">
            <listItem>
              <para>Membership in <embeddedLabel>Domain Admins</embeddedLabel> in the domain of the domain controller whose GUID you want to identify is the minimum required to complete this procedure. <token>review_details</token></para>
            </listItem>
            <listItem>
      ...
      <section address="BKMK_RemoveLingeringObjects">
        ...
        <content>
          <para>If the destination domain controller and source domain controller are running either Windows Server 2003 or <token>nextref_longhorn</token>, you can use this procedure to remove lingering objects with Repadmin. If either domain controller is running Windows 2000 Server, follow the instructions in article 314282 in the Microsoft Knowledge Base (<externalLink><linkText>http://go.microsoft.com/fwlink/?LinkId=41410</linkText><linkUri>http://go.microsoft.com/fwlink/?LinkId=41410</linkUri></externalLink>).</para>
          <para>
            <embeddedLabel>Requirements</embeddedLabel>
          </para>
          <list class="bullet">
            <listItem>
              <para>Membership in <embeddedLabel>Domain Admins</embeddedLabel> in the domain of the domain controller that has lingering objects, or <embeddedLabel>Enterprise Admins</embeddedLabel> if the directory partition that has lingering objects is the configuration or schema directory partition, is the minimum required to complete this procedure. <token>review_details</token></para>
            </listItem>
            <listItem>
              <para>Operating system: Windows Server 2003 or <token>nextref_longhorn</token> for &lt;ServerName&gt; and &lt;ServerGUID&gt;</para>
            </listItem>
            <listItem>
              <para>Tool: Repadmin.exe</para>
            </listItem>
      ...
      <section address="BKMK_Enable_Strict">
        ...
        <sections>
          <section address="BKMK_Use_Repadmin">
            ...
            <content>
              <para>Use this procedure to remove lingering objects on a domain controller that is running Windows Server 2003 with SP1, Windows Server 2003 with SP2, Windows Server 2003 R2, or <token>nextref_longhorn</token>.</para>
              <para>Membership in <embeddedLabel>Domain Admins</embeddedLabel>, or equivalent, is the minimum required to complete this procedure on a single domain controller. Membership in <embeddedLabel>Enterprise Admins</embeddedLabel>, or equivalent, is the minimum required to complete this procedure on all domain controllers in the forest. <token>review_details</token></para>
              <procedure>
          ...
          <section address="BKMK_Edit_Registry">
            ...
            <content>
              ...
              <list class="bullet">
                ...
                <listItem>
                  <para>Default: <system>1</system> (enabled) in a new Windows Server 2003 or <token>nextref_longhorn</token> forest; otherwise <system>0</system>.</para>
                </listItem>
                <listItem>
              ...
              <list class="bullet">
                <listItem>
                  <para>Membership in <embeddedLabel>Domain Admins</embeddedLabel>, or equivalent, is the minimum required to complete this procedure. <token>review_details</token></para>
                </listItem>
                <listItem>
  ...
  <section address="BKMK_Ensure_Strict">
    ...
    <content>
      ...
      <para>Membership in <embeddedLabel>Enterprise Admins</embeddedLabel>, or equivalent, is the minimum required to complete this procedure. <token>review_details</token></para>
      <procedure>
