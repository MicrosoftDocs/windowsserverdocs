...
Storage Quality of Service \(Storage QoS\) in [!INCLUDE[winthreshold_server_2](../Token/winthreshold_server_2_md.md)] provides a way to centrally monitor and manage storage performance for virtual machines using Hyper\-V and the Scale\-Out File Server roles. The feature automatically improves storage resource fairness between multiple virtual machines using the same file server cluster and allows policy basedminimum and maximum performance goals to be configured in units of normalized IOPs.  
  
You can use Storage QoS in [!INCLUDE[winthreshold_server_2](../Token/winthreshold_server_2_md.md)] to accomplish the following:  
...
-   [Overview](../Topic/Storage-Quality-of-Service.md#BKMK_Overview)  
  
-   [How to set up Storage QoS and monitor basic performance](../Topic/Storage-Quality-of-Service.md#BKMK_SetUpQoS)  
  
-   [How to create storage QoS policies](../Topic/Storage-Quality-of-Service.md#BKMK_CreateQoSPolicies)  
  
-   [How to identify and address common issues](../Topic/Storage-Quality-of-Service.md#BKMK_KnownIssues)  
  
-   [Monitor Health using Storage QoS](../Topic/Storage-Quality-of-Service.md#BKMK_Health)  
  
-   [Updates from previous version of Windows Server Technical Preview](../Topic/Storage-Quality-of-Service.md#BKMK_Updates)  
  
> [!IMPORTANT]  
> [!INCLUDE[winthreshold_server_2](../Token/winthreshold_server_2_md.md)] is an early pre\-release build. Features and scenarios are still in development and the experiences are still evolving. At this stage, [!INCLUDE[winthreshold_server_2](../Token/winthreshold_server_2_md.md)] and Storage QoS are for evaluations only and should not be used in production environments.  
  
> [!NOTE]  
> There are two changes [!INCLUDE[winthreshold_server_2](../Token/winthreshold_server_2_md.md)] (TP5) for Storage Quality of Service \(sQoS\). The policy type names have been changed in the PowerShell scripts. The new type names are **Dedicated** and **Aggregated**. This document includes more information on these features and also contains examples of configuring and managing them.  
...
Failover Cluster is required. All servers must be running the same version of [!INCLUDE[winthreshold_server_2](../Token/winthreshold_server_2_md.md)].  
...
![](../Image/Clustering_SOFSStorageQoS.png)  
...
This section discusses how to enable Storage QoS on either a new or an existing Failover Cluster and Scale\-Out File Server that is running [!INCLUDE[winthreshold_server_2](../Token/winthreshold_server_2_md.md)]  
  
#### Set up Storage QoS on a new installation  
If you have configured a new Failover Clusterand configured a Cluster Shared Volume\(CSV\) on [!INCLUDE[winthreshold_server_2](../Token/winthreshold_server_2_md.md)], then the Storage QoS feature will be set up automatically.  
...
![](../Image/Clustering_StorageQoSFCM.png)  
...
The Hyper\-V role in [!INCLUDE[winthreshold_server_2](../Token/winthreshold_server_2_md.md)] has built\-in support for Storage QoS and is set up by default.  
  
#### Install Remote Administration Tools to manage Storage QoS policies from remote computers  
You can manage Storage QoS policies and monitor flows from compute hosts using the Remote Server Administration Tools.  These are available as optional features on all [!INCLUDE[winthreshold_server_2](../Token/winthreshold_server_2_md.md)] installations, and can be downloaded separately for [!INCLUDE[winthreshold_client_2](../Token/winthreshold_client_2_md.md)] at the [Microsoft Download Center](http://www.microsoft.com/en-us/download/details.aspx?id=44280) website. .  
...
219-0769a4aba072 -PolicyType Aggregated -Name RestoredPolicy -MinimumIops 100 -MaximumIop  
s 2000  
...
VMs with invalid policies are also reported through the storage subsystem health monitoring.  Here is an example from the same state as described in [Finding VMs with invalid policies](../Topic/Storage-Quality-of-Service.md#BKMK_FindingVMsWithInvalidPoilicies) section of this document.  
...
There are 2 new features in this release of [!INCLUDE[winthreshold_server_2](../Token/winthreshold_server_2_md.md)]  
...
-   [Windows Server 2016 Technical Preview 5](../Topic/Windows-Server-2016-Technical-Preview-5.md)  
