---
title: STEP 4: Create the Network Load Balanced Remote Access Cluster_1
ms.custom: na
...
ms.tgt_pltfrm: na
ms.topic: article
ms.assetid: 0607a59a-e295-4b5c-aabe-a299d461e544
author: vhorne
---
# STEP 4: Create the Network Load Balanced Remote Access Cluster_1
[!INCLUDE[win8_server_1](../Token/win8_server_1_md.md)] enables you to create clusters of Remote Access servers. A cluster acts as a single logical server and provides centralized configuration and management for the servers in the cluster. When using Network Load Balancing \(NLB\) there is support for up to 8 Remote Access members in a single cluster. Remote Access clusters provide high availability and load balancing of connections from DirectAccess clients to the internal network.  
  
The following procedures enable you to create and test a Remote Access cluster:  
  
1.  [Install the Network Load Balancing feature on EDGE1 and EDGE2](../Topic/STEP-4--Create-the-Network-Load-Balanced-Remote-Access-Cluster_1.md#NLBfeature)—Before enabling load balancing, you must install the Network Load Balancing feature on both EDGE1 and EDGE2.  
  
2.  [Enable load balancing on EDGE1](../Topic/STEP-4--Create-the-Network-Load-Balanced-Remote-Access-Cluster_1.md#EnableNLB)—EDGE1 was originally installed in single server mode. To enable load balancing, you configure new external and internal dedicated IP addresses \(DIPs\) for EDGE1. The previous DIPs on EDGE1 are automatically configured as virtual IP addresses \(VIPs\) for the cluster. The new external DIP is 131.107.0.10, the new internal IPv4 DIP is 10.0.0.10, the new internal IPv6 DIP is 2001:db8:1::10. The cluster VIPs are 131.107.0.2 and 131.107.0.3 \(external\), and 10.0.0.2 and 2001:db8:1::2 \(internal\).  
  
3.  [Add EDGE2 to the load balanced cluster](../Topic/STEP-4--Create-the-Network-Load-Balanced-Remote-Access-Cluster_1.md#AddDA2)—After enabling load balancing, you can now add EDGE2 to the cluster to provide load balancing and high availability for DirectAccess client connections.  
...
1.  On EDGE1, in the [!INCLUDE[sm](../Token/sm_md.md)] console, in the **Dashboard**, click **Add roles and features**.  
...
1.  On EDGE1, click **Start**, type **RAMgmtUI.exe**, and then press ENTER. [!INCLUDE[uac_confirm_action](../Token/uac_confirm_action_md.md)]  
...
9. [!INCLUDE[clickstart](../Token/clickstart_md.md)]**nlbmgr.exe**, and the press ENTER. [!INCLUDE[uac_confirm_action](../Token/uac_confirm_action_md.md)]  
