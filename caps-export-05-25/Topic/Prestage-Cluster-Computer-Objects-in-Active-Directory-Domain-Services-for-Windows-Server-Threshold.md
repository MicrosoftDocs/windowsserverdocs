---
title: Prestage Cluster Computer Objects in Active Directory Domain Services for Windows Server Threshold
ms.custom: na
ms.reviewer: na
ms.suite: na
ms.tgt_pltfrm: na
ms.topic: article
ms.assetid: 13b58f28-4ee8-4b9e-b112-eebc380cbe92
---
# Prestage Cluster Computer Objects in Active Directory Domain Services for Windows Server Threshold
...
> [!NOTE]  
> In [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)], there is the option to create an Active Directory\-detached cluster, where no CNO or VCOs are created in AD DS. This is targeted for specific types of cluster deployments. For more information, see [Deploy an Active Directory-Detached Cluster](assetId:///21bc89d4-e744-4a94-8ee2-a9d93c68a804).  
...
-   [Step 1: Prestage the CNO in AD DS](#BKMK_PrestageCNO)  
  
-   [Step 2: Grant the user or group permissions to create the cluster](#BKMK_UserPerms)  
  
-   [Step 3: Grant the CNO permissions to the OU or prestage VCOs for clustered roles](#BKMK_CreateVCOs)  
  
## <a name="BKMK_PrestageCNO"></a>Step 1: Prestage the CNO in AD DS  
...
6.  Right\-click the computer account that you just created, and then click **Disable Account**. Click **Yes** to confirm, and then click **OK**.  
    ...
    ![](../Image/PrestageCNO.JPG)  
  
    **Figure 1. Disabled CNO in the example Clusters OU**  
  
## <a name="BKMK_UserPerms"></a>Step 2: Grant the user permissions to create the cluster  
...
5.  Select the user account or group that you just added, and then next to **Full control**, select the **Allow** check box.  
    ...
    ![](../Image/PrestageUserPermissions.JPG)  
...
## <a name="BKMK_CreateVCOs"></a>Step 3: Grant the CNO permissions to the OU or prestage VCOs for clustered roles  
...
-   Option 1: [Grant the CNO permissions to the OU](#BKMK_GrantCNOPerms). If you use this option, the cluster can automatically create VCOs in AD DS. Therefore, an administrator for the failover cluster can create clustered roles without having to request that you prestage VCOs in AD DS.  
  
    > [!NOTE]  
    > Membership in the **Domain Admins** group, or equivalent, is the minimum required to complete the steps for this option.  
  
-   Option 2: [Prestage VCOs for clustered roles](#BKMK_PrestageVCO). Use this option if it is necessary to prestage accounts for clustered roles because of requirements in your organization. For example, you may want to control the naming convention, or control which clustered roles are created.  
  
    > [!NOTE]  
    > Membership in the **Account Operators** group is the minimum required to complete the steps for this option.  
  
#### <a name="BKMK_GrantCNOPerms"></a>To grant the CNO permissions to the OU  
  
1.  In Active Directory Users and Computers, on the **View** menu, make sure that **Advanced Features** is selected.  
  
2.  Right\-click the OU where you created the CNO in [Step 1: Prestage the CNO in AD DS](#BKMK_PrestageCNO), and then click **Properties**.  
...
9. Under **Permissions**, select the **Create Computer objects** check box.  
    ...
    ![](../Image/PrestageCreateComputerObjects.JPG)  
...
##### <a name="BKMK_PrestageVCO"></a>To prestage a VCO for a clustered role  
