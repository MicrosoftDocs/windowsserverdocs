...
In all scenarios, the CSP gateway is a [!INCLUDE[winthreshold_server_2](../Token/winthreshold_server_2_md.md)] RAS Gateway at the edge. The RAS Gateway, which is capable of handling multiple connections from multiple tenants, consists of a Hyper\-V host and a virtual machine \(VM\) that is actually configured as the gateway. This edge gateway is configured with site\-to\-site VPN connections as a multitenant BGP router to exchange Enterprise and CSP subnet routes.  
...
For additional information, see [BGP Windows PowerShell Command Reference](../Topic/BGP-Windows-PowerShell-Command-Reference.md) and the Networking blog [Border Gateway Protocol (BGP) with Windows Server 2012 R2](http://blogs.technet.com/b/networking/archive/2013/10/11/border-gateway-protocol-bgp-with-windows-server-2012-r2.aspx).  
  
## See Also  
[RAS Gateway](../Topic/RAS-Gateway.md)  
[Remote Access](../Topic/Remote-Access.md)  
