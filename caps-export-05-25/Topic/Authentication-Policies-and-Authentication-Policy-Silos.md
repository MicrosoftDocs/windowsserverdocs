...
For information about configuring authentication policy silos and authentication policies, see [How to Configure Protected Accounts](../Topic/How-to-Configure-Protected-Accounts.md).  
...
Authentication policy silos can be configured by using the Active Directory Administrative Console or Windows PowerShell. For more information, see [How to Configure Protected Accounts](../Topic/How-to-Configure-Protected-Accounts.md).  
...
-   **User**  
    ...
    For more information see [Protected Users Security Group](../Topic/Protected-Users-Security-Group.md).  
...
Authentication policies can be configured for each silo by using the Active Directory Administrative Console or Windows PowerShell. For more information, see [How to Configure Protected Accounts](../Topic/How-to-Configure-Protected-Accounts.md).  
...
The Protected Users security group triggers non\-configurable protection on devices and host computers running [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] and [!INCLUDE[winblue_client_2](../Token/winblue_client_2_md.md)], and on domain controllers in domains with a primary domain controller running [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)]. Depending on the domain functional level of the account, members of the Protected Users security group are further protected because of changes in the authentication methods that are supported in Windows.  
  
-   The member of the Protected Users security group cannot authenticate by using NTLM, Digest Authentication, or CredSSP default credential delegation. On a device running [!INCLUDE[winblue_client_2](../Token/winblue_client_2_md.md)] that uses any one of these Security Support Providers \(SSPs\), authentication to a domain will fail when the account is a member of the Protected Users security group.  
...
For more information about this security group, see [How the Protected Users group works](../Topic/Protected-Users-Security-Group.md#BKMK_HowItWorks).  
...
-   [What's New in Kerberos Authentication](../Topic/What-s-New-in-Kerberos-Authentication.md) \(Windows Server 2012\)  
...
-   The domain controller in a domain running [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] queries for the user account and determines if it is configured with an authentication policy that restricts initial authentication that requires armored requests.  
  
-   The domain controller will fail the request.  
  
-   Because armoring is required, the user can attempt to sign in by using a computer running [!INCLUDE[winblue_client_2](../Token/winblue_client_2_md.md)] or Windows 8, which is enabled to support Kerberos armoring to retry the sign\-in process.  
...
Even when operating systems support Kerberos armoring, access control requirements can be applied and must be met before access is granted. Users sign in to Windows or enter their domain credentials in a credential prompt for an application. By default, Windows sends an unarmored AS\-REQ to the domain controller. If the user is sending the request from a computer that supports armoring, such as [!INCLUDE[winblue_client_2](../Token/winblue_client_2_md.md)] or Windows 8, authentication policies are evaluated as follows:  
  
1.  The domain controller in a domain running [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] queries for the user account and determines if it is configured with an authentication policy that restricts initial authentication that requires armored requests.  
...
2.  The domain controller in a domain running [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] looks up SPN1 to find the Active Directory Domain Services account for the service and determines that the account is configured with an authentication policy that restricts service ticket issuance.  
...
For troubleshooting steps that use these events, see [Troubleshoot Authentication Policies](../Topic/How-to-Configure-Protected-Accounts.md#BKMK_TroubleshootAuthnPolicies) and [Troubleshoot events related to Protected Users](../Topic/How-to-Configure-Protected-Accounts.md#BKMK_TrubleshootingEvents).  
...
## See also  
[How to Configure Protected Accounts](../Topic/How-to-Configure-Protected-Accounts.md)  
  
[Credentials Protection and Management](../Topic/Credentials-Protection-and-Management.md)  
  
[Protected Users Security Group](../Topic/Protected-Users-Security-Group.md)  
