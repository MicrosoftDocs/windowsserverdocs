...
This topic provides an overview and supporting procedures for restoring or repairing a server running [!INCLUDE[wsessentials](../Token/wsessentials_md.md)], and includes the following sections:  
  
-   [Overview of server system restores](../Topic/Restore-or-repair-your-server-running-Windows-Server-Essentials.md#BKMK_Overview)  
  
-   [Restore or repair the system drive](../Topic/Restore-or-repair-your-server-running-Windows-Server-Essentials.md#BKMK_Restore)  
  
-   [Restore files and folders on the server](../Topic/Restore-or-repair-your-server-running-Windows-Server-Essentials.md#BKMK_RestoreFilesAndFolders)  
...
-   [Restoring the server from a backup](../Topic/Restore-or-repair-your-server-running-Windows-Server-Essentials.md#BKMK_RestoreFromBackup)  
  
-   [Resetting the server to factory default settings](../Topic/Restore-or-repair-your-server-running-Windows-Server-Essentials.md#BKMK_FactoryReset)  
...
-   **Reconfigure the server.** On the server, use the Configure Server Wizard to reenter configuration settings. To configure a remotely managed [!INCLUDE[sbs_sbs8web_2](../Token/sbs_sbs8web_2_md.md)] server from a client computer, open a web browser, and then type **http:\/\/***<YourServerName>* in the address bar.  
  
-   **Reconnect client computers to the server.** If a computer was previously connected to the server, you must uninstall the [!INCLUDE[wsessentials](../Token/wsessentials_md.md)] Connector software from the computer before you connect the computer to the server again. For more information, see [Uninstall the Connector software](../Topic/Get-Connected-in-Windows-Server-Essentials.md#BKMK_13) and [Connect computers to the server](../Topic/Get-Connected-in-Windows-Server-Essentials.md#BKMK_9).  
...
-   [Restore or repair your server using installation media](../Topic/Restore-or-repair-your-server-running-Windows-Server-Essentials.md#BKMK_Restore_1). Use the installation media from the server manufacturer to restore from a backup.  
...
-   [Restore or reset your server from a client computer using the recovery DVD](../Topic/Restore-or-repair-your-server-running-Windows-Server-Essentials.md#BKMK_Restore_2). If you need to restore a remotely administered server that is running [!INCLUDE[sbs_sbs8web_2](../Token/sbs_sbs8web_2_md.md)], you must perform the restore from a client computer by using the restoration DVD from the server manufacturer.  
  
### <a name="BKMK_Restore_1"></a>Restore or repair your server using installation media  
The following procedure describes how to restore your server system drive from a backup by using the [!INCLUDE[wsessentials](../Token/wsessentials_md.md)] installation media. \(To find out how to use the installation media to restore to factory default settings, see the documentation from the server manufacturer.\)  
  
> [!NOTE]  
> If the server uses Storage Spaces, and you are restoring the data to a new server, you should recover the system drive first, and then log on to the [!INCLUDE[wsessentials](../Token/wsessentials_md.md)] Dashboard, configure Storage Spaces in a similar way as on the old server, and then recover the data volumes.  
  
##### To restore the server system drive from a backup using installation media  
  
1.  Insert the [!INCLUDE[wsessentials](../Token/wsessentials_md.md)] installation DVD in the server DVD drive, restart the server, and then press any key to start from the DVD.  
...
> [!NOTE]  
> To restore and share folders on the server, you may need to take additional steps. For more information, see [Restore files and folders on the server](../Topic/Restore-or-repair-your-server-running-Windows-Server-Essentials.md#BKMK_RestoreFilesAndFolders).  
  
### <a name="BKMK_Restore_2"></a>Restore or reset your server from a client computer using the recovery DVD  
In [!INCLUDE[sbs_sbs8web_2](../Token/sbs_sbs8web_2_md.md)], you can start the server from a bootable USB flash drive that you create, and then you recover the server from a client computer by using the recovery DVD that you received from the server manufacturer. The client computer must be on the same network as the server. This method is not available in [!INCLUDE[wseblue_2](../Token/wseblue_2_md.md)].  
...
1.  Insert the [!INCLUDE[sbs_sbs8web_2](../Token/sbs_sbs8web_2_md.md)] server recovery media that you received from the server manufacturer in a client computer.  
...
> [!NOTE]  
> You cannot use the Restore Files and Folders Wizard to restore the full system drive. For information about how to restore the full system, see [Restore or repair your server using installation media](../Topic/Restore-or-repair-your-server-running-Windows-Server-Essentials.md#BKMK_Restore_1) or [Restore or reset your server from a client computer using the recovery DVD](../Topic/Restore-or-repair-your-server-running-Windows-Server-Essentials.md#BKMK_Restore_2).  
  
##### To restore files and folders from a server backup  
  
1.  Open the [!INCLUDE[wsessentials](../Token/wsessentials_md.md)] Dashboard, and then click the **Devices** tab.  
...
> [!WARNING]  
> For more information about backing up and restoring files and folders, see [Manage Backup and Restore in Windows Server Essentials](../Topic/Manage-Backup-and-Restore-in-Windows-Server-Essentials.md).  
...
3.  Log on to the [!INCLUDE[wsessentials](../Token/wsessentials_md.md)] Dashboard, click the **Storage** tab, and then click **Add a folder** in the **Server Folders Tasks** pane.  
...
7.  Specify the permissions that you wrote down in step 2, and then click **Add folder**.  
    ...
    > [!IMPORTANT]  
    > These permissions will replace any existing permissions that were not added to the folder by using the [!INCLUDE[wsessentials](../Token/wsessentials_md.md)] Dashboard.  
  
> [!IMPORTANT]  
> After you finish adding folders to the list of shared folders, ensure that the folders are included in the server backup. For information about adding folders to the server backup, see [Set up or customize server backup](../Topic/Set-up-or-customize-server-backup.md).  
