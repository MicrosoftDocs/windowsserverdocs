---
title: Step 2: Configure the Remote Access Server_1
ms.custom: na
...
ms.tgt_pltfrm: na
ms.topic: article
ms.assetid: ab8c0761-5ed0-4018-9c69-b4b1af28ddf3
author: vhorne
---
# Step 2: Configure the Remote Access Server_1
[!INCLUDE[remote_access_combo](../Token/remote_access_combo_md.md)]  
  
This topic describes how to configure the client and server settings that are required for remote management of DirectAccess clients. Before you begin the deployment steps, ensure that you have completed the planning steps that are described in [Step 2: Plan the Remote Access Deployment_1](../Topic/Step-2--Plan-the-Remote-Access-Deployment_1.md).  
  
|Task|Description|  
|--------|---------------|  
|[Install the Remote Access role](../Topic/Step-2--Configure-the-Remote-Access-Server_1.md#BKMK_Role)|Install the Remote Access role.|  
|[Configure the deployment type](../Topic/Step-2--Configure-the-Remote-Access-Server_1.md#BKMK_Deploy)|Configure the deployment type as DirectAccess and VPN, DirectAccess only, or VPN only.|  
|[Configure DirectAccess clients](../Topic/Step-2--Configure-the-Remote-Access-Server_1.md#BKMK_Clients)|Configure the Remote Access server with the security groups that contain DirectAccess clients.|  
|[Configure the Remote Access server](../Topic/Step-2--Configure-the-Remote-Access-Server_1.md#BKMK_Server)|Configure the Remote Access server settings.|  
|[Configure the infrastructure servers](../Topic/Step-2--Configure-the-Remote-Access-Server_1.md#BKMK_Infra)|Configure the infrastructure servers that are used in the organization.|  
|[Configure application servers](../Topic/Step-2--Configure-the-Remote-Access-Server_1.md#BKMK_App)|Configure the application servers to require authentication and encryption.|  
|[Configuration summary and alternate GPOs](../Topic/Step-2--Configure-the-Remote-Access-Server_1.md#BKMK_GPO)|View the Remote Access configuration summary, and modify the GPOs if desired.|  
...
1.  On the server that will act as the Remote Access server, in the [!INCLUDE[sm](../Token/sm_md.md)] console, in the **Dashboard**, click **Add roles and features**.  
  
2.  Click **Next** three times to get to the server role selection screen.  
  
3.  In the **Select server roles** dialog box, select **Remote Access**, click **Add Features**, and then click **Next**.  
  
4.  Click **Next** five times.  
  
5.  In the **Confirm installation selections** dialog box, click **Install**.  
  
6.  In the **Installation progress** dialog box, verify that the installation was successful, and then click **Close**.  
...
1.  On the Remote Access server, open the Remote Access Management console: [!INCLUDE[startscreen](../Token/startscreen_md.md)], type **Remote Access Management Console**, and then press ENTER. [!INCLUDE[uac_confirm_action](../Token/uac_confirm_action_md.md)]  
...
6.  On the **Authentication** page:  
    ...
    -   To enable client computers running [!INCLUDE[nextref_client_7](../Token/nextref_client_7_md.md)] to connect via DirectAccess, select the **Enable Windows 7 client computers to connect via DirectAccess** check box. You must also use computer certificate authentication in this type of deployment.  
