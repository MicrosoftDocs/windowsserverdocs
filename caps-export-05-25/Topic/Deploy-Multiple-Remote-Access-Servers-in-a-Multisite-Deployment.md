...
[!INCLUDE[winthreshold_server_1](../Token/winthreshold_server_1_md.md)] and [!INCLUDE[win8_server_1](../Token/win8_server_1_md.md)] combine DirectAccess and Routing and Remote Access Service \(RRAS\) VPN into a single Remote Access role. Remote Access can be deployed in a number of enterprise scenarios. This overview provides an introduction to the enterprise scenario for deploying Remote Access servers in a multisite configuration.  
...
A multisite deployment supports client computers running [!INCLUDE[winthreshold_client_1](../Token/winthreshold_client_1_md.md)], [!INCLUDE[win8_client_1](../Token/win8_client_1_md.md)] or [!INCLUDE[nextref_client_7](../Token/nextref_client_7_md.md)]. Client computers running [!INCLUDE[winthreshold_client_1](../Token/winthreshold_client_2_md.md)] or [!INCLUDE[win8_client_1](../Token/win8_client_2_md.md)] automatically identify an entry point, or the user can manually select an entry point. Automatic assignment occurs in the following priority order:  
...
-   Windows 7 clients will always connect to a specific site. They will not be able to connect to the closest site based on location of the client \(unlike [!INCLUDE[winthreshold_client_1](../Token/winthreshold_client_2_md.md)], 8, or 8.1 clients\).  
...
-   In order to test the scenario, at least one computer running [!INCLUDE[win8_client_2](../Token/win8_client_2_md.md)] and configured as a DirectAccess client is required. To test the scenario for clients running Windows 7 at least one computer running Windows 7 is required.  
...
-   In addition to software requirements for a single server there are a number of multisite\-specific requirements:  
    ...
    -   Security group requirements—Requirements are as follows:  
        ...
        -   A single security group is required for all [!INCLUDE[win8_client_2](../Token/win8_client_2_md.md)] client computers from all domains. It is recommended to create a unique security group of these clients for each domain.  
...
-   When using the Windows PowerShell cmdlet **Disable\-DAMultiSite**, the WhatIf and Confirm parameters have no effect, and Multisite will be disabled and [!INCLUDE[nextref_client_7](../Token/nextref_client_7_md.md)] GPOs will be removed.  
  
-   When [!INCLUDE[nextref_client_7](../Token/nextref_client_7_md.md)] clients using DCA in a Multisite deployment are upgraded to [!INCLUDE[win8_client_2](../Token/win8_client_2_md.md)], the Network Connectivity Assistant will not function. This issue can be resolved in advance of the client upgrade by modifying the [!INCLUDE[nextref_client_7](../Token/nextref_client_7_md.md)] GPOs using the following Windows PowerShell cmdlets:  
    ...
    In the event that the client has already been upgraded, then move the client computer to the [!INCLUDE[win8_client_2](../Token/win8_client_2_md.md)] security group.  
