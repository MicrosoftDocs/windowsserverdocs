...
-   **Basic—Redirect everyone's folder to the same location**. This setting enables you to redirect everyone's folder to the same location and is applied to all users included in the [!INCLUDE[gpo](../Token/gpo_md.md)]. For this setting, you have the following options in specifying a target folder location:  
...
1.  In the GPMC console tree, right\-click the [!INCLUDE[gpo](../Token/gpo_md.md)] that is linked to the site, domain, or organizational unit that contains the users whose user profile folders you want to redirect, and then click **Edit**.  
