...
Allows you to generate a rollback template for a specified configuration template. For examples of how this command can be used, see [Examples](../Topic/Secedit-generaterollback.md#BKMK_Examples).  
...
|rbk|Required.<br /><br />Specifies a security template into which the rollback information is written. Security templates are created using the Security Templates snap\-in. Rollback files can be created with this command.|  
...
Beginning with Windows Server 2008, `Secedit /refreshpolicy` has been replaced with `gpupdate`. For information on how to refresh security settings, see [Gpupdate](../Topic/Gpupdate.md).  
