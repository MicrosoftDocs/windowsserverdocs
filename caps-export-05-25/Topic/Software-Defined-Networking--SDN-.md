...
>   
> -   [Software Defined Networking Technologies](../Topic/Software-Defined-Networking-Technologies.md)  
...
For more information, see [Hyper-V Network Virtualization](../Topic/Hyper-V-Network-Virtualization.md).  
...
In Hyper\-V Virtual Switch in [!INCLUDE[winthreshold_server_2](../Token/winthreshold_server_2_md.md)], you can also deploy Switch Embedded Teaming \(SET\) and Remote Direct Memory Access \(RDMA\). For more information, see the section [Remote Direct Memory Access (RDMA) and Switch Embedded Teaming (SET)](#bkmk_rdma) in this topic.  
  
For more information about Hyper\-V Virtual Switch, see [Hyper-V Virtual Switch](../Topic/Hyper-V-Virtual-Switch.md).  
...
In [!INCLUDE[winthreshold_server_2](../Token/winthreshold_server_2_md.md)], you can  enable RDMA on network adapters that are  bound to a Hyper\-V Virtual Switch with or without Switch Embedded Teaming \(SET\). This allows you to use fewer network adapters when you want to use RDMA and SET at the same time.  
  
SET is an alternative NIC Teaming solution that you can use in environments that include Hyper\-V and the Software Defined Networking \(SDN\) stack in [!INCLUDE[winthreshold_server_2](../Token/winthreshold_server_2_md.md)]. SET integrates some of the NIC Teaming functionality into the Hyper\-V Virtual Switch.  
...
RAS Gateway is a software\-based, multitenant, Border Gateway Protocol \(BGP\) capable router in [!INCLUDE[winthreshold_server_2](../Token/winthreshold_server_2_md.md)] that is designed for Cloud Service Providers \(CSPs\) and Enterprises that host multiple tenant virtual networks using Hyper\-V Network Virtualization.  
...
When you configure network adapters into a NIC team, they are connected into the NIC teaming solution common core, which then presents one or more virtual adapters \(also called team NICs \[tNICs\] or team interfaces\) to the operating system. [!INCLUDE[winthreshold_server_2](../Token/winthreshold_server_2_md.md)] supports up to 32 team interfaces per team. There are a variety of algorithms that distribute outbound traffic \(load\) between the NICs.  
  
In addition, you can use NIC Teaming with both VMs and VLANs.  
  
For more information, see [NIC Teaming](../Topic/NIC-Teaming.md) and [Windows Server 2012 R2 NIC Teaming (LBFO) Deployment and Management](https://www.microsoft.com/en-us/download/details.aspx?id=40319).  
