...
-   [Walkthrough: Workplace Join with an iOS Device](../Topic/Walkthrough--Workplace-Join-with-an-iOS-Device.md)  
  
-   [Walkthrough: Workplace Join with a Windows Device](../Topic/Walkthrough--Workplace-Join-with-a-Windows-Device.md)  
  
-   [Walkthrough Guide: Connect to Applications and Services from Anywhere with Web Application Proxy](../Topic/Walkthrough-Guide--Connect-to-Applications-and-Services-from-Anywhere-with-Web-Application-Proxy.md)  
...
1.  [Step 1: Configure the domain controller \(DC1\)](../Topic/Set-up-the-lab-environment-for-AD-FS-in-Windows-Server-2012-R2.md#BKMK_1)  
  
2.  [Step 2: Configure the federation server \(ADFS1\) with Device Registration Service](../Topic/Set-up-the-lab-environment-for-AD-FS-in-Windows-Server-2012-R2.md#BKMK_4)  
  
3.  [Step 3: Configure the web server \(WebServ1\) and a sample claims\-based application](../Topic/Set-up-the-lab-environment-for-AD-FS-in-Windows-Server-2012-R2.md#BKMK_5)  
  
4.  [Step 4: Configure the client computer \(Client1\)](../Topic/Set-up-the-lab-environment-for-AD-FS-in-Windows-Server-2012-R2.md#BKMK_10)  
...
-   Install the [!INCLUDE[ad_adds_3](../Token/ad_adds_3_md.md)] role service and install Active Directory Domain Services \(AD DS\) to make your computer a domain controller in [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)]. This action upgrades your [!INCLUDE[ad_adds_3](../Token/ad_adds_3_md.md)] schema as part of the domain controller creation. For more information and step\-by\-step instructions, see[http:\/\/technet.microsoft.com\/ library\/hh472162.aspx](http://technet.microsoft.com/library/hh472162.aspx).  
...
The group Managed Service Account \(GMSA\) account is required during the Active Directory Federation Services \([!INCLUDE[nextref_adfs2](../Token/nextref_adfs2_md.md)]\) installation and configuration.  
...
To set up another virtual machine, install [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] and connect it to the domain **contoso.com**. Set up the computer after you have joined it to the domain, and then proceed to install and configure the [!INCLUDE[nextref_adfs2](../Token/nextref_adfs2_md.md)] role.  
...
1.  > [!IMPORTANT]  
    ...
    Open a [!INCLUDE[wps_2](../Token/wps_2_md.md)] command window and type:  
    ...
    Now run the [!INCLUDE[wps_2](../Token/wps_2_md.md)] cmdlet.  
...
|Entry|Type|Address|  
|---------|--------|-----------|  
|adfs1|Host \(A\)|IP address of the [!INCLUDE[nextref_adfs2](../Token/nextref_adfs2_md.md)] server|  
...
Set up a virtual machine \(WebServ1\) by installing the [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] operating system and connect it to the domain **contoso.com**. After it is joined to the domain, you can proceed to install and configure the Web Server role.  
...
> [!NOTE]  
> These steps have been tested on a web server that runs the [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] operating system.  
  
1.  [Install the Web Server Role and Windows Identity Foundation](../Topic/Set-up-the-lab-environment-for-AD-FS-in-Windows-Server-2012-R2.md#BKMK_15)  
  
2.  [Install Windows Identity Foundation SDK](../Topic/Set-up-the-lab-environment-for-AD-FS-in-Windows-Server-2012-R2.md#BKMK_13)  
  
3.  [Configure the simple claims app in IIS](../Topic/Set-up-the-lab-environment-for-AD-FS-in-Windows-Server-2012-R2.md#BKMK_9)  
  
4.  [Create a relying party trust on your federation server](../Topic/Set-up-the-lab-environment-for-AD-FS-in-Windows-Server-2012-R2.md#BKMK_11)  
  
### <a name="BKMK_15"></a>Install the Web Server role and Windows Identity Foundation  
  
1.  > [!NOTE]  
    > You must have access to the [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] installation media.  
...
10. On the **Confirm installation selections** page, click **Specify an alternate source path**. Enter the path to the Sxs directory that is located in the [!INCLUDE[winblue_server_2](../Token/winblue_server_2_md.md)] installation media. For example D:\\Sources\\Sxs. Click **OK**, and then click **Install**.  
...
Set up another virtual machine and install [!INCLUDE[winblue_client_2](../Token/winblue_client_2_md.md)]. This virtual machine must be on the same virtual network as the other machines. This machine should NOT be joined to the Contoso domain.  
  
The client MUST trust the SSL certificate that is used for the federation server \(ADFS1\), which you set up in [Step 2: Configure the federation server \(ADFS1\) with Device Registration Service](../Topic/Set-up-the-lab-environment-for-AD-FS-in-Windows-Server-2012-R2.md#BKMK_4). It must also be able to validate certificate revocation information for the certificate.  
